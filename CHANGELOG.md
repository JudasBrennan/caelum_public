...
**Manual planet and moon composition inventories**
(engine/rockyBodyComposition.js, engine/planet.js, engine/moon.js,
ui/compositionEditor.js, ui/planetPage.js, ui/moonPage.js,
ui/store/compositionInventoryInputs.js)

Planets and moons now share a rocky-body composition inventory with inferred,
reservoir, and expert-element modes. Reservoir inputs model metal, silicate,
water ice, volatile ice, carbonaceous material, sulfur, and salts as
structure-relevant body fractions, while expert element inputs expose Fe, Ni,
Mg, Si, O, H, C, N, S, Na, Cl, Al, Ca, and trace heat-producing elements for
chemistry and diagnostics.

The manual modes preserve existing inferred behavior by default. When users
author reservoirs, planets can route effective core and water fractions into
the existing radius and density path; moons keep bulk density as the radius
source while using the reservoirs for composition, material response,
hydrosphere priors, and validation.

**Suggested sensible composition values**
(engine/rockyBodyCompositionSuggestions.js, ui/compositionEditor.js,
ui/store/bodyMutations.js)

The shared composition editor now includes `Seed sensible values` and
`Sync elements from reservoirs` actions. Suggestions are generated by engine
helpers from the current inferred composition, body state, moon class,
hydrosphere fields, parent context, and bounded stellar metallicity context
where available. The seed fills reservoir, element, and trace boxes as editable
starting points instead of continuously overwriting user inputs.

Suggested values can fill empty boxes or replace all manual values, store
stable provenance metadata, and show a non-blocking stale hint when relevant
body inputs change afterward.

**Composition-aware downstream science**
(engine/compositionCoupling.js, engine/environment/oceanChemistry.js,
engine/environment/atmosphereLedger.js,
engine/contexts/interiorEvolutionContext.js, engine/planet/magnetism.js,
engine/moon/geology.js, engine/moon/magnetosphere.js, ui/moonStyles.js,
ui/rockyPlanetStyles.js)

Manual inventories now feed supported downstream contexts. Salts, sulfur,
carbonaceous material, water, volatile inventory, trace heat, metal fraction,
and major elements can influence ocean chemistry, atmosphere source ledgers,
interior heat context, rocky planet magnetism, moon geology, moon
magnetosphere diagnostics, and visual/material cues without treating
element-only data as a full mineral-equilibrium solver.

### Changed

- Planet and moon guided creation, store defaults, persistence compatibility,
  and world migration helpers now preserve manual composition fields and
  suggestion metadata.
- Planet and moon outputs now distinguish inferred, reservoir, and
  expert-element sources and display composition validation messages beside
  the bulk and element mix summaries.
- The completed manual composition implementation plan moved from outstanding
  plans to complete plans for this release.

### Fixed

- Seeded planet inventories now compare their manual density proxy against the
  same compressed-density expectation used by the rocky-body solver, avoiding
  false cautions where massive rocky planets naturally exceed uncompressed
  mixture densities.
- Moon composition warnings now remain advisory when density, class, and manual
  inventory disagree, instead of silently forcing planet-style structure
  assumptions onto moons.

### Tests

- Added or updated composition inventory, suggestion, downstream coupling,
  editor UI, moon page composition, planet render, body mutation, guided
  adapter, migration, planet, and moon tests.
- Regenerated the Science Verification Matrix and compatibility calibration
  artifacts for 3.1.0.
- Ran release verification so syntax, lint, dependency, bundle, science matrix,
  and browser-facing gates cover the 3.1.0 build.
...
The splash screen now shows the Enter button immediately, starts the main app
behind the overlay, and falls back to "3D preview unavailable" if the decorative
planet preview hangs. This keeps WorldSmith usable in browsers where the
GLTF/Draco preview path or an injected script fails without affecting the main
app startup path.
