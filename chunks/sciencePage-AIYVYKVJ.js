import{a as he,b as fe,c as ee}from"./chunk-XCN227BY.js";import{b as pe,d as ue,e as me}from"./chunk-KD5CBWLO.js";import{a as de}from"./chunk-RNZKX373.js";import{d as ce}from"./chunk-FMNLRRDG.js";import{a as O}from"./chunk-WYZYYRUA.js";import{d as le}from"./chunk-3MZTWI7A.js";import{b as J,c as Y}from"./chunk-XMLMEZIZ.js";import{a as E,b as z}from"./chunk-WNGVR2CK.js";import{K as X,N as oe,O as ne}from"./chunk-PEDZU4MZ.js";import{j as m}from"./chunk-T2JC7ILW.js";import"./chunk-24KHYB4L.js";import"./chunk-VC46IEJQ.js";function j(i,r){return i&&(i.textContent=r==null?"":String(r),i)}function ge(i,{countLabel:r="32",countValue:l="",alphaValue:d=""}={}){return i&&(Y(i,["N",J("sub",{text:r}),` = ${l} \u2014 \u03B1 = ${d}`]),i)}function U(i,r=[],l=""){if(!i)return i;let d=(r||[]).filter(Boolean);if(!d.length)return i.textContent=l==null?"":String(l),i;let n=[];return d.forEach((u,f)=>{f>0&&n.push(J("br")),n.push(J("strong",{text:u?.fraction||""}),` \u2014 ${u?.description||""}`)}),Y(i,n),i}function t(i){return`<span class="sci-math sci-math--block">${i}</span>`}function e(i){return`<span class="sci-math">${i}</span>`}function o(i){return`<table class="sci-vars"><tbody>${i.map(([r,l])=>`<tr><td>${e(r)}</td><td>${l}</td></tr>`).join("")}</tbody></table>`}function s(i){return`<p class="sci-cite">${i}</p>`}function a(i,r){return`<div class="sci-formula"><h3 class="sci-formula__name">${i}</h3>${r}</div>`}function p(i,r){return`<table class="sci-data"><thead><tr>${i.map(l=>`<th>${l}</th>`).join("")}</tr></thead><tbody>${r.map(l=>`<tr>${l.map(d=>`<td>${d}</td>`).join("")}</tr>`).join("")}</tbody></table>`}var Z=Object.freeze({tempMinK:240,tempMaxK:715,pressureMinGPa:.05,pressureMaxGPa:10,left:104,right:664,top:44,bottom:300});function ve(i,r,l){return Math.min(l,Math.max(r,i))}function B(i){let{tempMinK:r,tempMaxK:l,left:d,right:n}=Z,u=(Number(i)-r)/(l-r);return d+ve(u,0,1)*(n-d)}function V(i){let{pressureMinGPa:r,pressureMaxGPa:l,top:d,bottom:n}=Z,u=Math.log10(r),f=Math.log10(l),v=(Math.log10(Math.max(Number(i),r))-u)/(f-u);return n-ve(v,0,1)*(n-d)}function xe(i,r){return`${m(i,1)},${m(r,1)}`}function $e(i,r,l=3){let d=[];for(let u=i;u<=r;u+=l){let f=z(u);!f.validRange||!Number.isFinite(f.pressurePa)||d.push({tempK:u,pressureGPa:f.pressurePa/1e9,x:B(u),y:V(f.pressurePa/1e9)})}let n=z(r);return n.validRange&&Number.isFinite(n.pressurePa)&&d.push({tempK:r,pressureGPa:n.pressurePa/1e9,x:B(r),y:V(n.pressurePa/1e9)}),d}function Se(i){return i.length?`M ${i.map(r=>xe(r.x,r.y)).join(" L ")}`:""}function Me(){let i=[250,300,350,400,500,600,700],r=[.1,.3,.6,1,2.2,5,10],{left:l,right:d,top:n,bottom:u}=Z;return[...i.map(f=>{let v=B(f);return`<g class="sci-phase-gridline"><line x1="${m(v,1)}" y1="${n}" x2="${m(v,1)}" y2="${u}" /><text x="${m(v,1)}" y="${u+24}">${f} K</text></g>`}),...r.map(f=>{let v=V(f);return`<g class="sci-phase-gridline"><line x1="${l}" y1="${m(v,1)}" x2="${d}" y2="${m(v,1)}" /><text class="sci-phase-y-tick" x="${l-14}" y="${m(v+4,1)}">${f>=1?m(f,1):m(f,1)} GPa</text></g>`})].join("")}function we(){return[{tempK:288,pressureGPa:.52,label:"0.52 GPa / 288 K",note:"below liquidus",className:"liquid",dx:10,dy:-12},{tempK:288,pressureGPa:1.2,label:"1.2 GPa / 288 K",note:"ice VI stable",className:"ice",dx:10,dy:-12},{tempK:500,pressureGPa:2.3,label:"2.3 GPa / 500 K",note:"hot deep liquid",className:"warm",dx:10,dy:18}].map(r=>{let l=B(r.tempK),d=V(r.pressureGPa);return`<g class="sci-phase-marker sci-phase-marker--${r.className}" transform="translate(${m(l,1)} ${m(d,1)})">
        <circle r="5" />
        <text x="${r.dx}" y="${r.dy}">${r.label}</text>
        <text x="${r.dx}" y="${r.dy+14}" class="sci-phase-marker__note">${r.note}</text>
      </g>`}).join("")}function qe(){let{left:i,right:r,top:l,bottom:d}=Z,n=r-i,u=d-l,f=i+n/2,v=l+u/2,w=$e(E.iceIhIceIiiLiquid.tempK,715),M=Se(w),P=w[0],R=w[w.length-1],T=P&&R?`${M} L ${r},${l} L ${i},${l} Z`:"",A=[{label:"III",tempK:E.iceIhIceIiiLiquid.tempK,pressureGPa:E.iceIhIceIiiLiquid.pressurePa/1e9},{label:"V",tempK:E.iceIiiIceVLiquid.tempK,pressureGPa:E.iceIiiIceVLiquid.pressurePa/1e9},{label:"VI",tempK:E.iceVIceViLiquid.tempK,pressureGPa:E.iceVIceViLiquid.pressurePa/1e9},{label:"VII",tempK:E.iceViIceViiLiquid.tempK,pressureGPa:E.iceViIceViiLiquid.pressurePa/1e9}].map(G=>{let H=B(G.tempK),W=V(G.pressureGPa);return`<g class="sci-phase-triple">
        <circle cx="${m(H,1)}" cy="${m(W,1)}" r="4" />
        <text x="${m(H+7,1)}" y="${m(W-7,1)}">ice ${G.label}</text>
    ...
    <svg class="sci-phase-diagram" viewBox="0 0 700 360" role="img" aria-label="WorldSmith ocean-floor water phase diagram">
      ...
      <rect x="${i}" y="${l}" width="${m(B(251.165)-i,1)}" height="${u}" class="sci-phase-unsupported" />
      ${Me()}
      ...
      <text x="${r-220}" y="${d-30}" class="sci-phase-region sci-phase-region--liquid">Liquid remains stable below boundary</text>
    </svg>
    <div class="sci-phase-legend" aria-label="Diagram legend">
  ...
  </div>`}function N(i){return String(i??"").trim().toLowerCase()}function Te(i,r){if(!r)return 0;let l=N(i.title);if(l===r)return 120;if(l.startsWith(r))return 90;if(l.includes(r))return 70;let d=N(i.sectionTitle);return d===r?52:d.includes(r)?40:N(i.text).includes(r)?18:0}function Ce(i){return Array.from(i.querySelectorAll(".sci-section")).flatMap(r=>{let l=String(r.id||"").trim(),d=r.querySelector(".sci-section__title")?.textContent?.trim()||"Science Section";return Array.from(r.querySelectorAll(".sci-formula")).map((n,u)=>{let f=`${l}-formula-${u+1}`;return n.id=f,n.setAttribute("tabindex","-1"),{id:f,sectionId:l,sectionTitle:d,title:n.querySelector(".sci-formula__name")?.textContent?.trim()||"Formula",text:n.textContent||""}})})}function be(i,r,l=8){let d=N(r);return d?i.map(n=>({entry:n,score:Te(n,d)})).filter(n=>n.score>0).sort((n,u)=>u.score-n.score||n.entry.sectionTitle.localeCompare(u.entry.sectionTitle)||n.entry.title.localeCompare(u.entry.title)).slice(0,l).map(n=>n.entry):[]}function ke(){return[a("Mass-Luminosity Relation",`<div class="sci-formula__eq">${t("L = c \\cdot M^{\\alpha}")}</div>
      <p>Six-piece empirical power law relating stellar mass to luminosity, fitted to 509 eclipsing binary components.</p>
      ${p(["Mass range (M&#9737;)","\\(\\alpha\\)","c"],[["M &lt; 0.45","2.028","0.0892"],["0.45 &le; M &lt; 0.72","4.572","0.68"],["0.72 &le; M &lt; 1.05","5.743","1.0"],["1.05 &le; M &lt; 2.4","4.329","1.072"],["2.4 &le; M &lt; 7.0","3.967","1.471"],["M &ge; 7.0","2.865","12.55"]])}
      ...
      ${p(["Class","T range (K)","Span"],[["O","33,000 &ndash; 95,000","62,000"],["B","10,000 &ndash; 33,000","23,000"],["A","7,500 &ndash; 10,000","2,500"],["F","6,000 &ndash; 7,500","1,500"],["G","5,200 &ndash; 6,000","800"],["K","3,700 &ndash; 5,200","1,500"],["M","2,000 &ndash; 3,700","1,700"]])}`),a("Stellar Environment Diagnostics",`<div class="sci-formula__eq">${t("Ro = P_{\\text{rot}} / \\tau_c")}</div>
      ...
      ${p(["Boundary used","Temperature range","Anchor in model"],[["ice III / liquid","251.165&ndash;256.164 K","0.209&ndash;0.350 GPa"],["ice V / liquid","256.164&ndash;273.31 K","0.350&ndash;0.632 GPa"],["ice VI / liquid","273.31&ndash;355 K","0.632&ndash;2.216 GPa"],["ice VII / liquid","355&ndash;715 K",`${m(z(550).pressurePa/1e9,2)} GPa at 550 K`]])}
      ...
      ${p(["Gas","Formula","m<sub>i</sub> (kg/mol)"],[["N&#8322;","remainder (100 &minus; &sum; others)","0.028"],["O&#8322;","user input","0.032"],["CO&#8322;","user input","0.044"],["Ar","user input","0.040"],["H&#8322;O","user input","0.018"],["CH&#8324;","user input","0.016"],["H&#8322;","user input (Full mode)","0.002"],["He","user input (Full mode)","0.004"],["SO&#8322;","user input (Full mode)","0.064"],["NH&#8323;","user input (Full mode)","0.017"]])}
      ...
      ${p(["Rotation period","Cells"],[["&ge; 48 h","1 (single Hadley)"],["6 &ndash; 48 h","3 (Earth-like)"],["3 &ndash; 6 h","7 (rapid rotator)"],["&lt; 3 h","5 (very rapid)"]])}`)].join("")}function Ae(){return[a("Mass&ndash;Radius Relation",`<p>Two-regime power law calibrated to Solar System giants:</p>
      ...
      ${p(["Class","T<sub>eq</sub> (K)","Cloud deck","Albedo"],[["I","&le; 150","NH&#8323; ice","0.34"],["II","150&ndash;250","H&#8322;O","0.81"],["III","250&ndash;800","Cloudless","0.12"],["IV","800&ndash;1400","Alkali metals","0.10"],["V","&gt; 1400","Silicate/iron","0.55"]])}
      ...
      ${p(["Species","T<sub>cond</sub> (K)","Pressure level"],[["Iron (Fe)","1800","0.01 bar"],["Silicate (MgSiO&#8323;)","1400","0.1 bar"],["H&#8322;O","300","5 bar"],["NH&#8324;SH","200","2 bar"],["NH&#8323;","150","0.7 bar"],["CH&#8324; (ice giants)","80","1.5 bar"]])}
      ...
      ${p(["Planet","Model B (G)","Observed B (G)","Ratio"],[["Jupiter","4.28","4.28","1.00&times;"],["Saturn","~0.21","0.21","~0.99&times;"],["Uranus","~0.24","0.23","~1.02&times;"],["Neptune","~0.14","0.14","~1.00&times;"]])}
      ...
      ${p(["Planet","Model R<sub>mp</sub>","Observed R<sub>mp</sub>","Error"],[["Jupiter","75 Rp","75 Rp","~0%"],["Saturn","22 Rp","22 Rp","~0%"],["Uranus","19 Rp","18 Rp","~3%"],["Neptune","18 Rp","23 Rp","~21%"]])}
      ...
      ${p(["Body","Mass (M<sub>J</sub>)","Model k&#8322;","Observed k&#8322;","Source"],[["Jupiter","1.0","0.385","0.379","Wahl+ 2016 (Juno)"],["Saturn","0.30","0.385","0.390","Lainey+ 2017 (Cassini)"],["Uranus","0.046","0.104","0.104","Gavrilov &amp; Zharkov 1977"],["Neptune","0.054","0.128","0.127","Gavrilov &amp; Zharkov 1977"]])}
      ...
      ${p(["Regime","Mass range","Q","Analogue"],[["Ice giant","&lt; 0.15 M<sub>J</sub>","15,000","Uranus, Neptune"],["Saturn-like","0.2&ndash;0.5 M<sub>J</sub>","2,500","Saturn (resonance locking)"],["Jupiter-like","&ge; 0.8 M<sub>J</sub>","35,000","Jupiter"]])}
      ...
      ${p(["Label","Separation","Meaning"],[["Stable","&ge; 10 R<sub>H,m</sub>","Comfortably separated for this simplified diagnostic"],["Packed","6 to &lt; 10 R<sub>H,m</sub>","Dynamically close but often plausible"],["Crowded","2&radic;3 to &lt; 6 R<sub>H,m</sub>","Long-term stability is questionable"],["Unstable","&lt; 2&radic;3 R<sub>H,m</sub>","Below the circular coplanar Hill-stability boundary"]])}
      ...
      ${p(["Species","T_{sub} (K)","\\rho_{max}","Source"],[["N\\u2082","35","2.5 g/cm\\u00B3","Fray & Schmitt (2009)"],["CO","35","2.5 g/cm\\u00B3",""],["CH\\u2084","50","2.5 g/cm\\u00B3",""],["CO\\u2082","115","3.2 g/cm\\u00B3",""],["NH\\u2083","130","2.5 g/cm\\u00B3",""],["SO\\u2082","140","volcanic",""],["H\\u2082O","210","3.2 g/cm\\u00B3",""]])}
      ...
      are shown as dimmed amber diamonds.</p>`)].join("")}function Ee(){return[a("Stellar Absolute Magnitude",`<div class="sci-formula__eq">${t("M_V = 4.81 - 2.5\\log_{10}(L)")}</div>
      ...
      ${p(["Body","P (atm)","CO&#8322; %","H&#8322;O %","CH&#8324; %","&tau;","T<sub>surf</sub> (K)"],[["Earth","1.0","0.04","0.40","0","0.70","288"],["Venus","92","96.5","0","0","126","737"],["Mars","0.006","95.3","0","0","0.029","211"]])}
      ...
      ${p(["\\u03BB range","Status","Meaning"],[["\\u2265 6","Retained","Firmly held over geological time (> 4.5 Gyr)"],["3 &ndash; 6","Marginal","Slow escape; may be lost on Gyr timescales"],["< 3","Lost","Rapid thermal escape (lost within ~100 Myr)"]])}
      ...
      <p><b>Calibration:</b></p>
      ${p(["Body","T<sub>eq</sub> (K)","F/F<sub>0</sub>","P (atm)","&eta;<sub>abs</sub>","T<sub>exo</sub> (K)","Observed"],[["Earth","254","1.0","1.0","0.94","~944","700&ndash;1400"],["Venus","229","1.9","92","1.00","~229","250&ndash;300"],["Mars","210","0.43","0.006","0.09","~233","200&ndash;350"],["Pluto","32","0.0006","10&#8315;&#8309;","&lt;0.001","~32","~65&ndash;70"]])}
      ...
      ${p(["Gas","Factor","Lost","Marginal","Retained"],[["H&#8322;","&times;3.0","&lambda; < 9","9 &le; &lambda; < 18","&lambda; &ge; 18"],["He","&times;5.0","&lambda; < 15","15 &le; &lambda; < 30","&lambda; &ge; 30"],["Others","&times;1.0","&lambda; < 3","3 &le; &lambda; < 6","&lambda; &ge; 6"]])}
      ...
      ${p(["Zone","Base moisture"],[["Hadley equatorial (lat &lt; 0.7&times;cell)","0.9 &minus; 0.75&times;fraction"],["Hadley subsidence edge","0.15"],["Ferrel warm-coast","0.70"],["Ferrel cold-coast","0.45"],["Ferrel general","0.55"],["Polar","0.20"]])}
      ...
      ${p(["Spectral bin","T<sub>eff</sub> (K)","Old","Mid","Young","&alpha;"],[["FGK","&ge; 3900","0.05","0.25","1.0","1.8"],["Early M","3200&ndash;3900","0.5","2.0","8.0","2.0"],["Late M","&lt; 3200","2.0","8.0","30.0","2.2"]])}
      ...
      ${p(["Flare energy (erg)","P(CME)"],[["&lt; 10&sup3;&sup2;","0.005"],["10&sup3;&sup2; &ndash; 10&sup3;&sup3;","0.12"],["10&sup3;&sup3; &ndash; 10&sup3;&sup4;","0.4"],["&gt; 10&sup3;&sup4;","0.75"]])}
      ...
      ${p(["Spectral bin","Old","Mid","Young","&alpha;"],[["FGK (T &ge; 3900 K)","0.05","0.25","1.0","1.8"],["Early M (3200&ndash;3900 K)","0.5","2.0","8.0","2.0"],["Late M (&lt; 3200 K)","2.0","8.0","30.0","2.2"]])}
      ...
      ${p(["Spectral bin","Min (&phi;=0)","Mid (&phi;=0.5)","Max (&phi;=1)"],[["FGK","0.35","1.0","1.65"],["Early M","0.6","1.0","1.4"],["Late M","0.75","1.0","1.25"]])}
      ...
      ${p(["Category","Fraction"],[["Main-sequence stars","72%"],["White dwarfs","6%"],["Brown dwarfs","19%"],["Other (giants, subdwarfs)","3%"]])}
      ...
      ${p(["Class","Binary","Triple","Quadruple"],[["O","0.70","0.12","0.05"],["B","0.50","0.09","0.04"],["A","0.45","0.08","0.03"],["F","0.46","0.08","0.03"],["G","0.46","0.08","0.03"],["K","0.35","0.05","0.02"],["M","0.27","0.03","0.01"],["WD","0.25","0.02","0.005"],["L/T/Y","0.15","0.01","0.003"]])}
      ...
      ${p(["Era","Density (km&sup2;)","Growth (%/yr)"],[["Hunter-Gatherer","0.05","0.5"],["Neolithic","2","0.8"],["Bronze Age","8","1.0"],["Iron Age","15","1.0"],["Medieval","30","1.0"],["Early Industrial","80","1.5"],["Industrial","200","2.0"],["Post-Industrial","400","0.5"],["Sci-Fi High","1000","0.3"]])}`),a("Logistic Growth (Verhulst)",`<div class="sci-formula__eq">${t("P(t) = \\frac{K}{1 + \\frac{K - P_0}{P_0}\\,e^{-rt}}")}</div>
      ...
      ${p(["Isotope","Half-life (Gyr)","Heat fraction <i>w<sub>i</sub></i>"],[["U-238","4.47","0.39"],["U-235","0.70","0.04"],["Th-232","14.05","0.40"],["K-40","1.25","0.17"]])}
      ...
      ${p(["Class","Condition"],[["Dwarf planet","M &lt; 0.01 M&#8853;"],["Planet","M &ge; 0.01 M&#8853;"]])}
      ...
      ${p(["Class","Condition"],[["Ice world","WMF &gt; 0.1"],["Ocean world","WMF &gt; 0.001"],["Iron world","CMF &gt; 0.6"],["Mercury-like","CMF &gt; 0.45"],["Earth-like","CMF &ge; 0.25"],["Mars-like","CMF &ge; 0.1"],["Coreless","CMF &lt; 0.1"]])}
      ...
      ${p(["State","&Delta;IW","Primary gases"],[["Highly reduced","&minus;4","H&#8322; + CO"],["Moderately reduced","&minus;2","H&#8322; + CO&#8322; (mixed)"],["Earth-like","+1","CO&#8322; + H&#8322;O"],["Oxidised","+3","CO&#8322; + H&#8322;O + SO&#8322;"]])}
      <p>The iron-w&uuml;stite (IW) buffer sets the reference. Earth is approximately IW+1.</p>
      ${s("Ortenzi et al. (2020), Sci. Rep. 10, 10907")}`)].join("")}function Be(){return[a("Maximum Mountain Height",`<div class="sci-formula__eq">${t("H_{\\max} = \\frac{\\sigma_y}{\\rho \\cdot g} \\approx \\frac{9{,}267}{g} \\text{ m}")}</div>
      ...
      ${p(["Regime","Rate (mm/yr)","Label"],[["Mobile lid","20&ndash;200","Active spreading"],["Episodic overturn","5&ndash;50","Episodic spreading"],["Plutonic-squishy","2&ndash;20","Sluggish spreading"],["Stagnant lid","0","No spreading"]])}
      ...
      ${p(["Zone","Typical width","Depth range","Slope"],[["Continental shelf","80 km","0&ndash;130 m","~0.1&deg;"],["Continental slope","varies","130&ndash;3,000 m","3&ndash;4&deg;"],["Continental rise","200 km","3,000&ndash;4,500 m","~0.5&deg;"],["Abyssal plain","indefinite","4,500+ m","~0&deg;"]])}
    ...
    </div>`].join("")}function Ke(){return[a("Mean-Motion Resonance Positions",`<div class="sci-formula__eq">${t("a_{\\text{res}} = a_p\\,(p/q)^{2/3}")}</div>
      ...
      ${p(["Resonance","(p/q)<sup>2/3</sup>"],[["3:2 exterior","1.310"],["2:1 exterior","1.587"],["5:2 exterior","1.842"],["1:2 interior","0.630"],["1:4 interior","0.397"],["1:8 interior","0.250"]])}`),a("Condensation Sequence",`<p>Species condense from a cooling solar-composition nebula at characteristic temperatures:</p>
      ${p(["Species","T<sub>cond</sub> (K)","Mass %"],[["Corundum (Al&#8322;O&#8323;)","1700","0.4"],["Iron-nickel","1450","7"],["Enstatite (MgSiO&#8323;)","1350","12"],["Forsterite (Mg&#8322;SiO&#8324;)","1300","14"],["Feldspar","1200","6"],["Troilite (FeS)","700","4"],["Organics","300","6"],["Water ice","170","33"],["NH&#8323; hydrate","130","2"],["CO&#8322; ice","70","5"],["CH&#8324; ice","31","4"],["CO ice","25","3"],["N&#8322; ice","22","2"]])}
    ...
    to WorldSmith; items marked <b>Simplified</b> are reductions of published work.</p>`,i("Oort Cloud Mass And LPC Flux Proxy (WS-derived / Simplified)",`<p>WorldSmith&rsquo;s Oort-cloud mass and long-period-comet flux are not taken from a single published closed-form model. The edge scaling follows exo-Oort Hill-radius work, but the mass and LPC-rate outputs are WorldSmith proxies built from five factors: total giant mass, outer-giant extent, inner-ejector depletion, age saturation, and environmental retention.</p>
      ...
      ${s("Mardling &amp; Aarseth (2001), MNRAS 321, 398; WorldSmith hierarchy guardrails in engine/homeSystem/stability.js")}`)].join("")}function Je(i,r){let l=r/100,d=Math.log(Math.max(i,1e-6)),n=Math.min(1/3,.257-.0161*d),u=(1.07-.21*l)*Math.pow(i,n),f=i*5.51/Math.pow(u,3);return{R:u,rho:f}}function je(i){let r=i.querySelector("#sci-mlr-mass"),l=i.querySelector("#sci-mlr-slider"),d=i.querySelector("#sci-mlr-result");if(r&&l&&d){let _=()=>{let h=X(Number(r.value));d.textContent=`${m(h,4)} L\u2609`};O({numberEl:r,sliderEl:l,min:.075,max:100,step:.01,mode:"log"}),r.addEventListener("input",_),l.addEventListener("input",()=>{r.dispatchEvent(new Event("input"))}),_()}let n=i.querySelector("#sci-hz-mass"),u=i.querySelector("#sci-hz-slider"),f=i.querySelector("#sci-hz-result");if(n&&u&&f){let _=()=>{let h=Number(n.value),S=X(h),C=oe(h),I=ne({luminosityLsol:S,teffK:C});j(f,`${m(I.innerAu,2)} \u2013 ${m(I.outerAu,2)} AU`)};O({numberEl:n,sliderEl:u,min:.075,max:100,step:.01,mode:"log"}),n.addEventListener("input",_),u.addEventListener("input",()=>{n.dispatchEvent(new Event("input"))}),_()}let v=i.querySelector("#sci-dens-mass"),w=i.querySelector("#sci-dens-mass-slider"),M=i.querySelector("#sci-dens-cmf"),P=i.querySelector("#sci-dens-cmf-slider"),R=i.querySelector("#sci-dens-result");if(v&&w&&M&&P&&R){let _=()=>{let{R:h,rho:S}=Je(Number(v.value),Number(M.value));j(R,`${m(h,3)} R\u2295 \u2014 ${m(S,2)} g/cm\xB3`)};O({numberEl:v,sliderEl:w,min:.01,max:100,step:.01,mode:"log"}),O({numberEl:M,sliderEl:P,min:0,max:100,step:1,mode:"linear"}),[v,w,M,P].forEach(h=>h.addEventListener("input",_)),_()}let T=i.querySelector("#sci-hmag-rad"),L=i.querySelector("#sci-hmag-alb"),A=i.querySelector("#sci-hmag-alb-slider"),G=i.querySelector("#sci-hmag-result");if(T&&L&&G){let _=()=>{let h=Number(T.value),S=Number(L.value);if(h>0&&S>0){let C=ce({radiusKm:h,geometricAlbedo:S});G.textContent=`H = ${m(C,2)}`}};A&&O({numberEl:L,sliderEl:A,min:.01,max:1,step:.01,mode:"linear"}),[T,L].forEach(h=>h.addEventListener("input",_)),A&&A.addEventListener("input",_),_()}let H=i.querySelector("#sci-flare-temp"),W=i.querySelector("#sci-flare-temp-slider"),D=i.querySelector("#sci-flare-age"),c=i.querySelector("#sci-flare-age-slider"),g=i.querySelector("#sci-flare-result");if(H&&D&&g){let _=()=>{let h=le({teffK:Number(H.value),ageGyr:Number(D.value)});ge(g,{countLabel:"32",countValue:h.N32,alphaValue:h.alpha})};W&&O({numberEl:H,sliderEl:W,min:2e3,max:1e4,step:100,mode:"linear"}),c&&O({numberEl:D,sliderEl:c,min:.01,max:13,step:.1,mode:"linear"}),[H,W,D,c].filter(Boolean).forEach(h=>h.addEventListener("input",_)),_()}let x=i.querySelector("#sci-leap-len"),$=i.querySelector("#sci-leap-result");if(x&&$){let _=S=>{if(S===0)return{p:0,q:1};let C=0,I=1;for(let F=1;F<=1e4;F++){let q=Math.round(S*F);if(Math.abs(q/F-S)<1e-9){C=q,I=F;break}}return{p:C,q:I}},h=()=>{let S=Number(x.value),C=S-Math.floor(S);if(C<1e-9){U($,[],"No leap cycle needed (integer year)");return}let F=de(C,6).slice(1).map(_).filter(q=>q.q>0);if(F.length===0){U($,[],"No usable cycle found");return}U($,F.map(q=>{let se=Math.abs(q.p/q.q-C),re=se>1e-12?Math.round(1/se):1/0,_e=re===1/0?"exact":`~1 day drift per ${m(re,0)} yr`;return{fraction:`${q.p}/${q.q}`,description:`${q.p} leap day${q.p!==1?"s":""} every ${q.q} years (${_e})`}}))};x.addEventListener("input",h),h()}let k=i.querySelector("#sci-ghz-r"),b=i.querySelector("#sci-ghz-loc"),y=i.querySelector("#sci-ghz-result");if(k&&b&&y){let _=()=>{let h=Number(k.value),S=Number(b.value),C=.53*h,I=.1*h,F=Math.exp(-.5*Math.pow((S-C)/I,2)),q=S>=.47*h&&S<=.6*h;j(y,`${m(F*100,1)}%${q?" (in hard band)":""}`)};[k,b].forEach(h=>h.addEventListener("input",_)),_()}let K=i.querySelector("#sci-tec-grav"),Q=i.querySelector("#sci-tec-grav-slider"),te=i.querySelector("#sci-tec-mtn"),ae=i.querySelector("#sci-tec-shield"),ie=i.querySelector("#sci-tec-root");if(K&&Q&&te&&ae&&ie){let _=()=>{let h=Number(K.value);h>0&&(te.textContent=`${m(pe(h),0)} m`,ae.textContent=`${m(me(h),0)} m`,ie.textContent=`${m(ue(5e3),0)} m`)};O({numberEl:K,sliderEl:Q,min:.05,max:5,step:.01,mode:"log"}),K.addEventListener("input",_),Q.addEventListener("input",()=>{K.dispatchEvent(new Event("input"))}),_()}}var ye=[{id:"stellar",title:"Stellar Physics",count:9,builder:ke},{id:"evolution",title:"Stellar Evolution",count:7,builder:Pe},{id:"planetary",title:"Planetary Physics",count:12,builder:Re},{id:"gasgiant",title:"Gas Giant Physics",count:13,builder:Ae},{id:"interior",title:"Interior &amp; Composition",count:7,builder:Ne},{id:"tectonics",title:"Tectonics &amp; Geodynamics",count:14,builder:Be},{id:"orbital",title:"Orbital Mechanics",count:22,builder:He},{id:"lagrange",title:"Lagrange Points",count:4,builder:Fe},{id:"photometry",title:"Photometry &amp; Magnitudes",count:8,builder:Ee},{id:"atmosphere",title:"Atmosphere &amp; Colour",count:9,builder:Le},{id:"climate",title:"Climate Classification",count:8,builder:Ge},{id:"activity",title:"Stellar Activity",count:7,builder:We},{id:"calendar",title:"Calendar Systems",count:5,builder:Oe},{id:"cluster",title:"Local Cluster",count:7,builder:Ie},{id:"population",title:"Population Dynamics",count:5,builder:De},{id:"system",title:"System Architecture",count:7,builder:ze},{id:"debris",title:"Debris Disks",count:12,builder:Ke},{id:"divergences",title:"Divergences from Published Science",count:36,builder:Ve}];function lt(i){let r=document.createElement("div");r.className="page";let l=ye.map(c=>`<a class="sci-toc__link" data-target="sci-${c.id}" href="#sci-${c.id}">${c.title}</a>`).join(""),d=ye.map((c,g)=>`<details class="sci-section" id="sci-${c.id}"${g===0?" open":""}>
  ...
  `,i.innerHTML="",i.appendChild(r);let n=Array.from(r.querySelectorAll(".sci-section")),u=Array.from(r.querySelectorAll(".sci-formula")),f=Array.from(r.querySelectorAll(".sci-toc__link")),v=r.querySelector("#sciencePageSearch"),w=r.querySelector("#sciencePageSearchStatus"),M=r.querySelector("#sciencePageSearchResults"),P=Ce(r),R=null,T=!1,L=n.find(c=>c.open)?.id||n[0]?.id||null,A=(c,{exclusive:g=!T}={})=>{c&&(g?n.forEach(x=>{x.open=x===c}):c.open=!0,!T&&c.id&&(L=c.id))},G=()=>{R&&(clearTimeout(R),R=null),r.querySelectorAll(".sci-formula.is-search-hit").forEach(c=>{c.classList.remove("is-search-hit")})},H=c=>{if(!c)return;let g=r.querySelector(`#${c}`);if(!g)return;let x=g.closest(".sci-section");A(x),G(),g.classList.add("is-search-hit"),ee(g,{block:"center"}),g.focus?.({preventScroll:!0}),R=window.setTimeout(()=>{g.classList.remove("is-search-hit"),R=null},2200)},W=c=>{let g=N(c);if(G(),!g){T=!1,r.classList.remove("is-search-filtering"),u.forEach(y=>{y.hidden=!1,y.classList.remove("is-search-match","is-search-filtered-out")}),n.forEach(y=>{y.hidden=!1,y.classList.remove("is-search-match","is-search-filtered-out")}),f.forEach(y=>{y.hidden=!1,y.classList.remove("is-search-filtered-out")});let b=n.find(y=>y.id===L)||n[0]||null;return A(b,{exclusive:!0}),[]}let x=be(P,g,P.length),$=new Set(x.map(b=>b.id)),k=new Set(x.map(b=>b.sectionId));return T=!0,r.classList.add("is-search-filtering"),u.forEach(b=>{let y=$.has(b.id);b.hidden=!y,b.classList.toggle("is-search-match",y),b.classList.toggle("is-search-filtered-out",!y)}),n.forEach(b=>{let y=k.has(b.id);b.hidden=!y,b.open=y,b.classList.toggle("is-search-match",y),b.classList.toggle("is-search-filtered-out",!y)}),f.forEach(b=>{let y=k.has(b.dataset.target||"");b.hidden=!y,b.classList.toggle("is-search-filtered-out",!y)}),x},D=(c,g=null)=>{if(!M||!w)return[];let x=N(c);if(!x)return M.hidden=!0,M.innerHTML="",w.hidden=!0,w.textContent="",[];let $=g||be(P,x,P.length),k=$.slice(0,8);return w.hidden=!1,w.textContent=$.length>0?`${$.length} match${$.length===1?"":"es"}${$.length>k.length?`, showing top ${k.length}`:""}`:"No matches found",$.length===0?(M.hidden=!1,M.innerHTML='<div class="sci-search__empty">No formulas or topics matched that search.</div>',[]):(M.hidden=!1,M.innerHTML=k.map(b=>`
          ...
          </button>`).join(""),k)};n.forEach(c=>{c.addEventListener("toggle",()=>{if(c.open){if(!T&&c.id&&(L=c.id),T)return;n.forEach(g=>{g!==c&&(g.open=!1)})}})}),f.forEach(c=>{c.addEventListener("click",g=>{g.preventDefault();let x=c.dataset.target,$=r.querySelector(`#${x}`);$&&(A($),ee($,{block:"start"}))})}),v?.addEventListener("input",()=>{let c=W(v.value);D(v.value,c)}),v?.addEventListener("keydown",c=>{if(c.key!=="Enter")return;let g=W(v.value),[x]=D(v.value,g);x&&(c.preventDefault(),H(x.id))}),M?.addEventListener("click",c=>{let g=c.target.closest(".sci-search__result");g&&H(g.getAttribute("data-target-id"))}),je(r),he().then(c=>fe(r,c))}export{lt as initSciencePage};
