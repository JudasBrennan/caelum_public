import{a as he,b as fe,c as ee}from"./chunk-XCN227BY.js";import{b as pe,d as ue,e as me}from"./chunk-KD5CBWLO.js";import{a as de}from"./chunk-RNZKX373.js";import{d as ce}from"./chunk-FMNLRRDG.js";import{a as O}from"./chunk-WYZYYRUA.js";import{d as le}from"./chunk-3MZTWI7A.js";import{b as J,c as Y}from"./chunk-XMLMEZIZ.js";import{a as H,b as z}from"./chunk-WNGVR2CK.js";import{K as X,N as oe,O as ne}from"./chunk-PEDZU4MZ.js";import{j as m}from"./chunk-T2JC7ILW.js";import"./chunk-24KHYB4L.js";import"./chunk-VC46IEJQ.js";function j(i,r){return i&&(i.textContent=r==null?"":String(r),i)}function ge(i,{countLabel:r="32",countValue:l="",alphaValue:d=""}={}){return i&&(Y(i,["N",J("sub",{text:r}),` = ${l} \u2014 \u03B1 = ${d}`]),i)}function U(i,r=[],l=""){if(!i)return i;let d=(r||[]).filter(Boolean);if(!d.length)return i.textContent=l==null?"":String(l),i;let n=[];return d.forEach((p,f)=>{f>0&&n.push(J("br")),n.push(J("strong",{text:p?.fraction||""}),` \u2014 ${p?.description||""}`)}),Y(i,n),i}function t(i){return`<span class="sci-math sci-math--block">${i}</span>`}function e(i){return`<span class="sci-math">${i}</span>`}function o(i){return`<table class="sci-vars"><tbody>${i.map(([r,l])=>`<tr><td>${e(r)}</td><td>${l}</td></tr>`).join("")}</tbody></table>`}function s(i){return`<p class="sci-cite">${i}</p>`}function a(i,r){return`<div class="sci-formula"><h3 class="sci-formula__name">${i}</h3>${r}</div>`}function u(i,r){return`<table class="sci-data"><thead><tr>${i.map(l=>`<th>${l}</th>`).join("")}</tr></thead><tbody>${r.map(l=>`<tr>${l.map(d=>`<td>${d}</td>`).join("")}</tr>`).join("")}</tbody></table>`}var Z=Object.freeze({tempMinK:240,tempMaxK:715,pressureMinGPa:.05,pressureMaxGPa:10,left:104,right:664,top:44,bottom:300});function ve(i,r,l){return Math.min(l,Math.max(r,i))}function K(i){let{tempMinK:r,tempMaxK:l,left:d,right:n}=Z,p=(Number(i)-r)/(l-r);return d+ve(p,0,1)*(n-d)}function V(i){let{pressureMinGPa:r,pressureMaxGPa:l,top:d,bottom:n}=Z,p=Math.log10(r),f=Math.log10(l),v=(Math.log10(Math.max(Number(i),r))-p)/(f-p);return n-ve(v,0,1)*(n-d)}function xe(i,r){return`${m(i,1)},${m(r,1)}`}function $e(i,r,l=3){let d=[];for(let p=i;p<=r;p+=l){let f=z(p);!f.validRange||!Number.isFinite(f.pressurePa)||d.push({tempK:p,pressureGPa:f.pressurePa/1e9,x:K(p),y:V(f.pressurePa/1e9)})}let n=z(r);return n.validRange&&Number.isFinite(n.pressurePa)&&d.push({tempK:r,pressureGPa:n.pressurePa/1e9,x:K(r),y:V(n.pressurePa/1e9)}),d}function Se(i){return i.length?`M ${i.map(r=>xe(r.x,r.y)).join(" L ")}`:""}function Me(){let i=[250,300,350,400,500,600,700],r=[.1,.3,.6,1,2.2,5,10],{left:l,right:d,top:n,bottom:p}=Z;return[...i.map(f=>{let v=K(f);return`<g class="sci-phase-gridline"><line x1="${m(v,1)}" y1="${n}" x2="${m(v,1)}" y2="${p}" /><text x="${m(v,1)}" y="${p+24}">${f} K</text></g>`}),...r.map(f=>{let v=V(f);return`<g class="sci-phase-gridline"><line x1="${l}" y1="${m(v,1)}" x2="${d}" y2="${m(v,1)}" /><text class="sci-phase-y-tick" x="${l-14}" y="${m(v+4,1)}">${f>=1?m(f,1):m(f,1)} GPa</text></g>`})].join("")}function we(){return[{tempK:288,pressureGPa:.52,label:"0.52 GPa / 288 K",note:"below liquidus",className:"liquid",dx:10,dy:-12},{tempK:288,pressureGPa:1.2,label:"1.2 GPa / 288 K",note:"ice VI stable",className:"ice",dx:10,dy:-12},{tempK:500,pressureGPa:2.3,label:"2.3 GPa / 500 K",note:"hot deep liquid",className:"warm",dx:10,dy:18}].map(r=>{let l=K(r.tempK),d=V(r.pressureGPa);return`<g class="sci-phase-marker sci-phase-marker--${r.className}" transform="translate(${m(l,1)} ${m(d,1)})">
        <circle r="5" />
        <text x="${r.dx}" y="${r.dy}">${r.label}</text>
        <text x="${r.dx}" y="${r.dy+14}" class="sci-phase-marker__note">${r.note}</text>
      </g>`}).join("")}function qe(){let{left:i,right:r,top:l,bottom:d}=Z,n=r-i,p=d-l,f=i+n/2,v=l+p/2,w=$e(H.iceIhIceIiiLiquid.tempK,715),M=Se(w),P=w[0],R=w[w.length-1],T=P&&R?`${M} L ${r},${l} L ${i},${l} Z`:"",A=[{label:"III",tempK:H.iceIhIceIiiLiquid.tempK,pressureGPa:H.iceIhIceIiiLiquid.pressurePa/1e9},{label:"V",tempK:H.iceIiiIceVLiquid.tempK,pressureGPa:H.iceIiiIceVLiquid.pressurePa/1e9},{label:"VI",tempK:H.iceVIceViLiquid.tempK,pressureGPa:H.iceVIceViLiquid.pressurePa/1e9},{label:"VII",tempK:H.iceViIceViiLiquid.tempK,pressureGPa:H.iceViIceViiLiquid.pressurePa/1e9}].map(G=>{let F=K(G.tempK),W=V(G.pressureGPa);return`<g class="sci-phase-triple">
        <circle cx="${m(F,1)}" cy="${m(W,1)}" r="4" />
        <text x="${m(F+7,1)}" y="${m(W-7,1)}">ice ${G.label}</text>
    ...
    <div class="sci-phase-legend" aria-label="Diagram legend">
      ...
      ${u(["Class","T range (K)","Span"],[["O","33,000 &ndash; 95,000","62,000"],["B","10,000 &ndash; 33,000","23,000"],["A","7,500 &ndash; 10,000","2,500"],["F","6,000 &ndash; 7,500","1,500"],["G","5,200 &ndash; 6,000","800"],["K","3,700 &ndash; 5,200","1,500"],["M","2,000 &ndash; 3,700","1,700"]])}`),a("Stellar Environment Diagnostics",`<div class="sci-formula__eq">${t("Ro = P_{\\text{rot}} / \\tau_c")}</div>
      ...
      ${u(["Boundary used","Temperature range","Anchor in model"],[["ice III / liquid","251.165&ndash;256.164 K","0.209&ndash;0.350 GPa"],["ice V / liquid","256.164&ndash;273.31 K","0.350&ndash;0.632 GPa"],["ice VI / liquid","273.31&ndash;355 K","0.632&ndash;2.216 GPa"],["ice VII / liquid","355&ndash;715 K",`${m(z(550).pressurePa/1e9,2)} GPa at 550 K`]])}
      ...
      ${s("Peale, Cassen &amp; Reynolds (1979); Wahl et al. (2016); Lainey et al. (2009, 2017); Fuller, Luan &amp; Quataert (2016)")}`)].join("")}function Fe(){return[a("Kepler&rsquo;s Third Law (Moon Orbit)",`<div class="sci-formula__eq">${t("P = \\frac{2\\pi\\sqrt{a^3 / G(M_p + M_m)}}{86400} \\text{ days}")}</div>
      ...
      <p><b>Reference inputs:</b> this moon-world layer sits on top of the tidal heating, moon temperature, magnetospheric radiation, volatile-retention, magnetosphere, and atmosphere-stability blocks documented above, then adds WorldSmith&rsquo;s own hydrosphere, geology, biosphere, and integrated habitability policy layers.</p>`)].join("")}function Ee(){return[a("Hill Sphere",`<div class="sci-formula__eq">${t("r_H = a \\left(\\frac{m}{3M_\\star}\\right)^{1/3}")}</div>
  ...
  `,i.innerHTML="",i.appendChild(r);let n=Array.from(r.querySelectorAll(".sci-section")),p=Array.from(r.querySelectorAll(".sci-formula")),f=Array.from(r.querySelectorAll(".sci-toc__link")),v=r.querySelector("#sciencePageSearch"),w=r.querySelector("#sciencePageSearchStatus"),M=r.querySelector("#sciencePageSearchResults"),P=Ce(r),R=null,T=!1,L=n.find(c=>c.open)?.id||n[0]?.id||null,A=(c,{exclusive:g=!T}={})=>{c&&(g?n.forEach(x=>{x.open=x===c}):c.open=!0,!T&&c.id&&(L=c.id))},G=()=>{R&&(clearTimeout(R),R=null),r.querySelectorAll(".sci-formula.is-search-hit").forEach(c=>{c.classList.remove("is-search-hit")})},F=c=>{if(!c)return;let g=r.querySelector(`#${c}`);if(!g)return;let x=g.closest(".sci-section");A(x),G(),g.classList.add("is-search-hit"),ee(g,{block:"center"}),g.focus?.({preventScroll:!0}),R=window.setTimeout(()=>{g.classList.remove("is-search-hit"),R=null},2200)},W=c=>{let g=N(c);if(G(),!g){T=!1,r.classList.remove("is-search-filtering"),p.forEach(y=>{y.hidden=!1,y.classList.remove("is-search-match","is-search-filtered-out")}),n.forEach(y=>{y.hidden=!1,y.classList.remove("is-search-match","is-search-filtered-out")}),f.forEach(y=>{y.hidden=!1,y.classList.remove("is-search-filtered-out")});let b=n.find(y=>y.id===L)||n[0]||null;return A(b,{exclusive:!0}),[]}let x=be(P,g,P.length),$=new Set(x.map(b=>b.id)),k=new Set(x.map(b=>b.sectionId));return T=!0,r.classList.add("is-search-filtering"),p.forEach(b=>{let y=$.has(b.id);b.hidden=!y,b.classList.toggle("is-search-match",y),b.classList.toggle("is-search-filtered-out",!y)}),n.forEach(b=>{let y=k.has(b.id);b.hidden=!y,b.open=y,b.classList.toggle("is-search-match",y),b.classList.toggle("is-search-filtered-out",!y)}),f.forEach(b=>{let y=k.has(b.dataset.target||"");b.hidden=!y,b.classList.toggle("is-search-filtered-out",!y)}),x},D=(c,g=null)=>{if(!M||!w)return[];let x=N(c);if(!x)return M.hidden=!0,M.innerHTML="",w.hidden=!0,w.textContent="",[];let $=g||be(P,x,P.length),k=$.slice(0,8);return w.hidden=!1,w.textContent=$.length>0?`${$.length} match${$.length===1?"":"es"}${$.length>k.length?`, showing top ${k.length}`:""}`:"No matches found",$.length===0?(M.hidden=!1,M.innerHTML='<div class="sci-search__empty">No formulas or topics matched that search.</div>',[]):(M.hidden=!1,M.innerHTML=k.map(b=>`
          ...
          </button>`).join(""),k)};n.forEach(c=>{c.addEventListener("toggle",()=>{if(c.open){if(!T&&c.id&&(L=c.id),T)return;n.forEach(g=>{g!==c&&(g.open=!1)})}})}),f.forEach(c=>{c.addEventListener("click",g=>{g.preventDefault();let x=c.dataset.target,$=r.querySelector(`#${x}`);$&&(A($),ee($,{block:"start"}))})}),v?.addEventListener("input",()=>{let c=W(v.value);D(v.value,c)}),v?.addEventListener("keydown",c=>{if(c.key!=="Enter")return;let g=W(v.value),[x]=D(v.value,g);x&&(c.preventDefault(),F(x.id))}),M?.addEventListener("click",c=>{let g=c.target.closest(".sci-search__result");g&&F(g.getAttribute("data-target-id"))}),je(r),he().then(c=>fe(r,c))}export{lt as initSciencePage};
