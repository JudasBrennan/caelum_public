import{a as he,b as fe,c as ee}from"./chunk-XCN227BY.js";import{b as pe,d as ue,e as me}from"./chunk-KD5CBWLO.js";import{a as de}from"./chunk-RNZKX373.js";import{d as ce}from"./chunk-ZFAIBQGO.js";import{a as O}from"./chunk-WYZYYRUA.js";import{d as le}from"./chunk-MTSXNHS6.js";import{b as z,c as Y}from"./chunk-XMLMEZIZ.js";import{s as H,t as V}from"./chunk-47ATKL5F.js";import{p as X,s as oe,t as ne}from"./chunk-XFZMQA63.js";import{j as m}from"./chunk-T2JC7ILW.js";import"./chunk-24KHYB4L.js";import"./chunk-VC46IEJQ.js";function j(i,s){return i&&(i.textContent=s==null?"":String(s),i)}function be(i,{countLabel:s="32",countValue:l="",alphaValue:d=""}={}){return i&&(Y(i,["N",z("sub",{text:s}),` = ${l} \u2014 \u03B1 = ${d}`]),i)}function U(i,s=[],l=""){if(!i)return i;let d=(s||[]).filter(Boolean);if(!d.length)return i.textContent=l==null?"":String(l),i;let n=[];return d.forEach((p,f)=>{f>0&&n.push(z("br")),n.push(z("strong",{text:p?.fraction||""}),` \u2014 ${p?.description||""}`)}),Y(i,n),i}function t(i){return`<span class="sci-math sci-math--block">${i}</span>`}function e(i){return`<span class="sci-math">${i}</span>`}function o(i){return`<table class="sci-vars"><tbody>${i.map(([s,l])=>`<tr><td>${e(s)}</td><td>${l}</td></tr>`).join("")}</tbody></table>`}function r(i){return`<p class="sci-cite">${i}</p>`}function a(i,s){return`<div class="sci-formula"><h3 class="sci-formula__name">${i}</h3>${s}</div>`}function u(i,s){return`<table class="sci-data"><thead><tr>${i.map(l=>`<th>${l}</th>`).join("")}</tr></thead><tbody>${s.map(l=>`<tr>${l.map(d=>`<td>${d}</td>`).join("")}</tr>`).join("")}</tbody></table>`}var Z=Object.freeze({tempMinK:240,tempMaxK:715,pressureMinGPa:.05,pressureMaxGPa:10,left:104,right:664,top:44,bottom:300});function ve(i,s,l){return Math.min(l,Math.max(s,i))}function K(i){let{tempMinK:s,tempMaxK:l,left:d,right:n}=Z,p=(Number(i)-s)/(l-s);return d+ve(p,0,1)*(n-d)}function J(i){let{pressureMinGPa:s,pressureMaxGPa:l,top:d,bottom:n}=Z,p=Math.log10(s),f=Math.log10(l),v=(Math.log10(Math.max(Number(i),s))-p)/(f-p);return n-ve(v,0,1)*(n-d)}function xe(i,s){return`${m(i,1)},${m(s,1)}`}function $e(i,s,l=3){let d=[];for(let p=i;p<=s;p+=l){let f=V(p);!f.validRange||!Number.isFinite(f.pressurePa)||d.push({tempK:p,pressureGPa:f.pressurePa/1e9,x:K(p),y:J(f.pressurePa/1e9)})}let n=V(s);return n.validRange&&Number.isFinite(n.pressurePa)&&d.push({tempK:s,pressureGPa:n.pressurePa/1e9,x:K(s),y:J(n.pressurePa/1e9)}),d}function Me(i){return i.length?`M ${i.map(s=>xe(s.x,s.y)).join(" L ")}`:""}function Se(){let i=[250,300,350,400,500,600,700],s=[.1,.3,.6,1,2.2,5,10],{left:l,right:d,top:n,bottom:p}=Z;return[...i.map(f=>{let v=K(f);return`<g class="sci-phase-gridline"><line x1="${m(v,1)}" y1="${n}" x2="${m(v,1)}" y2="${p}" /><text x="${m(v,1)}" y="${p+24}">${f} K</text></g>`}),...s.map(f=>{let v=J(f);return`<g class="sci-phase-gridline"><line x1="${l}" y1="${m(v,1)}" x2="${d}" y2="${m(v,1)}" /><text class="sci-phase-y-tick" x="${l-14}" y="${m(v+4,1)}">${f>=1?m(f,1):m(f,1)} GPa</text></g>`})].join("")}function we(){return[{tempK:288,pressureGPa:.52,label:"0.52 GPa / 288 K",note:"below liquidus",className:"liquid",dx:10,dy:-12},{tempK:288,pressureGPa:1.2,label:"1.2 GPa / 288 K",note:"ice VI stable",className:"ice",dx:10,dy:-12},{tempK:500,pressureGPa:2.3,label:"2.3 GPa / 500 K",note:"hot deep liquid",className:"warm",dx:10,dy:18}].map(s=>{let l=K(s.tempK),d=J(s.pressureGPa);return`<g class="sci-phase-marker sci-phase-marker--${s.className}" transform="translate(${m(l,1)} ${m(d,1)})">
        <circle r="5" />
        <text x="${s.dx}" y="${s.dy}">${s.label}</text>
        <text x="${s.dx}" y="${s.dy+14}" class="sci-phase-marker__note">${s.note}</text>
      </g>`}).join("")}function qe(){let{left:i,right:s,top:l,bottom:d}=Z,n=s-i,p=d-l,f=i+n/2,v=l+p/2,w=$e(H.iceIhIceIiiLiquid.tempK,715),S=Me(w),k=w[0],R=w[w.length-1],T=k&&R?`${S} L ${s},${l} L ${i},${l} Z`:"",A=[{label:"III",tempK:H.iceIhIceIiiLiquid.tempK,pressureGPa:H.iceIhIceIiiLiquid.pressurePa/1e9},{label:"V",tempK:H.iceIiiIceVLiquid.tempK,pressureGPa:H.iceIiiIceVLiquid.pressurePa/1e9},{label:"VI",tempK:H.iceVIceViLiquid.tempK,pressureGPa:H.iceVIceViLiquid.pressurePa/1e9},{label:"VII",tempK:H.iceViIceViiLiquid.tempK,pressureGPa:H.iceViIceViiLiquid.pressurePa/1e9}].map(G=>{let E=K(G.tempK),W=J(G.pressureGPa);return`<g class="sci-phase-triple">
        <circle cx="${m(E,1)}" cy="${m(W,1)}" r="4" />
        <text x="${m(E+7,1)}" y="${m(W-7,1)}">ice ${G.label}</text>
    ...
    <svg class="sci-phase-diagram" viewBox="0 0 700 360" role="img" aria-label="WorldSmith ocean-floor water phase diagram">
      <rect x="${i}" y="${l}" width="${n}" height="${p}" class="sci-phase-liquid" />
      <path d="${T}" class="sci-phase-ice" />
      <rect x="${i}" y="${l}" width="${m(K(251.165)-i,1)}" height="${p}" class="sci-phase-unsupported" />
      ${Se()}
      <path d="${S}" class="sci-phase-liquidus" />
      ${A}
      ${we()}
      <text x="${m(f,1)}" y="${l-16}" class="sci-phase-axis sci-phase-axis--top">Estimated bottom-ocean temperature</text>
      <text x="28" y="${m(v,1)}" class="sci-phase-axis sci-phase-axis--left" transform="rotate(-90 28 ${m(v,1)})">Seafloor pressure</text>
      <text x="${i+28}" y="${l+38}" class="sci-phase-region sci-phase-region--ice">Dense ice stable above boundary</text>
      <text x="${s-220}" y="${d-30}" class="sci-phase-region sci-phase-region--liquid">Liquid remains stable below boundary</text>
    </svg>
    <div class="sci-phase-legend" aria-label="Diagram legend">
  ...
  </div>`}function N(i){return String(i??"").trim().toLowerCase()}function Te(i,s){if(!s)return 0;let l=N(i.title);if(l===s)return 120;if(l.startsWith(s))return 90;if(l.includes(s))return 70;let d=N(i.sectionTitle);return d===s?52:d.includes(s)?40:N(i.text).includes(s)?18:0}function Ce(i){return Array.from(i.querySelectorAll(".sci-section")).flatMap(s=>{let l=String(s.id||"").trim(),d=s.querySelector(".sci-section__title")?.textContent?.trim()||"Science Section";return Array.from(s.querySelectorAll(".sci-formula")).map((n,p)=>{let f=`${l}-formula-${p+1}`;return n.id=f,n.setAttribute("tabindex","-1"),{id:f,sectionId:l,sectionTitle:d,title:n.querySelector(".sci-formula__name")?.textContent?.trim()||"Formula",text:n.textContent||""}})})}function ge(i,s,l=8){let d=N(s);return d?i.map(n=>({entry:n,score:Te(n,d)})).filter(n=>n.score>0).sort((n,p)=>p.score-n.score||n.entry.sectionTitle.localeCompare(p.entry.sectionTitle)||n.entry.title.localeCompare(p.entry.title)).slice(0,l).map(n=>n.entry):[]}function Pe(){return[a("Mass-Luminosity Relation",`<div class="sci-formula__eq">${t("L = c \\cdot M^{\\alpha}")}</div>
      <p>Six-piece empirical power law relating stellar mass to luminosity, fitted to 509 eclipsing binary components.</p>
      ${u(["Mass range (M&#9737;)","\\(\\alpha\\)","c"],[["M &lt; 0.45","2.028","0.0892"],["0.45 &le; M &lt; 0.72","4.572","0.68"],["0.72 &le; M &lt; 1.05","5.743","1.0"],["1.05 &le; M &lt; 2.4","4.329","1.072"],["2.4 &le; M &lt; 7.0","3.967","1.471"],["M &ge; 7.0","2.865","12.55"]])}
      ${r("Eker et al. (2018) MNRAS 479, 5491 &mdash; Table 4")}
      <div class="sci-try">
      ...
      ${r("Schweitzer et al. (2019, A&amp;A 625, A68); Eker et al. (2018, MNRAS 479, 5491)")}`),a("Mass-Temperature Relation (Eker MTR)",`<div class="sci-formula__eq">${t("\\log T_{\\text{eff}} = -0.170\\,(\\log M)^2 + 0.888\\,\\log M + 3.671")}</div>
      ...
      ${r("Eker et al. (2018, MNRAS 479, 5491)")}`),a("Effective Temperature (Stefan-Boltzmann)",`<div class="sci-formula__eq">${t("T_{\\text{eff}} = \\left(\\frac{L}{R^2}\\right)^{1/4} \\times 5776 \\text{ K}")}</div>
      ...
      ${r("Kopparapu et al. (2013/2014) style; Chromant Desmos correction")}
      <div class="sci-try">
        <div class="sci-try__title">Try it</div>
        <div class="sci-try__row">
          <label>Mass <span class="unit">M&#9737;</span></label>
      ...
      ${r("Fischer &amp; Valenti (2005) ApJ 622, 1102; Johnson et al. (2010) PASP 122, 905; Petigura et al. (2018) AJ 155, 89")}`),a("Star Colour (Blackbody RGB)",`<p>Tanner Helland&rsquo;s piecewise approximation converts effective temperature to sRGB. Valid 1000&ndash;40,000 K (R&sup2; &gt; 0.987). Let ${e("t = T/100")}:</p>
      ...
      ${r("Tanner Helland (2012) &mdash; tannerhelland.com")}`),a("Spectral Classification",`<p>Temperature mapped to spectral class via boundary lookup, then linear interpolation within the class gives subtype 0&ndash;9.9:</p>
      ${u(["Class","T range (K)","Span"],[["O","33,000 &ndash; 95,000","62,000"],["B","10,000 &ndash; 33,000","23,000"],["A","7,500 &ndash; 10,000","2,500"],["F","6,000 &ndash; 7,500","1,500"],["G","5,200 &ndash; 6,000","800"],["K","3,700 &ndash; 5,200","1,500"],["M","2,000 &ndash; 3,700","1,700"]])}`)].join("")}function ke(){return[a("Metallicity Conversion",`<div class="sci-formula__eq">${t("Z = Z_\\odot \\cdot 10^{[\\text{Fe/H}]}")}</div>
      ...
      ${r("Tout, Pols, Eggleton &amp; Han (1996), MNRAS 281, 257")}`),a("ZAMS Radius",`<div class="sci-formula__eq">${t("R_{\\text{ZAMS}} = \\frac{a_0 M^{2.5} + a_1 M^{6.5} + a_2 M^{11} + a_3 M^{19} + a_4 M^{19.5}}{a_5 + a_6 M^2 + a_7 M^{8.5} + M^{18.5} + a_8 M^{19.5}}")}</div>
      <p>Matching rational polynomial for ZAMS radius, with metallicity-dependent coefficients.</p>
      ${r("Tout et al. (1996), MNRAS 281, 257")}`),a("Main-Sequence Lifetime",`<div class="sci-formula__eq">${t("t_{\\text{BGB}} = \\frac{a_0 + a_1 M^4 + a_2 M^{5.5} + M^7}{a_3 M^2 + a_4 M^7} \\;\\text{Myr}")}</div>
      ...
      ${r("Hurley, Pols &amp; Tout (2000), MNRAS 315, 543 &mdash; eq. 4")}`),a("Terminal MS Luminosity",`<div class="sci-formula__eq">${t("L_{\\text{TMS}} = \\frac{a_{11} M^3 + a_{12} M^4 + a_{13} M^{a_{16}+1.8}}{a_{14} + a_{15} M^5 + M^{a_{16}}}")}</div>
      <p>Luminosity at the end of the main sequence (terminal-age). Anchors the evolved luminosity track.</p>
      ${r("Hurley et al. (2000) &mdash; eq. 8")}`),a("Terminal MS Radius",`<p>Piecewise fit: low-mass stars (${e("M \\le a_{17}")}) and high-mass stars are computed
      with different rational polynomials, smoothly interpolated across the boundary.</p>
      ${r("Hurley et al. (2000) &mdash; eq. 9")}`),a("Evolved Luminosity &amp; Radius",`<div class="sci-formula__eq">${t("\\log(L/L_{\\text{ZAMS}}) = \\alpha_L \\tau + \\beta_L \\tau^\\eta + \\gamma \\tau^2")}</div>
      ...
      ${r("Hurley, Pols &amp; Tout (2000), MNRAS 315, 543")}`)].join("")}function Re(){return[a("Mass&ndash;Radius Relation (Radius-First)",`<div class="sci-formula__eq">${t("R_\\oplus = (1.07 - 0.21 \\cdot \\text{CMF}) \\cdot M_\\oplus^{\\,\\alpha}")}</div>
      ...
      ${o([["A","Bond albedo"],["\\sigma","Stefan-Boltzmann constant (5.670 &times; 10&supmin;&sup5; erg cm&supmin;&sup2; s&supmin;&sup1; K&supmin;&sup4;)"],["\\tau","Grey IR optical depth = G<sub>h</sub> &times; 0.5841"],["G_h","Greenhouse effect (dimensionless; 1.0 &asymp; Earth)"],["\\text{surfDiv}","1 &minus; (1 &minus; 0.9) &times; min(&tau;, 1) &mdash; ramps from 1.0 (airless) to 0.9 (&tau; &ge; 1)"]])}
      ...
      <p>Pressure-dependent boiling point of water. ${e("L_v/R_g = 40700/8.314 = 4894.4")} K. Calibrated: 1 atm &rarr; 373 K, 218 atm &rarr; 647 K (critical point).</p>`),a("Ocean-Floor Water Phase Boundary",`${qe()}
      ...
      <p>WorldSmith uses this reduced phase diagram for ocean-world and moon-ocean dense-ice warnings. The model compares local seafloor pressure against the pure-water liquidus pressure at the estimated bottom-ocean temperature ${e("T_b")}.</p>
      ...
      ${u(["Boundary used","Temperature range","Anchor in model"],[["ice III / liquid","251.165&ndash;256.164 K","0.209&ndash;0.350 GPa"],["ice V / liquid","256.164&ndash;273.31 K","0.350&ndash;0.632 GPa"],["ice VI / liquid","273.31&ndash;355 K","0.632&ndash;2.216 GPa"],["ice VII / liquid","355&ndash;715 K",`${m(V(550).pressurePa/1e9,2)} GPa at 550 K`]])}
      ...
      ${r("IAPWS R14-08(2011), Revised Release on the Pressure along the Melting and Sublimation Curves of Ordinary Water Substance; Wagner, Riethmann, Feistel &amp; Harvey (2011), JPCRD 40, 043103.")}`),a("Atmospheric Density (Ideal Gas Law)",`<div class="sci-formula__eq">${t("\\rho = \\frac{p \\cdot M_w}{R \\cdot T}")}</div>
      ...
      ${u(["Gas","Formula","m<sub>i</sub> (kg/mol)"],[["N&#8322;","remainder (100 &minus; &sum; others)","0.028"],["O&#8322;","user input","0.032"],["CO&#8322;","user input","0.044"],["Ar","user input","0.040"],["H&#8322;O","user input","0.018"],["CH&#8324;","user input","0.016"],["H&#8322;","user input (Full mode)","0.002"],["He","user input (Full mode)","0.004"],["SO&#8322;","user input (Full mode)","0.064"],["NH&#8323;","user input (Full mode)","0.017"]])}
      ...
      ${u(["Rotation period","Cells"],[["&ge; 48 h","1 (single Hadley)"],["6 &ndash; 48 h","3 (Earth-like)"],["3 &ndash; 6 h","7 (rapid rotator)"],["&lt; 3 h","5 (very rapid)"]])}`)].join("")}function Ae(){return[a("Mass&ndash;Radius Relation",`<p>Two-regime power law calibrated to Solar System giants:</p>
      ...
      ${r("Chen &amp; Kipping (2017), ApJ 834, 17")}`),a("Sudarsky Classification",`<p>Temperature-based atmospheric classification with bond albedos:</p>
      ${u(["Class","T<sub>eq</sub> (K)","Cloud deck","Albedo"],[["I","&le; 150","NH&#8323; ice","0.34"],["II","150&ndash;250","H&#8322;O","0.81"],["III","250&ndash;800","Cloudless","0.12"],["IV","800&ndash;1400","Alkali metals","0.10"],["V","&gt; 1400","Silicate/iron","0.55"]])}
      <p>Ice giants (${e("M < 0.15\\,M_J")}) at ${e("T_{\\text{eq}} < 100")} K override to methane haze (albedo 0.3).</p>
      ${r("Sudarsky, Burrows &amp; Pinto (2000), ApJ 538, 885")}`),a("Cloud Condensation Layers",`<p>Species condense out of the atmosphere at characteristic temperatures:</p>
      ${u(["Species","T<sub>cond</sub> (K)","Pressure level"],[["Iron (Fe)","1800","0.01 bar"],["Silicate (MgSiO&#8323;)","1400","0.1 bar"],["H&#8322;O","300","5 bar"],["NH&#8324;SH","200","2 bar"],["NH&#8323;","150","0.7 bar"],["CH&#8324; (ice giants)","80","1.5 bar"]])}
      ${r("Lodders &amp; Fegley (2002); Visscher, Moses &amp; Fegley (2010)")}`),a("Atmospheric Metallicity",`<div class="sci-formula__eq">${t("\\log_{10}(Z/Z_\\odot) = 0.66 - 0.68\\,\\log_{10}(M/M_J)")}</div>
      ...
      ${r("Thorngren &amp; Fortney (2019), ApJL 874, L31")}`),a("Internal Heat Ratio",`<p>Ratio of total emitted flux to absorbed flux, interpolated by mass:</p>
      ${u(["Mass range","Ratio","Analogue"],[["&lt; 0.05 M<sub>J</sub>","1.06","Uranus"],["0.1&ndash;0.2 M<sub>J</sub>","2.6","Neptune"],["0.2&ndash;0.5 M<sub>J</sub>","2.6 &rarr; 1.67","Saturn &rarr; Jupiter"],["0.5&ndash;1.5 M<sub>J</sub>","1.67","Jupiter"]])}
      <p>Effective temperature: ${e("T_{\\text{eff}} = (T_{\\text{eq}}^4 \\cdot \\text{IHR})^{1/4}")}.</p>`),a("Magnetic Field (Dual-Normalised Energy-Flux Dynamo)",`<div class="sci-formula__eq">${t("B_{\\text{surf}} = B_{\\text{ref}} \\cdot \\frac{\\text{raw}(\\text{planet})}{\\text{raw}(\\text{ref})}")}</div>
      ...
      ${u(["Planet","Model B (G)","Observed B (G)","Ratio"],[["Jupiter","4.28","4.28","1.00&times;"],["Saturn","~0.21","0.21","~0.99&times;"],["Uranus","~0.24","0.23","~1.02&times;"],["Neptune","~0.14","0.14","~1.00&times;"]])}
      ...
      ${u(["Planet","Model R<sub>mp</sub>","Observed R<sub>mp</sub>","Error"],[["Jupiter","75 Rp","75 Rp","~0%"],["Saturn","22 Rp","22 Rp","~0%"],["Uranus","19 Rp","18 Rp","~3%"],["Neptune","18 Rp","23 Rp","~21%"]])}
      ${r("Chapman &amp; Ferraro (1931); Peale, Cassen &amp; Reynolds (1979); Christensen+ (2009), Nature 457, 167; Stanley &amp; Bloxham (2004), Nature 428, 151")}`),a("Atmospheric Dynamics",`<div class="sci-formula__eq">${t("L_{\\text{Rh}} = \\pi\\sqrt{U/\\beta}, \\quad \\beta = 2\\Omega/R")}</div>
      ...
      ${r("Rhines (1975), JFM 69; Vasavada &amp; Showman (2005)")}`),a("Oblateness (Darwin&ndash;Radau)",`<div class="sci-formula__eq">${t("f = \\frac{2.5\\,q}{1 + 6.25\\,(1 - 1.5\\xi)^2}, \\quad q = \\frac{\\omega^2 R^3}{GM}")}</div>
      ...
      ${r("Ribas et al. (2005), ApJ 622; Eggleton (1983), ApJ 268")}`),a("Heavy Element / Core Mass",`<div class="sci-formula__eq">${t("M_Z = 49.3\\,M_J^{\\,0.61}\\;M_\\oplus")}</div>
      <p>Total heavy-element content from transit + RV constraints. Estimated core mass:
      ${e("M_{\\text{core}} = \\min(M_Z/2,\\; 25\\,M_\\oplus)")}.</p>
      ${r("Thorngren, Fortney, Murray-Clay &amp; Lopez (2016), ApJ 831, 64")}`),a("Radius Inflation",`<div class="sci-formula__eq">${t("R_{\\text{inflated}} = R \\cdot \\left(1 + 0.1\\,(5/t)^{0.35}\\right)")}</div>
      <p>Young giants are larger due to Kelvin-Helmholtz contraction. Hot Jupiters
      (${e("T_{\\text{eq}} > 1000")} K) receive an additional 0.1&ndash;0.3 ${e("R_J")}
      proximity bonus from stellar irradiation.</p>
      ${r("Fortney, Marley &amp; Barnes (2007), ApJ 659")}`),a("Ring Properties",`<p>Ring mass model with a Gaussian enhancement peaked at Saturn&rsquo;s mass:</p>
      ...
      ${u(["Body","Mass (M<sub>J</sub>)","Model k&#8322;","Observed k&#8322;","Source"],[["Jupiter","1.0","0.385","0.379","Wahl+ 2016 (Juno)"],["Saturn","0.30","0.385","0.390","Lainey+ 2017 (Cassini)"],["Uranus","0.046","0.104","0.104","Gavrilov &amp; Zharkov 1977"],["Neptune","0.054","0.128","0.127","Gavrilov &amp; Zharkov 1977"]])}
      ...
      ${u(["Regime","Mass range","Q","Analogue"],[["Ice giant","&lt; 0.15 M<sub>J</sub>","15,000","Uranus, Neptune"],["Saturn-like","0.2&ndash;0.5 M<sub>J</sub>","2,500","Saturn (resonance locking)"],["Jupiter-like","&ge; 0.8 M<sub>J</sub>","35,000","Jupiter"]])}
      ...
      ${r("Peale, Cassen &amp; Reynolds (1979); Wahl et al. (2016); Lainey et al. (2009, 2017); Fuller, Luan &amp; Quataert (2016)")}`)].join("")}function Ee(){return[a("Kepler&rsquo;s Third Law (Moon Orbit)",`<div class="sci-formula__eq">${t("P = \\frac{2\\pi\\sqrt{a^3 / G(M_p + M_m)}}{86400} \\text{ days}")}</div>
      ...
      <p>Orbital distance inside which tidal forces exceed the satellite&rsquo;s self-gravity, causing disruption. The classical fluid-body result.</p>`),a("Hill Sphere",`<div class="sci-formula__eq">${t("r_H = a_p \\left(\\frac{M_p}{3 M_\\star}\\right)^{1/3}")}</div>
      <p>Gravitational sphere of influence of the planet relative to the host star. Moons beyond ${e("r_H")} cannot remain bound.</p>`),a("Love Number k<sub>2</sub>",`<div class="sci-formula__eq">${t("k_2 = \\frac{1.5}{1 + \\dfrac{19\\mu}{2\\rho g R}}")}</div>
      ...
      ${r("Duchene &amp; Kraus (2013); tidal dissipation model")}`),a("Spin-Orbit Resonance",`<div class="sci-formula__eq">${t("H(p,\\,e)")}</div>
      ...
      ${r("Goldreich &amp; Peale (1966), AJ 71, 425")}`),a("Atmospheric Tide Resistance",`<div class="sci-formula__eq">${t("b_{\\text{atm}} = C \\; \\frac{P_s \\; S}{g \\; T_{\\text{eq}}}")}</div>
      ...
      ${r("Leconte et al. (2015), Science 347, 632; Ingersoll &amp; Dobrovolskis (1978), Nature 275, 37")}`),a("Tidal Force",`<div class="sci-formula__eq">${t("F_{\\text{tidal}} = \\frac{2 G M_1 M_2}{d^3}")}</div>
      ...
      ${r("Wisdom (2004); Peale, Cassen &amp; Reynolds (1979)")}`),a("Moon Tidal Heating on Planet",`<div class="sci-formula__eq">${t("\\dot{E}_{\\text{planet}} = \\frac{21}{2}\\,\\frac{k_{2p}}{Q_p} \\, \\frac{G \\, M_m^{\\,2} \\, R_p^{\\,5} \\, n}{a^6} \\, f(e)")}</div>
      ...
      internal heat budget (${e("\\sim 44")} TW &times; ${e("M / M_\\oplus")} &times; ${e("A")}), tidal heating
      extends the core solidification timescale:</p>
      <div class="sci-formula__eq">${t("\\tau_{\\text{core}} = \\frac{\\tau_{\\text{base}}}{\\max(0.01,\\; 1 - f_{\\text{tidal}})}, \\quad f_{\\text{tidal}} = \\frac{\\dot{E}_{\\text{planet}}}{44 \\times 10^{12} \\cdot (M/M_\\oplus) \\cdot A}")}</div>
      <p>When ${e("f_{\\text{tidal}} \\ge 1")}, tidal heating dominates and the core stays liquid indefinitely.
      This can sustain a magnetic dynamo well past the planet&rsquo;s natural core solidification age.</p>
      ${r("Peale, Cassen &amp; Reynolds (1979); Wisdom (2008)")}`),a("Tidal Recession",`<div class="sci-formula__eq">${t("\\frac{da}{dt} = \\underbrace{\\operatorname{sgn}(\\Omega_p - n)\\;\\frac{3\\,k_{2p}}{Q_p}\\,\\frac{m_m}{m_p}\\,\\frac{n\\,R_p^5}{a^4}}_{\\text{planet tide}} \\;-\\; \\underbrace{\\frac{21}{2}\\,\\frac{k_{2m}}{Q_m}\\,\\frac{m_p}{m_m}\\,\\frac{n\\,R_m^5\\,e^2}{a^4}}_{\\text{moon tide}}")}</div>
      ...
      ${r("Moore (2003); Segatz et al. (1988) \u2014 tidal-convective equilibrium models")}`),a("Moon Surface Temperature",`<div class="sci-formula__eq">${t("T_{\\text{eq}} = \\left(\\frac{L_\\star\\,(1 - a)}{16\\,\\pi\\,\\sigma\\,d^2}\\right)^{1/4}")}</div>
      ...
      ${r("Paranicas et al. (2009); Divine & Garrett (1983) \u2014 Jupiter radiation environment")}`),a("Volatile Inventory &amp; Atmospheric Retention",`<p>Identifies surface ices and thin atmospheres on airless moons via three checks per species:</p>
      <p><b>1. Presence:</b> In <b>Core</b> mode, species availability falls back to the older bulk-density guard (${e("\\rho < \\rho_{\\text{max}}")}) for that ice. In <b>Full</b> and <b>Manual</b> modes, WorldSmith instead folds in explicit water inventory, ammonia fraction, composition override, and manual gas-mix requests before deciding whether a species reservoir is available. Exception: SO&#8322; still requires active tidal feedback or a manual atmospheric request.</p>
      ...
      ${u(["Species","T_{sub} (K)","\\rho_{max}","Source"],[["N\\u2082","35","2.5 g/cm\\u00B3","Fray & Schmitt (2009)"],["CO","35","2.5 g/cm\\u00B3",""],["CH\\u2084","50","2.5 g/cm\\u00B3",""],["CO\\u2082","115","3.2 g/cm\\u00B3",""],["NH\\u2083","130","2.5 g/cm\\u00B3",""],["SO\\u2082","140","volcanic",""],["H\\u2082O","210","3.2 g/cm\\u00B3",""]])}
      ${r("Fray & Schmitt (2009, PSS 57, 2053); Jeans (1916) \u2014 atmospheric escape theory; Chamberlain (1963) \u2014 hydrodynamic escape timescale")}`),a("Moon Worlds in WorldSmith",`<p>WorldSmith&rsquo;s moon-world outputs are built as a layered model rather than a single published formula. The Moon page combines atmosphere, hydrosphere, climate, geology, biosphere, and habitability outputs into one reference state for each moon.</p>
      ...
      <p><b>Reference inputs:</b> this moon-world layer sits on top of the tidal heating, moon temperature, magnetospheric radiation, volatile-retention, magnetosphere, and atmosphere-stability blocks documented above, then adds WorldSmith&rsquo;s own hydrosphere, geology, biosphere, and integrated habitability policy layers.</p>`)].join("")}function Fe(){return[a("Hill Sphere",`<div class="sci-formula__eq">${t("r_H = a \\left(\\frac{m}{3M_\\star}\\right)^{1/3}")}</div>
      ...
      are shown as dimmed amber diamonds.</p>`)].join("")}function He(){return[a("Stellar Absolute Magnitude",`<div class="sci-formula__eq">${t("M_V = 4.81 - 2.5\\log_{10}(L)")}</div>
      ...
      ${r("Bowell et al. (1989) H-G photometry system")}
      <div class="sci-try">
        <div class="sci-try__title">Try it</div>
        <div class="sci-try__row">
      ...
      ${r("Bowell et al. (1989)")}`),a("Phase Angle (Law of Cosines)",`<div class="sci-formula__eq">${t("\\cos\\alpha = \\frac{r^2 + \\Delta^2 - d_h^2}{2\\,r\\,\\Delta}")}</div>
      ...
      <p>If ${e("\\theta_{\\text{moon}} \\ge \\theta_\\star")}: total eclipses. Otherwise: annular only.</p>`)].join("")}function Le(){return[a("Scale Height Correction",`<div class="sci-formula__eq">${t("\\frac{H_{\\text{planet}}}{H_\\oplus} = \\frac{T / T_\\oplus}{g / g_\\oplus}")}</div>
      ...
      ${r("PanoptesV (panoptesv.com/SciFi)")}`),a("OKLab Colour Space",`<p>Perceptually uniform colour space used for all colour interpolation. sRGB &rarr; linear &rarr; LMS (cube roots) &rarr; OKLab:</p>
      ...
      ${r("Bjorn Ottosson (2020) &mdash; OKLab perceptually uniform colour space")}`),a("CO&#8322; Atmospheric Tint",`<div class="sci-formula__eq">${t("\\text{strength} = \\text{clamp}\\!\\left(\\sqrt{f_{\\text{CO}_2}} \\times 0.7,\\; 0,\\; 1\\right)")}</div>
      ...
      ${u(["Body","P (atm)","CO&#8322; %","H&#8322;O %","CH&#8324; %","&tau;","T<sub>surf</sub> (K)"],[["Earth","1.0","0.04","0.40","0","0.70","288"],["Venus","92","96.5","0","0","126","737"],["Mars","0.006","95.3","0","0","0.029","211"]])}
      <p>The greenhouse effect parameter used by the energy-balance model is ${e("G_h = \\tau / 0.5841")}.</p>
      ${r("Robinson &amp; Catling (2012) pressure broadening; Myhre (1998) CO&#8322; band saturation; IPCC TAR (2001) CH&#8324; square-root law. Coefficients: WorldSmith calibration.")}`),a("CO&#8322;&ndash;H&#8322;O Band Overlap Suppression",`<p>CO&#8322; and H&#8322;O share absorption in the 12&ndash;18 &mu;m and 4.3 &mu;m regions. When CO&#8322; is optically thick, those bands are already saturated and additional H&#8322;O contributes little extra opacity. This is modelled by an overlap factor applied to the H&#8322;O optical depth:</p>
      ...
      ${u(["\\u03BB range","Status","Meaning"],[["\\u2265 6","Retained","Firmly held over geological time (> 4.5 Gyr)"],["3 &ndash; 6","Marginal","Slow escape; may be lost on Gyr timescales"],["< 3","Lost","Rapid thermal escape (lost within ~100 Myr)"]])}
      ...
      <p><b>Calibration:</b></p>
      ${u(["Body","T<sub>eq</sub> (K)","F/F<sub>0</sub>","P (atm)","&eta;<sub>abs</sub>","T<sub>exo</sub> (K)","Observed"],[["Earth","254","1.0","1.0","0.94","~944","700&ndash;1400"],["Venus","229","1.9","92","1.00","~229","250&ndash;300"],["Mars","210","0.43","0.006","0.09","~233","200&ndash;350"],["Pluto","32","0.0006","10&#8315;&#8309;","&lt;0.001","~32","~65&ndash;70"]])}
      ...
      ${r("Ribas et al. (2005), ApJ 622, 680 &mdash; Evolution of the Solar Activity over Time.")}`),a("Non-Thermal Escape Enhancement",`<p>Pure Jeans (thermal) escape underestimates loss of light gases from warm terrestrial planets. Charge exchange with stellar-wind protons, polar wind escape through magnetic cusps, and ion pickup by the stellar wind all strip H&#8322; and He regardless of whether the body has a magnetic field.</p>
      ...
      ${u(["Gas","Factor","Lost","Marginal","Retained"],[["H&#8322;","&times;3.0","&lambda; < 9","9 &le; &lambda; < 18","&lambda; &ge; 18"],["He","&times;5.0","&lambda; < 15","15 &le; &lambda; < 30","&lambda; &ge; 30"],["Others","&times;1.0","&lambda; < 3","3 &le; &lambda; < 6","&lambda; &ge; 6"]])}
      ...
      ${u(["Zone","Base moisture"],[["Hadley equatorial (lat &lt; 0.7&times;cell)","0.9 &minus; 0.75&times;fraction"],["Hadley subsidence edge","0.15"],["Ferrel warm-coast","0.70"],["Ferrel cold-coast","0.45"],["Ferrel general","0.55"],["Polar","0.20"]])}
      ...
      and antistellar point while cooling the substellar point.</p>`),a("Environmental Lapse Rate",`<div class="sci-formula__eq">${t("\\frac{dT}{dz} = -6.5\\;\xB0\\text{C/km}")}</div>
      <p>ISA standard tropospheric lapse rate, applied to altitude-adjusted temperatures for
      elevated terrain. Used for all climate zone calculations.</p>`)].join("")}function We(){return[a("Power-Law Flare Rate",`<div class="sci-formula__eq">${t("N(> E) = N_{32} \\left(\\frac{E}{10^{32} \\text{ erg}}\\right)^{-\\alpha}")}</div>
      <p>Cumulative flare frequency distribution: expected flares per day with energy above ${e("E")}.</p>
      ${u(["Spectral bin","T<sub>eff</sub> (K)","Old","Mid","Young","&alpha;"],[["FGK","&ge; 3900","0.05","0.25","1.0","1.8"],["Early M","3200&ndash;3900","0.5","2.0","8.0","2.0"],["Late M","&lt; 3200","2.0","8.0","30.0","2.2"]])}
      ${r("Gunther et al. (2020) TESS superflare rates; Lacy et al. (1976) power-law")}
      <div class="sci-try">
        <div class="sci-try__title">Try it</div>
        <div class="sci-try__row">
      ...
      ${u(["Flare energy (erg)","P(CME)"],[["&lt; 10&sup3;&sup2;","0.005"],["10&sup3;&sup2; &ndash; 10&sup3;&sup3;","0.12"],["10&sup3;&sup3; &ndash; 10&sup3;&sup4;","0.4"],["&gt; 10&sup3;&sup4;","0.75"]])}
      ...
      ${r("Yashiro et al. (2006) flare-CME association rates; probabilities WorldSmith-calibrated")}`),a("CME Rate from Activity Cycle",`<div class="sci-formula__eq">${t("\\text{rate} = 0.5 + 5.5t \\text{ CME/day}")}</div>
      ...
      ${u(["Spectral bin","Old","Mid","Young","&alpha;"],[["FGK (T &ge; 3900 K)","0.05","0.25","1.0","1.8"],["Early M (3200&ndash;3900 K)","0.5","2.0","8.0","2.0"],["Late M (&lt; 3200 K)","2.0","8.0","30.0","2.2"]])}
      ...
      ${r("G\xFCnther et al. (2020) TESS superflare rates; binning WorldSmith")}`),a("Flare Cycle Multiplier",`<p>Flare rate is modulated by an 11-year-analogue activity cycle. At cycle phase ${e("\\phi \\in [0,1]")}:</p>
      ${u(["Spectral bin","Min (&phi;=0)","Mid (&phi;=0.5)","Max (&phi;=1)"],[["FGK","0.35","1.0","1.65"],["Early M","0.6","1.0","1.4"],["Late M","0.75","1.0","1.25"]])}
      <p>Cooler stars have a smaller cycle amplitude, consistent with
      observations that M-dwarf activity varies less over magnetic cycles.</p>`)].join("")}function Oe(){return[a("Local Day Scale",`<div class="sci-formula__eq">${t("\\text{scale} = \\frac{P_{\\text{rot}}}{24}")}</div>
      ...
      <p>Day-of-week index for year ${e("y")}, where ${e("L_i")} is the length of year ${e("i")} and ${e("D_w")} is days per week. Computed in O(rules) time by counting leap rule firings.</p>`)].join("")}function Ie(){return[a("Neighbourhood Volume",`<div class="sci-formula__eq">${t("V = \\frac{4}{3}\\pi R^3")}</div>
      ...
      ${u(["Category","Fraction"],[["Main-sequence stars","72%"],["White dwarfs","6%"],["Brown dwarfs","19%"],["Other (giants, subdwarfs)","3%"]])}
      ${r("Reyle et al. (2021) solar-neighbourhood census; RECONS 10 pc survey")}`),a("Galactic Habitable Zone (GHZ)",`<div class="sci-formula__eq">${t("P = \\exp\\!\\left(-\\frac{1}{2}\\left(\\frac{r - 0.53R}{0.1R}\\right)^2\\right)")}</div>
      ...
      ${r("Lineweaver et al. (2004) Galactic Habitable Zone")}
      <div class="sci-try">
        <div class="sci-try__title">Try it</div>
        <div class="sci-try__row">
      ...
      ${r("Luck &amp; Lambert (2011); Schlesinger et al. (2014)")}`),a("Multiplicity (Stars per System)",`<div class="sci-formula__eq">${t("\\bar{n} = 1 + f_b + 2f_t + 3f_q")}</div>
      <p>Average stars per system from binary (${e("f_b")}), triple (${e("f_t")}), and quadruple (${e("f_q")}) fractions, weighted by spectral class.</p>
      ${u(["Class","Binary","Triple","Quadruple"],[["O","0.70","0.12","0.05"],["B","0.50","0.09","0.04"],["A","0.45","0.08","0.03"],["F","0.46","0.08","0.03"],["G","0.46","0.08","0.03"],["K","0.35","0.05","0.02"],["M","0.27","0.03","0.01"],["WD","0.25","0.02","0.005"],["L/T/Y","0.15","0.01","0.003"]])}
      ${r("Duch&ecirc;ne &amp; Kraus (2013) multiplicity survey")}`),a("Park-Miller PRNG",`<div class="sci-formula__eq">${t("s_{n+1} = 48271 \\cdot s_n \\mod (2^{31} - 1)")}</div>
      ...
      <p>Galactic disk flattening for large neighbourhood radii. At 50 ly the neighbourhood is spherical; at 500 ly z &asymp; 0.55; floors at 0.15.</p>`)].join("")}function De(){return[a("Ocean Fraction by Water Regime",`${u(["Water regime","Ocean fraction"],[["Dry","0%"],["Shallow oceans","50%"],["Extensive oceans","71%"],["Global ocean","90%"],["Deep ocean","95%"],["Ice world","0%"]])}
      ...
      <p>K&ouml;ppen classes E (polar) and X (uninhabitable) are excluded.</p>`),a("Carrying Capacity",`<div class="sci-formula__eq">${t("K = A \\cdot d \\cdot \\frac{1 + (C_e - 1)\\,f_c}{1 + (C_e - 1) \\cdot 0.77}")}</div>
      ...
      ${u(["Era","Density (km&sup2;)","Growth (%/yr)"],[["Hunter-Gatherer","0.05","0.5"],["Neolithic","2","0.8"],["Bronze Age","8","1.0"],["Iron Age","15","1.0"],["Medieval","30","1.0"],["Early Industrial","80","1.5"],["Industrial","200","2.0"],["Post-Industrial","400","0.5"],["Sci-Fi High","1000","0.3"]])}`),a("Logistic Growth (Verhulst)",`<div class="sci-formula__eq">${t("P(t) = \\frac{K}{1 + \\frac{K - P_0}{P_0}\\,e^{-rt}}")}</div>
      ...
      ${r("Verhulst (1838), Correspondance math&eacute;matique et physique")}`),a("Zipf Rank&ndash;Size Distribution",`<div class="sci-formula__eq">${t("P(\\text{rank}) = \\frac{P(1)}{\\text{rank}^q}, \\quad P(1) = \\frac{P_{\\text{total}}}{H(n,q)}")}</div>
      ...
      ${r("Zipf (1949), Human Behavior and the Principle of Least Effort")}`)].join("")}function Ne(){return[a("Water-Aware Radius (Zeng & Sasselov 2016)",`<div class="sci-formula__eq">${t("R = R_{\\text{dry}} \\cdot \\left(1 + \\left(\\frac{R_{50}}{R_{\\text{ref}}} - 1\\right) \\cdot \\min\\!\\left(\\frac{\\text{WMF}}{0.5},\\;1\\right)\\right)")}</div>
      ...
      ${r("Zeng, L. &amp; Sasselov, D. (2016), ApJ 819, 127")}`),a("Core Radius Fraction",`<div class="sci-formula__eq">${t("\\text{CRF} = \\sqrt{\\text{CMF}}")}</div>
      ...
      ${r("Zeng, L. &amp; Jacobsen, S. (2017)")}`),a("Magnetic Dipole Scaling (Olson & Christensen 2006)",`<div class="sci-formula__eq">${t("B_{\\text{surf}} = \\frac{\\sqrt{\\rho_c} \\cdot \\text{CRF}^3 \\cdot M^{1/3} \\cdot C_b}{B_{\\oplus}}")}</div>
      ...
      ${o([["\\rho_c = \\text{CMF} \\cdot \\rho / \\text{CRF}^3","Core density from mass conservation (two-layer model)"],["\\text{CRF} = \\sqrt{\\text{CMF}}","Core radius fraction (Zeng &amp; Jacobsen 2017)"],["M^{1/3}","Heat-flux proxy from planet mass"],["C_b","Three-phase convective boost (see below)"],["B_{\\oplus}","Earth reference: same formula with CMF=0.33, &rho;=5.514, M=1"]])}
      <p><b>Convective boost ${e("C_b(s_f)")}:</b> Three-phase function of solid fraction ${e("s_f = t / \\tau_{\\text{core}}")}:</p>
      <ul style="font-size:13px;color:var(--muted);margin:4px 0 4px 18px">
        <li>${e("s_f < 0.5")}: ramp ${e("1 + 0.4\\,s_f")} &mdash; inner core forming, compositional convection building</li>
        <li>${e("0.5 \\le s_f < 0.85")}: plateau at 1.2 &mdash; peak compositional convection from inner core growth</li>
        <li>${e("s_f \\ge 0.85")}: decay ${e("1.2 \\cdot e^{-2.5(s_f - 0.85)/0.15}")} &mdash; thin-shell suppression (e.g. Mercury)</li>
      </ul>
      ...
      <ul style="font-size:13px;color:var(--muted);margin:4px 0 4px 18px">
        ...
        <li><b>Multipolar</b> (${e("P_{\\text{dip}} < P < 50\\,P_{\\text{dip}}")}): smooth sigmoid transition to 0.1&times; (10&times; dipole reduction)</li>
      ...
      ${r("Olson, P. &amp; Christensen, U. (2006), EPSL 250, 561; Christensen, U. &amp; Aubert, J. (2006), GJI 166")}`),a("Core Solidification Timescale",`<div class="sci-formula__eq">${t("\\tau = (2 + 12 \\cdot \\text{CMF} \\cdot \\sqrt{M_\\oplus}) \\;\\times\\; A \\quad \\text{(Gyr)}")}</div>
      ...
      ${o([["\\tau","Core solidification timescale (Gyr)"],["\\text{CMF}","Core mass fraction (0&ndash;1)"],["M_\\oplus","Planet mass (Earth masses)"],["A","Radioisotope abundance (1.0 = Earth; see below)"]])}`),a("Radioisotope Abundance",`<div class="sci-formula__eq">${t("A = \\sum_i a_i \\cdot w_i")}</div>
      ...
      ${u(["Isotope","Half-life (Gyr)","Heat fraction <i>w<sub>i</sub></i>"],[["U-238","4.47","0.39"],["U-235","0.70","0.04"],["Th-232","14.05","0.40"],["K-40","1.25","0.17"]])}
      <p>When all four abundances equal 1.0, ${e("A = 1.0")} (Earth).
      ${e("A")} scales the internal heat budget, volcanic decay rate, lithosphere cooling age, and core solidification timescale.</p>
      <p>Range: 0.01&ndash;5.0 (per-isotope), 0.1&ndash;3.0 (simple slider).</p>`),a("Stellar CMF Derivation",`<div class="sci-formula__eq">${t("\\text{Fe/Mg} = 0.83 \\cdot 10^{[\\text{Fe/H}]}")}</div>
      ...
      ${r("Schulze, J. et al. (2021), PSJ 2, 113")}`),a("Body Classification",`<p>Rocky bodies are classified by mass:</p>
      ${u(["Class","Condition"],[["Dwarf planet","M &lt; 0.01 M&#8853;"],["Planet","M &ge; 0.01 M&#8853;"]])}
      ...
      ${u(["Class","Condition"],[["Ice world","WMF &gt; 0.1"],["Ocean world","WMF &gt; 0.001"],["Iron world","CMF &gt; 0.6"],["Mercury-like","CMF &gt; 0.45"],["Earth-like","CMF &ge; 0.25"],["Mars-like","CMF &ge; 0.1"],["Coreless","CMF &lt; 0.1"]])}
      <p>Water regime labels: Dry (&lt; 0.01%), Shallow oceans (&lt; 0.1%), Extensive (&lt; 1%),
      Global ocean (&lt; 10%), Deep ocean (&lt; 30%), Ice world (&ge; 30%).</p>`),a("Mantle Outgassing Oxidation States",`<p>Volcanic gas composition depends on mantle oxygen fugacity:</p>
      ${u(["State","&Delta;IW","Primary gases"],[["Highly reduced","&minus;4","H&#8322; + CO"],["Moderately reduced","&minus;2","H&#8322; + CO&#8322; (mixed)"],["Earth-like","+1","CO&#8322; + H&#8322;O"],["Oxidised","+3","CO&#8322; + H&#8322;O + SO&#8322;"]])}
      <p>The iron-w&uuml;stite (IW) buffer sets the reference. Earth is approximately IW+1.</p>
      ${r("Ortenzi et al. (2020), Sci. Rep. 10, 10907")}`)].join("")}function Ke(){return[a("Maximum Mountain Height",`<div class="sci-formula__eq">${t("H_{\\max} = \\frac{\\sigma_y}{\\rho \\cdot g} \\approx \\frac{9{,}267}{g} \\text{ m}")}</div>
      ...
      ${r("Weisskopf, V. F. (1975), &ldquo;Of Atoms, Mountains, and Stars&rdquo;, Science 187, 605&ndash;612")}`),a("Ocean Floor Subsidence (PSM Plate Model)",`<p>Seafloor depth increases with crustal age as the lithosphere cools after formation at a mid-ocean ridge. Two regimes:</p>
      ...
      ${r("Parsons, B. &amp; Sclater, J. G. (1977), JGR 82, 803. Stein, C. &amp; Stein, S. (1992), Nature 359 (GDH1 model).")}`),a("Airy Isostatic Root Depth",`<div class="sci-formula__eq">${t("d_{\\text{root}} = h \\cdot \\frac{\\rho_c}{\\rho_m - \\rho_c}")}</div>
      ...
      ${r("Turcotte, D. L. &amp; Schubert, G. (2014), Geodynamics, Ch. 2")}`),a("Pratt Isostatic Compensation",`<div class="sci-formula__eq">${t("\\rho(h) = \\rho_0 \\cdot \\frac{D}{D + h}")}</div>
      ...
      ${r("Turcotte, D. L. &amp; Schubert, G. (2014), Geodynamics, Ch. 2")}`),a("Volcanic Arc Distance",`<div class="sci-formula__eq">${t("d_{\\text{arc}} = \\frac{h_{\\text{slab}}}{\\tan(\\theta)}")}</div>
      ...
      ${r("Syracuse, E. &amp; Abers, G. (2006), G&sup3;, 7 &mdash; global mean 105&plusmn;19 km")}`),a("Linear Erosion",`<div class="sci-formula__eq">${t("H(t) = H_0 - \\varepsilon \\cdot t")}</div>
      ...
      ${r("Cosmogenic nuclide compilation &mdash; global outcrop median 5.4 m/Myr")}`),a("Spreading Rate Categories",`<p>Seafloor spreading velocity at mid-ocean ridges, classified by tectonic regime:</p>
      ${u(["Regime","Rate (mm/yr)","Label"],[["Mobile lid","20&ndash;200","Active spreading"],["Episodic overturn","5&ndash;50","Episodic spreading"],["Plutonic-squishy","2&ndash;20","Sluggish spreading"],["Stagnant lid","0","No spreading"]])}
      ...
      ${r("Dalton, C. A. et al. (2022), GRL &mdash; global plate speed evolution since 200 Ma")}`),a("Shield Volcano Height Scaling",`<div class="sci-formula__eq">${t("H_{\\text{shield}} = \\frac{10{,}000}{g} \\times f_{\\text{lid}}")}</div>
      ...
      ${r("McGovern, P. J. &amp; Solomon, S. C. (1993, 1998), JGR &mdash; volcanic loading and lithospheric support")}`),a("Continental Margin Dimensions",`<p>Passive continental margins comprise four morphological zones:</p>
      ${u(["Zone","Typical width","Depth range","Slope"],[["Continental shelf","80 km","0&ndash;130 m","~0.1&deg;"],["Continental slope","varies","130&ndash;3,000 m","3&ndash;4&deg;"],["Continental rise","200 km","3,000&ndash;4,500 m","~0.5&deg;"],["Abyssal plain","indefinite","4,500+ m","~0&deg;"]])}
      ...
      ${r("Standard geomorphology references; shelf break 130 m from Pleistocene lowstands")}`),a("Tectonic Regime Probability Distribution",`<div class="sci-formula__eq">${t("P_i = \\frac{w_i}{\\sum_j w_j}, \\quad w_i = f_M(i) \\cdot f_t(i) \\cdot f_W(i) \\cdot f_C(i) \\cdot f_T(i)")}</div>
      ...
      ${r("Valencia, D. et al. (2007), ApJL 670, L45; O&rsquo;Neill, C. &amp; Lenardic, A. (2007), GRL 34; Noack, L. &amp; Breuer, D. (2014), P&amp;SS 98; Korenaga, J. (2010), ApJL 725, L43")}`),a("Composition-Dependent Peak Heights",`${u(["Composition class","H<sub>max</sub> (m)"],[["Iron world","12,000"],["Mercury-like","11,000"],["Earth-like","9,267"],["Mars-like","8,500"],["Ocean world","7,000"],["Ice world","3,000"],["Coreless","7,000"]])}
    ...
    </div>`].join("")}function Be(){return[a("Mean-Motion Resonance Positions",`<div class="sci-formula__eq">${t("a_{\\text{res}} = a_p\\,(p/q)^{2/3}")}</div>
      ...
      ${u(["Resonance","(p/q)<sup>2/3</sup>"],[["3:2 exterior","1.310"],["2:1 exterior","1.587"],["5:2 exterior","1.842"],["1:2 interior","0.630"],["1:4 interior","0.397"],["1:8 interior","0.250"]])}`),a("Condensation Sequence",`<p>Species condense from a cooling solar-composition nebula at characteristic temperatures:</p>
      ${u(["Species","T<sub>cond</sub> (K)","Mass %"],[["Corundum (Al&#8322;O&#8323;)","1700","0.4"],["Iron-nickel","1450","7"],["Enstatite (MgSiO&#8323;)","1350","12"],["Forsterite (Mg&#8322;SiO&#8324;)","1300","14"],["Feldspar","1200","6"],["Troilite (FeS)","700","4"],["Organics","300","6"],["Water ice","170","33"],["NH&#8323; hydrate","130","2"],["CO&#8322; ice","70","5"],["CH&#8324; ice","31","4"],["CO ice","25","3"],["N&#8322; ice","22","2"]])}
      ${r("Lodders (2003), ApJ 591, 1220")}`),a("Dust Equilibrium Temperature",`<div class="sci-formula__eq">${t("T = 279\\,\\frac{\\sqrt{L_\\star}}{\\sqrt{r_{\\text{AU}}}}\\;\\text{K}")}</div>
      ...
      ${r("Wyatt et al. (2007), ApJ 658, 569")}`),a("Blowout Grain Size",`<div class="sci-formula__eq">${t("s_{\\text{blow}} = 0.57\\,L_\\star / M_\\star\\;\\mu\\text{m}")}</div>
      ...
      ${r("Wisdom (1980), AJ 85, 1122")}`),a("Exo-Oort Cloud Hill Scaling",`<div class="sci-formula__eq">${t("r_{\\text{H,OC}} \\approx 1.5\\times10^5\\,\\left(\\frac{M_\\star}{M_\\odot}\\right)^{1/3}\\left(\\frac{R_{\\text{gal}}}{R_{\\text{gal},\\odot}}\\right)^{2/3}\\;\\text{AU}")}</div>
      <p>WorldSmith scales exo-Oort cloud size from the parent system&rsquo;s Hill radius in the Galactic potential. This follows the same ${e("M_\\star^{1/3} R_{\\text{gal}}^{2/3}")} dependence used in exo-Oort scaling work.</p>
      ${o([["r_{\\text{H,OC}}","Characteristic Hill / tidal size of the exo-Oort reservoir"],["M_\\star","Total mass of the host stellar system"],["R_{\\text{gal}}","Galactocentric distance of the system"],["R_{\\text{gal},\\odot}","Solar-circle reference radius used by WorldSmith's Local Cluster defaults"]])}
      ${r("Moro-Mart\xEDn (2019, AJ 157, 86), following Hanse et al. (2018) and Veras et al. (2014)")}`),a("Exo-Oort Cloud Edges",`<div class="sci-formula__eq">${t("a_{\\text{in}} \\approx 3\\times10^3\\,\\left(\\frac{r_{\\text{H,OC}}}{1.5\\times10^5\\;\\text{AU}}\\right)")}</div>
      ...
      ${r("Wyatt et al. (2017, MNRAS) for scattering outcomes; Brasser et al. (2010, A&A 516, A72) and Kaib et al. (2011, Icarus 215, 491) for environment dependence; Moro-Mart\xEDn (2019, AJ 157, 86) for the Solar LPC flux reference")}`),a("Oort Cloud Authoring Layers (WorldSmith Overlay)",`<p><b>Automatic baseline science layer:</b> the Oort-cloud formulas above are the literature-inspired baseline model. WorldSmith resolves that automatic model first, then optionally layers authoring controls on top.</p>
      ...
      ${r("Automatic baseline science: see the Oort-cloud entries above. Guided and manual controls are WorldSmith authoring overlays resolved in engine/oortCloud.js.")}`),a("IR Excess at 24 &mu;m",`<div class="sci-formula__eq">${t("\\text{excess} = f \\cdot \\frac{B_\\nu(T_{\\text{disk}},\\,24\\,\\mu\\text{m})}{B_\\nu(T_\\star,\\,24\\,\\mu\\text{m})}")}</div>
      <p>Ratio of disk to stellar flux at 24 &mu;m via Planck functions.
      Detectable if &gt; 0.1 (easily), marginal 0.01&ndash;0.1, undetectable &lt; 0.01.</p>`)].join("")}function Je(){let i=(s,l)=>`<div class="sci-formula"><h3 class="sci-formula__name">${s}</h3>${l}</div>`;return[`<p class="sci-diverge-intro">WorldSmith aims to reproduce published astrophysical models wherever possible.
    ...
    to WorldSmith; items marked <b>Simplified</b> are reductions of published work.</p>`,i("Oort Cloud Mass And LPC Flux Proxy (WS-derived / Simplified)",`<p>WorldSmith&rsquo;s Oort-cloud mass and long-period-comet flux are not taken from a single published closed-form model. The edge scaling follows exo-Oort Hill-radius work, but the mass and LPC-rate outputs are WorldSmith proxies built from five factors: total giant mass, outer-giant extent, inner-ejector depletion, age saturation, and environmental retention.</p>
      ...
      ${r("Coefficients: WorldSmith calibration. Functional forms: Myhre (1998), IPCC TAR (2001), Robinson &amp; Catling (2012).")}`),i("CO&#8322;&ndash;H&#8322;O Band Overlap Suppression (WS-derived)",`<p>The half-saturation model ${e("\\omega = 1/(1 + \\tau_{\\text{CO}_2}/6)")} and the constant
      ${e("k = 6")} are WorldSmith-derived. No published radiative-transfer study provides a
      ...
      ${r("H&#8322;&ndash;N&#8322; CIA: Wordsworth &amp; Pierrehumbert (2013), Science 339. SO&#8322;/NH&#8323; overlap: WorldSmith calibration.")}`),i("Surface Temperature Divisor (WS-derived)",`<p>The factor ${e("\\text{surfDiv} = 1 - 0.1 \\cdot \\min(\\tau, 1)")} that ramps from
      1.0 (airless) to 0.9 (atmosphere with ${e("\\tau \\ge 1")}) is a WorldSmith parameterisation.</p>
      ...
      ${r("CMF scaling: Zeng &amp; Sasselov (2016). Exponent: WorldSmith fit to Solar System data.")}`),i("Core Solidification Timescale (Simplified)",`<p>The formula ${e("\\tau = 2 + 12 \\cdot \\text{CMF} \\cdot \\sqrt{M_\\oplus}")} is a WorldSmith
      ...
      at 4.6 Gyr), Mars ~2.6 Gyr (fully solidified), Mercury ~6.7 Gyr (just solidified at 4.6 Gyr).</p>`),i("Magnetic Field: Three-Phase Convective Boost (WS-derived)",`<p>The ${e("C_b(s_f)")} function with its phase boundaries (0.5, 0.85) and exponential
      ...
      outer core narrows. The decay was calibrated so Mercury (${e("s_f \\approx 1")})
      produces ~0.8% of Earth&rsquo;s field, matching MESSENGER observations (~1%).</p>`),i("Magnetic Field: Dipolar Limit Scaling (Simplified)",`<p>The dipolar limit ${e("P_{\\text{dip}} = 96 \\cdot \\sqrt{M} \\cdot \\sqrt{\\text{CMF}/0.33}")}
      ...
      (5832 h) has no field.</p>`),i("Magnetic Field: Multipolar Factor 0.1 (Simplified)",`<p>The 10&times; reduction in dipole field strength for multipolar dynamos is a literature-informed
      ...
      function would produce at the dipolar limit.</p>`),i("Core Radius Fraction CRF = &radic;CMF (Simplified)",`<p>The relation ${e("\\text{CRF} = \\sqrt{\\text{CMF}}")} is a first-order approximation.
      ...
      ${r("Zeng, L. &amp; Jacobsen, S. (2017). Approximation: WorldSmith simplification.")}`),i("Water-Radius Inflation: Linear Interpolation (Simplified)",`<p>WorldSmith linearly interpolates between the Zeng dry curve (${e("R = 1.00\\,M^{0.270}")})
      ...
      approximate and should not be interpreted as sharp physical transitions.</p>`),i("Tectonic Regime Probabilities (WS-derived)",`<p>The entire five-factor multiplicative model for tectonic regime probabilities is a WorldSmith
      ...
      ${r("Qualitative basis: Valencia et al. (2007), O'Neill &amp; Lenardic (2007), Noack &amp; Breuer (2014), Korenaga (2010). Quantitative model: WorldSmith.")}`),i("Habitable Zone: Chromant Desmos Correction (Modified)",`<p>The Seff polynomials and the temperature proxy ${e("T_{\\text{eff}} = 5778 \\cdot M^{0.55}")}
      ...
      ${e("b = C \\cdot P_s \\cdot S / (g \\cdot T_{\\text{eq}})")} is a WorldSmith calibration.</p>
      ...
      (${e("\\mu")} = 0.3 GPa, Q = 2) composition classes are WorldSmith calibrations that override
      ...
      ${r("Schulze, J. et al. (2021), PSJ 2, 113. Simplification: WorldSmith.")}`),i("Vegetation Colour Extrapolation (Simplified)",`<p>PanoptesV provides pre-computed vegetation colours at 1, 3, and 10 atm for spectral
      ...
      ${r("LUT data: PanoptesV (panoptesv.com/SciFi). Extrapolation: WorldSmith.")}`),i("Spin-Orbit Resonance Capture Thresholds (Simplified)",`<p>The thresholds for resonance capture (H &gt; 0.25 for 3:2, H &gt; 0.5 for 2:1 and 5:2)
      ...
      ${r("Goldreich, P. &amp; Peale, S. (1966), AJ 71, 425. Thresholds: WorldSmith simplification.")}`),i("Flare Rate Binning (Simplified)",`<p>The N&#8323;&#8322; values (flares per day above 10&sup3;&sup2; erg) are discretised into three spectral
      ...
      ${r("Parsons &amp; Sclater (1977), JGR 82, 803. Adjustment: WorldSmith calibration.")}`),i("Shield Volcano 1/g Scaling (Simplified)",`<p>The shield volcano height formula ${e("H = 10{,}000/g \\times f_{\\text{lid}}")} uses a
      ...
      glacioeustatic lowstands.</p>`),i("Gas Giant Internal Heat Ratio Ramps (WS-derived)",`<p>The mass-dependent ramp for internal heat ratio (1.0 at Uranus-mass to 1.67 at
      ...
      well explained and is treated as an outlier.</p>`),i("Gas Giant Ring Mass Gaussian Model (WS-derived)",`<p>The ring mass model ${e("M_{\\text{ring}} = 3 \\times 10^{19} \\, e^{-0.5(M/M_J - 1)^2}")} kg
      ...
      ${e("\\varepsilon")} is a WorldSmith parameterisation. Published tidally locked climate models
      ...
      patterns that depend on composition, pressure, and stellar spectrum.</p>`)].join("")}function Ve(){return[a("Titius-Bode Orbit Spacing",`<div class="sci-formula__eq">${t("a_n = \\begin{cases} a_1 & n = 1 \\\\ a_1 + s \\cdot 2^{n-2} & n \\ge 2 \\end{cases}")}</div>
      ...
      <p>WorldSmith also reuses Roche-limit logic inside the ring-science pipeline. In rocky-world auto mode, rings only appear when an assigned moon&rsquo;s current periapsis crosses the rocky Roche limit, turning the disrupted-moon case into a visible ring source rather than a stable moon orbit.</p>`),a("Binary Stability Limits (Holman-Wiegert)",`<p>For binary systems, WorldSmith uses Holman &amp; Wiegert (1999) style empirical critical radii to decide whether an orbit stays comfortably stable in an S-type or P-type frame.</p>
      ...
      ${r("Holman &amp; Wiegert (1999), AJ 117, 621; Artymowicz &amp; Lubow (1994), ApJ 421, 651")}`),a("Companion Flux and Habitable-Zone Shift",`<p>Outer companion stars are sampled across their hierarchy orbits and their mean visible-light forcing is added to the active host frame as an extra heating term:</p>
      ...
      ${r("WorldSmith hierarchical host-frame flux model in engine/homeSystem/flux.js")}`),a("Hierarchical Guardrail for Triples and Quads",`<p>For nested triples and quads, WorldSmith uses a Mardling-Aarseth style separation floor so outer branches do not crowd inner pairs:</p>
      ...
      ${o([["a_{\\text{in}}","inner pair semi-major axis"],["a_{\\text{out,min}}","recommended minimum outer semi-major axis"],["q_{\\text{out}}","outer companion-to-inner-system mass ratio"],["e_{\\text{out}}","outer eccentricity"],["f_i","inclination factor used by WorldSmith (0.7&ndash;1.0)"]])}
      ...
      ${r("Mardling &amp; Aarseth (2001), MNRAS 321, 398; WorldSmith hierarchy guardrails in engine/homeSystem/stability.js")}`)].join("")}function ze(i,s){let l=s/100,d=Math.log(Math.max(i,1e-6)),n=Math.min(1/3,.257-.0161*d),p=(1.07-.21*l)*Math.pow(i,n),f=i*5.51/Math.pow(p,3);return{R:p,rho:f}}function je(i){let s=i.querySelector("#sci-mlr-mass"),l=i.querySelector("#sci-mlr-slider"),d=i.querySelector("#sci-mlr-result");if(s&&l&&d){let _=()=>{let h=X(Number(s.value));d.textContent=`${m(h,4)} L\u2609`};O({numberEl:s,sliderEl:l,min:.075,max:100,step:.01,mode:"log"}),s.addEventListener("input",_),l.addEventListener("input",()=>{s.dispatchEvent(new Event("input"))}),_()}let n=i.querySelector("#sci-hz-mass"),p=i.querySelector("#sci-hz-slider"),f=i.querySelector("#sci-hz-result");if(n&&p&&f){let _=()=>{let h=Number(n.value),M=X(h),C=oe(h),I=ne({luminosityLsol:M,teffK:C});j(f,`${m(I.innerAu,2)} \u2013 ${m(I.outerAu,2)} AU`)};O({numberEl:n,sliderEl:p,min:.075,max:100,step:.01,mode:"log"}),n.addEventListener("input",_),p.addEventListener("input",()=>{n.dispatchEvent(new Event("input"))}),_()}let v=i.querySelector("#sci-dens-mass"),w=i.querySelector("#sci-dens-mass-slider"),S=i.querySelector("#sci-dens-cmf"),k=i.querySelector("#sci-dens-cmf-slider"),R=i.querySelector("#sci-dens-result");if(v&&w&&S&&k&&R){let _=()=>{let{R:h,rho:M}=ze(Number(v.value),Number(S.value));j(R,`${m(h,3)} R\u2295 \u2014 ${m(M,2)} g/cm\xB3`)};O({numberEl:v,sliderEl:w,min:.01,max:100,step:.01,mode:"log"}),O({numberEl:S,sliderEl:k,min:0,max:100,step:1,mode:"linear"}),[v,w,S,k].forEach(h=>h.addEventListener("input",_)),_()}let T=i.querySelector("#sci-hmag-rad"),L=i.querySelector("#sci-hmag-alb"),A=i.querySelector("#sci-hmag-alb-slider"),G=i.querySelector("#sci-hmag-result");if(T&&L&&G){let _=()=>{let h=Number(T.value),M=Number(L.value);if(h>0&&M>0){let C=ce({radiusKm:h,geometricAlbedo:M});G.textContent=`H = ${m(C,2)}`}};A&&O({numberEl:L,sliderEl:A,min:.01,max:1,step:.01,mode:"linear"}),[T,L].forEach(h=>h.addEventListener("input",_)),A&&A.addEventListener("input",_),_()}let E=i.querySelector("#sci-flare-temp"),W=i.querySelector("#sci-flare-temp-slider"),D=i.querySelector("#sci-flare-age"),c=i.querySelector("#sci-flare-age-slider"),b=i.querySelector("#sci-flare-result");if(E&&D&&b){let _=()=>{let h=le({teffK:Number(E.value),ageGyr:Number(D.value)});be(b,{countLabel:"32",countValue:h.N32,alphaValue:h.alpha})};W&&O({numberEl:E,sliderEl:W,min:2e3,max:1e4,step:100,mode:"linear"}),c&&O({numberEl:D,sliderEl:c,min:.01,max:13,step:.1,mode:"linear"}),[E,W,D,c].filter(Boolean).forEach(h=>h.addEventListener("input",_)),_()}let x=i.querySelector("#sci-leap-len"),$=i.querySelector("#sci-leap-result");if(x&&$){let _=M=>{if(M===0)return{p:0,q:1};let C=0,I=1;for(let F=1;F<=1e4;F++){let q=Math.round(M*F);if(Math.abs(q/F-M)<1e-9){C=q,I=F;break}}return{p:C,q:I}},h=()=>{let M=Number(x.value),C=M-Math.floor(M);if(C<1e-9){U($,[],"No leap cycle needed (integer year)");return}let F=de(C,6).slice(1).map(_).filter(q=>q.q>0);if(F.length===0){U($,[],"No usable cycle found");return}U($,F.map(q=>{let se=Math.abs(q.p/q.q-C),re=se>1e-12?Math.round(1/se):1/0,_e=re===1/0?"exact":`~1 day drift per ${m(re,0)} yr`;return{fraction:`${q.p}/${q.q}`,description:`${q.p} leap day${q.p!==1?"s":""} every ${q.q} years (${_e})`}}))};x.addEventListener("input",h),h()}let P=i.querySelector("#sci-ghz-r"),g=i.querySelector("#sci-ghz-loc"),y=i.querySelector("#sci-ghz-result");if(P&&g&&y){let _=()=>{let h=Number(P.value),M=Number(g.value),C=.53*h,I=.1*h,F=Math.exp(-.5*Math.pow((M-C)/I,2)),q=M>=.47*h&&M<=.6*h;j(y,`${m(F*100,1)}%${q?" (in hard band)":""}`)};[P,g].forEach(h=>h.addEventListener("input",_)),_()}let B=i.querySelector("#sci-tec-grav"),Q=i.querySelector("#sci-tec-grav-slider"),te=i.querySelector("#sci-tec-mtn"),ae=i.querySelector("#sci-tec-shield"),ie=i.querySelector("#sci-tec-root");if(B&&Q&&te&&ae&&ie){let _=()=>{let h=Number(B.value);h>0&&(te.textContent=`${m(pe(h),0)} m`,ae.textContent=`${m(me(h),0)} m`,ie.textContent=`${m(ue(5e3),0)} m`)};O({numberEl:B,sliderEl:Q,min:.05,max:5,step:.01,mode:"log"}),B.addEventListener("input",_),Q.addEventListener("input",()=>{B.dispatchEvent(new Event("input"))}),_()}}var ye=[{id:"stellar",title:"Stellar Physics",count:8,builder:Pe},{id:"evolution",title:"Stellar Evolution",count:7,builder:ke},{id:"planetary",title:"Planetary Physics",count:12,builder:Re},{id:"gasgiant",title:"Gas Giant Physics",count:13,builder:Ae},{id:"interior",title:"Interior &amp; Composition",count:7,builder:Ne},{id:"tectonics",title:"Tectonics &amp; Geodynamics",count:14,builder:Ke},{id:"orbital",title:"Orbital Mechanics",count:13,builder:Ee},{id:"lagrange",title:"Lagrange Points",count:4,builder:Fe},{id:"photometry",title:"Photometry &amp; Magnitudes",count:8,builder:He},{id:"atmosphere",title:"Atmosphere &amp; Colour",count:9,builder:Le},{id:"climate",title:"Climate Classification",count:7,builder:Ge},{id:"activity",title:"Stellar Activity",count:7,builder:We},{id:"calendar",title:"Calendar Systems",count:5,builder:Oe},{id:"cluster",title:"Local Cluster",count:7,builder:Ie},{id:"population",title:"Population Dynamics",count:5,builder:De},{id:"system",title:"System Architecture",count:7,builder:Ve},{id:"debris",title:"Debris Disks",count:12,builder:Be},{id:"divergences",title:"Divergences from Published Science",count:36,builder:Je}];function lt(i){let s=document.createElement("div");s.className="page";let l=ye.map(c=>`<a class="sci-toc__link" data-target="sci-${c.id}" href="#sci-${c.id}">${c.title}</a>`).join(""),d=ye.map((c,b)=>`<details class="sci-section" id="sci-${c.id}"${b===0?" open":""}>
        <summary class="sci-section__summary">
          <span class="sci-section__title">${c.title}</span>
          <span class="sci-section__count">${c.count} equations</span>
        </summary>
        <div class="sci-section__body">${c.builder()}</div>
      </details>`).join("");s.innerHTML=`
    <div class="panel">
      ...
      <div class="panel__body">
        ...
        <div class="sci-toc">${l}</div>
      </div>
    </div>
    <div class="sci-sections">${d}</div>
  `,i.innerHTML="",i.appendChild(s);let n=Array.from(s.querySelectorAll(".sci-section")),p=Array.from(s.querySelectorAll(".sci-formula")),f=Array.from(s.querySelectorAll(".sci-toc__link")),v=s.querySelector("#sciencePageSearch"),w=s.querySelector("#sciencePageSearchStatus"),S=s.querySelector("#sciencePageSearchResults"),k=Ce(s),R=null,T=!1,L=n.find(c=>c.open)?.id||n[0]?.id||null,A=(c,{exclusive:b=!T}={})=>{c&&(b?n.forEach(x=>{x.open=x===c}):c.open=!0,!T&&c.id&&(L=c.id))},G=()=>{R&&(clearTimeout(R),R=null),s.querySelectorAll(".sci-formula.is-search-hit").forEach(c=>{c.classList.remove("is-search-hit")})},E=c=>{if(!c)return;let b=s.querySelector(`#${c}`);if(!b)return;let x=b.closest(".sci-section");A(x),G(),b.classList.add("is-search-hit"),ee(b,{block:"center"}),b.focus?.({preventScroll:!0}),R=window.setTimeout(()=>{b.classList.remove("is-search-hit"),R=null},2200)},W=c=>{let b=N(c);if(G(),!b){T=!1,s.classList.remove("is-search-filtering"),p.forEach(y=>{y.hidden=!1,y.classList.remove("is-search-match","is-search-filtered-out")}),n.forEach(y=>{y.hidden=!1,y.classList.remove("is-search-match","is-search-filtered-out")}),f.forEach(y=>{y.hidden=!1,y.classList.remove("is-search-filtered-out")});let g=n.find(y=>y.id===L)||n[0]||null;return A(g,{exclusive:!0}),[]}let x=ge(k,b,k.length),$=new Set(x.map(g=>g.id)),P=new Set(x.map(g=>g.sectionId));return T=!0,s.classList.add("is-search-filtering"),p.forEach(g=>{let y=$.has(g.id);g.hidden=!y,g.classList.toggle("is-search-match",y),g.classList.toggle("is-search-filtered-out",!y)}),n.forEach(g=>{let y=P.has(g.id);g.hidden=!y,g.open=y,g.classList.toggle("is-search-match",y),g.classList.toggle("is-search-filtered-out",!y)}),f.forEach(g=>{let y=P.has(g.dataset.target||"");g.hidden=!y,g.classList.toggle("is-search-filtered-out",!y)}),x},D=(c,b=null)=>{if(!S||!w)return[];let x=N(c);if(!x)return S.hidden=!0,S.innerHTML="",w.hidden=!0,w.textContent="",[];let $=b||ge(k,x,k.length),P=$.slice(0,8);return w.hidden=!1,w.textContent=$.length>0?`${$.length} match${$.length===1?"":"es"}${$.length>P.length?`, showing top ${P.length}`:""}`:"No matches found",$.length===0?(S.hidden=!1,S.innerHTML='<div class="sci-search__empty">No formulas or topics matched that search.</div>',[]):(S.hidden=!1,S.innerHTML=P.map(g=>`
          <button class="sci-search__result" type="button" data-target-id="${g.id}">
            <span class="sci-search__result-title">${g.title}</span>
            <span class="sci-search__result-meta">${g.sectionTitle}</span>
          </button>`).join(""),P)};n.forEach(c=>{c.addEventListener("toggle",()=>{if(c.open){if(!T&&c.id&&(L=c.id),T)return;n.forEach(b=>{b!==c&&(b.open=!1)})}})}),f.forEach(c=>{c.addEventListener("click",b=>{b.preventDefault();let x=c.dataset.target,$=s.querySelector(`#${x}`);$&&(A($),ee($,{block:"start"}))})}),v?.addEventListener("input",()=>{let c=W(v.value);D(v.value,c)}),v?.addEventListener("keydown",c=>{if(c.key!=="Enter")return;let b=W(v.value),[x]=D(v.value,b);x&&(c.preventDefault(),E(x.id))}),S?.addEventListener("click",c=>{let b=c.target.closest(".sci-search__result");b&&E(b.getAttribute("data-target-id"))}),je(s),he().then(c=>fe(s,c))}export{lt as initSciencePage};
