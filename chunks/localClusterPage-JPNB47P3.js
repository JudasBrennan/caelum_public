...
The system at (0, 0, 0) is treated as the home star.`,"Import Cluster":"Parse the pasted tab-separated cluster table and replace the current neighbourhood with the imported systems."};function Ie(s,n=2){return`${f(s,n)} ly`}function De(s){let n=Number(s);return Number.isFinite(n)?(n>0?"+":"")+f(n,2):"\u2014"}function Ae(s,n){let e=Number(s);return Number.isFinite(e)?Math.round(e):n}function Re(s){return String(s??"").replace(/\s+/g," ").trim().slice(0,80)}var Ge=["single","binary","triple","quadruple"];function Y(s){return Ge[N(s,1,4)-1]}var $e=[{key:"O",label:"O-type star"},{key:"B",label:"B-type star"},{key:"A",label:"A-type star"},{key:"F",label:"F-type star"},{key:"G",label:"G-type star"},{key:"K",label:"K-type star"},{key:"M",label:"M-type star"},{key:"D",label:"White Dwarf"},{key:"LTY",label:"Brown Dwarf"},{key:"OTHER",label:"Other"}];function Fe(s){let n=s.trim().toUpperCase();if(/^[LTY]/.test(n))return"LTY";if(/^D/.test(n))return"D";let e=n.match(/^([OBAFGKM])/);return e?e[1]:"OTHER"}function Pe(s){let n=s.trim().split(/\r?\n/).filter(l=>l.trim());if(n.length===0)return[];let e=/system name|coordinates|constituents/i.test(n[0])?1:0,d=[],c=Date.now();for(let l=e;l<n.length;l++){let t=n[l].split("	");if(t.length<4)continue;let i=t[0].trim(),v=t[1].match(/\(\s*([^,]+),\s*([^,]+),\s*([^)]+)\)/);if(!v)continue;let C=parseFloat(v[1]),L=parseFloat(v[2]),x=parseFloat(v[3]);if(!Number.isFinite(C)||!Number.isFinite(L)||!Number.isFinite(x))continue;let V=Math.hypot(C,L,x),H=C===0&&L===0&&x===0,U=t[3].trim().split(/,\s*(?:originally|formerly)/i)[0].trim().split(/\s*\+\s*/),K=H?[{objectClassKey:"HOME"}]:U.map(_=>({objectClassKey:Fe(_)}));d.push({id:H?"home":`imported-${c}-${l}`,name:i||`Star System ${l}`,index:l-e,isHome:H,objectClassKey:H?"HOME":K[0].objectClassKey,multiplicity:Y(K.length),components:K,x:C,y:L,z:x,distanceLy:V})}return d}function qe(s,n){let e=Math.random(),d=Math.random()*2*Math.PI,c=Math.acos(2*Math.random()-1),l=Math.cbrt(e)*s,t=l*Math.sin(c)*Math.cos(d),i=l*Math.sin(c)*Math.sin(d),v=l*Math.cos(c)*n;return{x:t,y:i,z:v,distanceLy:Math.hypot(t,i,v)}}function Be(s,n){let e={};for(let c of n.addedSystems)for(let l of c.components||[])l.objectClassKey!=="HOME"&&(e[l.objectClassKey]=(e[l.objectClassKey]||0)+1);let d=new Set(n.removedSystemIds);for(let c of s.systems)if(d.has(c.id))for(let l of c.components||[])l.objectClassKey!=="HOME"&&(e[l.objectClassKey]=(e[l.objectClassKey]||0)-1);for(let[c,l]of Object.entries(n.componentOverrides)){let t=s.systems.find(i=>i.id===c);if(t){for(let i of t.components||[])i.objectClassKey!=="HOME"&&(e[i.objectClassKey]=(e[i.objectClassKey]||0)-1);for(let i of l.components||[])i.objectClassKey!=="HOME"&&(e[i.objectClassKey]=(e[i.objectClassKey]||0)+1)}}return e}function Ze(s,n){let e=new Set(n.removedSystemIds),d=[];for(let c of s.systems){if(e.has(c.id))continue;let l=n.componentOverrides[c.id];l?d.push({...c,components:l.components,multiplicity:l.multiplicity}):d.push(c)}for(let c of n.addedSystems)d.push(c);return d}function ie(s){if(!Array.isArray(s.components)||s.components.length<=1){let n=M(s.objectClassKey,{isHome:s.isHome});return n==="LTY"?"L/T/Y":n}return s.components.map(n=>{let e=M(n.objectClassKey);return e==="LTY"?"L/T/Y":e}).join(" + ")}var Ye=[{title:"Getting Started",body:"The Local Cluster page generates a procedural neighbourhood of nearby star systems. Your home star sits at the centre, surrounded by randomly placed neighbours that the Visualiser can display as a 3D cluster."},{title:"Seed and Radius",body:"Change the random seed to regenerate the cluster layout. Adjust the neighbourhood radius to control the volume of space sampled and the number of systems generated."},{title:"Editing Systems",body:"Click any system to rename it, adjust its position, or edit its metallicity. Each neighbour has a spectral class and giant-planet probability derived from its galactic position."},{title:"Galactic Context",body:"The Galactic Habitable Zone indicator shows whether your star\u2019s position favours Earth-like planet formation. Metallicity gradients and stellar density vary with galactic radius."},{title:"Stellar Census",body:"The output panel breaks down the neighbourhood by spectral class, counting O, B, A, F, G, K, M stars, white dwarfs, and brown dwarfs. Binary and triple system rates are estimated."}];function dt(s){let n=ve(),e=me(n),d=ae(),c=null,l=[],t=document.createElement("div");t.className="page",t.innerHTML=`
  ...
  `,s.appendChild(t),ge(t),Se({steps:Ye,storageKey:"worldsmith.cluster.tutorial",container:t,triggerBtn:t.querySelector("#clusterTutorials")});let i=t.querySelector("#clusterGalacticRadius"),v=t.querySelector("#clusterLocation"),C=t.querySelector("#clusterRadius"),L=t.querySelector("#clusterDensity"),x=t.querySelector("#clusterSeed"),V=t.querySelector("#clusterGalacticRadiusSlider"),H=t.querySelector("#clusterLocationSlider"),oe=t.querySelector("#clusterRadiusSlider"),re=t.querySelector("#clusterDensitySlider"),U=t.querySelector("#clusterKpis"),K=t.querySelector("#clusterObjectsBody"),_=t.querySelector("#clusterSystemsBody");function le(a,r){let p=Re(d?.[a.id]);return p||(a.isHome?r:a.name)}q({numberEl:i,sliderEl:V,min:1e3,max:1e6,step:1,mode:"auto"}),q({numberEl:v,sliderEl:H,min:0,max:1e6,step:1,mode:"auto"}),q({numberEl:C,sliderEl:oe,min:.1,max:25,step:.1,mode:"auto"}),q({numberEl:L,sliderEl:re,min:1e-6,max:.1,step:1e-6,mode:"auto"});function ce(){i.value=String(e.galacticRadiusLy),v.value=String(e.locationLy),C.value=String(e.neighbourhoodRadiusLy),L.value=String(e.stellarDensityPerLy3),x.value=String(e.randomSeed),i.dispatchEvent(new Event("input",{bubbles:!0})),v.dispatchEvent(new Event("input",{bubbles:!0})),C.dispatchEvent(new Event("input",{bubbles:!0})),L.dispatchEvent(new Event("input",{bubbles:!0}))}function de(){let a=N(Number(i.value),1e3,1e6),r=N(Number(v.value),0,a),p=N(Number(C.value),.1,25),y=N(Number(L.value),1e-6,.1),o=N(Ae(x.value,ee.randomSeed),1,2147483646);e.galacticRadiusLy=a,e.locationLy=r,e.neighbourhoodRadiusLy=p,e.stellarDensityPerLy3=y,e.randomSeed=o,i.value=String(e.galacticRadiusLy),v.value=String(e.locationLy),C.value=String(e.neighbourhoodRadiusLy),L.value=String(e.stellarDensityPerLy3),x.value=String(e.randomSeed),i.dispatchEvent(new Event("input",{bubbles:!0})),v.dispatchEvent(new Event("input",{bubbles:!0})),C.dispatchEvent(new Event("input",{bubbles:!0})),L.dispatchEvent(new Event("input",{bubbles:!0}))}function T(){d=ae();let a=be(),r=fe(a),p=typeof r?.name=="string"&&r.name.trim()?r.name.trim():"home star system",y=r?.metallicityFeH??0,o=te({...e,homeMetallicityFeH:y}),w=F(),S=Ze(o,w),h=Be(o,w);c=o,l=S;let b=[{label:"Galactic Habitable Zone",tip:m["Galactic Habitable Zone"],value:`${f(o.galacticHabitableZoneLy.inner,0)} - ${f(o.galacticHabitableZoneLy.outer,0)}`,meta:"ly"},{label:"In Galactic Habitable Zone?",tip:m["In Galactic Habitable Zone?"],value:o.inHabitableZone?"Yes":"No",meta:`Location ${f(o.inputs.locationLy,0)} ly`},{label:"GHZ Probability",tip:m["GHZ Probability"],value:`${f(o.ghzProbability*100,1)}%`,meta:"Lineweaver (2004) Gaussian model"},{label:"Neighbourhood Volume",tip:m["Neighbourhood Volume"],value:f(o.neighbourhoodVolumeLy3,2),meta:"ly^3"},{label:"Estimated Stellar-Mass Objects",tip:m["Estimated Stellar-Mass Objects"],value:f(o.rawStellarMassObjects,2),meta:"raw estimate"},{label:"Main Sequence Total",tip:m["Main Sequence Total"],value:f(o.mainSequenceTotal,0),meta:"O/B/A/F/G/K/M"},{label:"Total Stellar-Mass Objects",tip:m["Total Stellar-Mass Objects"],value:f(o.totalStellarMassObjects,0),meta:"rounded class sum"},{label:"Total Star Systems",tip:m["Total Star Systems"],value:f(o.systemCounts.total,0),meta:`${f(o.systemCounts.single,0)} single | ${f(o.systemCounts.binary,0)} binary | ${f(o.systemCounts.triple,0)} triple | ${f(o.systemCounts.quadruple,0)} quadruple`},{label:"Generated Neighbours",tip:m["System Coordinates"],value:f(o.systems.length-1,0),meta:o.systemsOmitted>0?`plus 1 home \u2014 ${f(o.systemsOmitted,0)} systems omitted (visualiser cap: 750)`:"plus 1 home system"}];we(U,b),Le(K,o.stellarRows,h),je(_,S,{homeDefaultName:p,resolveSystemDisplayName:le,formatSystemLabel:ie,formatLy:Ie,fmtFeH:De})}function Ee(){let a=F();return{addedSystemCount:Array.isArray(a?.addedSystems)?a.addedSystems.length:0,hiddenSystemCount:Array.isArray(a?.removedSystemIds)?a.removedSystemIds.length:0,modifiedSystemCount:a?.componentOverrides&&typeof a.componentOverrides=="object"?Object.keys(a.componentOverrides).length:0}}async function X(a,r){let p=ye({actionLabel:a,finalConsequence:r,...Ee()});return p?W(p):!0}function J(){P({addedSystems:[],removedSystemIds:[],componentOverrides:{}})}async function ue(){await X("Apply cluster changes","The cluster will regenerate from the current galactic inputs.")&&(de(),$(e),J(),T())}async function Me(){await X("Reset to defaults","All local-cluster inputs will reset to their default values.")&&(Object.assign(e,ee),ce(),$(e),J(),T())}t.querySelector("#btnClusterApply")?.addEventListener("click",ue),t.querySelector("#btnClusterReset")?.addEventListener("click",Me);let Oe=t.querySelector("#clusterImportText");t.querySelector("#btnClusterImport")?.addEventListener("click",()=>{let a=(Oe?.value||"").trim();if(!a)return;let r=Pe(a);if(r.length===0){alert("Could not parse any systems from the pasted text. Expected tab-separated columns: System Name, Coordinates (ly), Distance, Constituents.");return}let p=r.find(b=>b.isHome),y=r.filter(b=>!b.isHome),o=Math.max(...r.map(b=>b.distanceLy));o>e.neighbourhoodRadiusLy&&(e.neighbourhoodRadiusLy=Math.ceil(o+1),C.value=String(e.neighbourhoodRadiusLy),C.dispatchEvent(new Event("input",{bubbles:!0})),$(e));let S=te(e).systems.filter(b=>!b.isHome).map(b=>b.id);P({addedSystems:y,removedSystemIds:S,componentOverrides:{}});let h={};p&&(h.home=p.name);for(let b of y)h[b.id]=b.name;ne(h),T()}),t.querySelector("#btnRandomSeed")?.addEventListener("click",async()=>{await X("Randomise seed","The cluster will regenerate from a new random seed.")&&(x.value=String(1+Math.floor(Math.random()*2147483645)),de(),$(e),J(),T())}),_?.addEventListener("change",a=>{let r=a.target?.closest?.(".cluster-name-input");if(!r)return;let p=String(r.dataset.systemId||"").trim();if(!p)return;let y=Re(r.value);y?d[p]=y:delete d[p],ne(d),r.value=y}),_?.addEventListener("keydown",a=>{let r=a.target?.closest?.(".cluster-name-input");r&&a.key==="Enter"&&(a.preventDefault(),r.blur())}),K?.addEventListener("click",async a=>{let r=a.target?.closest?.(".cluster-adjust-btn");if(!r)return;let p=r.dataset.class,y=r.dataset.action;if(!p||!y)return;let o=F(),w=e.neighbourhoodRadiusLy>50?Math.max(.15,1-(e.neighbourhoodRadiusLy-50)/1e3):1;if(y==="add"){let S=qe(e.neighbourhoodRadiusLy,w),h="added-"+Date.now()+"-"+Math.floor(Math.random()*1e4);o.addedSystems.push({id:h,name:"Star System (added)",index:(l.length||0)+1,isHome:!1,objectClassKey:p,multiplicity:"single",components:[{objectClassKey:p}],metallicityFeH:0,...S})}else if(y==="remove"){let S=r.closest("tr")?.querySelector("td")?.textContent?.trim()||String(p).trim(),h=ze(o.addedSystems,j=>j.objectClassKey===p),b=h>=0?se({classLabel:S,sourceKind:"manual"}):se({classLabel:S,sourceKind:"generated"});if(!await W(b))return;if(h>=0)o.addedSystems.splice(h,1);else if(c){let j=new Set(o.removedSystemIds),D=c.systems.filter(z=>!z.isHome&&!j.has(z.id)&&z.objectClassKey===p);D.length>0&&o.removedSystemIds.push(D[D.length-1].id)}}P(o),T()});let R=t.querySelector("#clusterContextMenu"),Te=R?.querySelector(".cluster-context-menu__title"),ke=R?.querySelector(".cluster-context-menu__items");function Q(){R&&(R.style.display="none")}function Ne(a,r,p){let y=l.find(j=>j.id===p);if(!y)return;let o=y.components||[{objectClassKey:y.objectClassKey}],w=ie(y);Te.textContent=w+" system",xe(ke,{systemId:p,components:o,companionClasses:$e});let S=220,h=R.offsetHeight||200,b=window.innerWidth,O=window.innerHeight;R.style.left=Math.min(a,b-S-10)+"px",R.style.top=Math.min(r,O-h-10)+"px",R.style.display="block"}_?.addEventListener("contextmenu",a=>{let r=a.target?.closest?.("tr[data-system-id]");if(!r)return;a.preventDefault();let p=r.dataset.systemId;p&&Ne(a.clientX,a.clientY,p)}),R?.addEventListener("click",async a=>{let r=a.target?.closest?.(".cluster-context-menu__item");if(!r)return;let p=r.dataset.action,y=r.dataset.systemId;if(!p||!y)return;let o=F();if(p==="add-companion"){let w=r.dataset.class;if(!w)return;if(y.startsWith("added-")){let h=o.addedSystems.find(b=>b.id===y);h&&(h.components||[]).length<4&&(h.components.push({objectClassKey:w}),h.multiplicity=Y(h.components.length))}else{let h=o.componentOverrides[y],b=c?.systems.find(j=>j.id===y),O=h?h.components:(b?.components||[]).map(j=>({...j}));if(O.length<4){let j=[...O,{objectClassKey:w}];o.componentOverrides[y]={components:j,multiplicity:Y(j.length)}}}}else if(p==="remove-companion"){let w=Number(r.dataset.compIndex);if(!Number.isFinite(w)||w<1)return;let S=l.find(E=>E.id===y);if(!S)return;let h=Array.isArray(S?.components)?S.components:[{objectClassKey:S?.objectClassKey}],b=h[w],O=M(b?.objectClassKey),j=O==="LTY"?"L/T/Y":O,D=le(S,"Home")||ie(S);if(!await W(he({systemLabel:D,companionLabel:j,beforeComponentCount:h.length})))return;if(y.startsWith("added-")){let E=o.addedSystems.find(A=>A.id===y);E&&E.components.length>1&&(E.components.splice(w,1),E.multiplicity=Y(E.components.length))}else{let E=o.componentOverrides[y],A=c?.systems.find(G=>G.id===y),k=E?[...E.components]:(A?.components||[]).map(G=>({...G}));k.length>1&&w<k.length&&(k.splice(w,1),k.length===(A?.components||[]).length&&k.every((G,He)=>G.objectClassKey===A.components[He]?.objectClassKey)?delete o.componentOverrides[y]:o.componentOverrides[y]={components:k,multiplicity:Y(k.length)})}}P(o),Q(),T()}),document.addEventListener("click",a=>{R&&!R.contains(a.target)&&Q()}),document.addEventListener("keydown",a=>{a.key==="Escape"&&Q()}),[i,v,C,L,x].forEach(a=>{a?.addEventListener("keydown",r=>{r.key==="Enter"&&ue()})}),ce(),T()}function ze(s,n){for(let e=s.length-1;e>=0;e--)if(n(s[e]))return e;return-1}export{dt as initLocalClusterPage};
