import{a as ce,b as de,c as U}from"./chunk-XCN227BY.js";import{b as oe,d as ne,e as le}from"./chunk-KD5CBWLO.js";import{a as re}from"./chunk-RNZKX373.js";import{d as se}from"./chunk-7TT5SDUM.js";import{a as P}from"./chunk-WYZYYRUA.js";import{d as ie}from"./chunk-MTSXNHS6.js";import{b as B,c as j}from"./chunk-XMLMEZIZ.js";import"./chunk-XYLDORKF.js";import{p as V,s as te,t as ae}from"./chunk-XFZMQA63.js";import{j as w}from"./chunk-T2JC7ILW.js";import"./chunk-24KHYB4L.js";import"./chunk-VC46IEJQ.js";function K(i,o){return i&&(i.textContent=o==null?"":String(o),i)}function pe(i,{countLabel:o="32",countValue:p="",alphaValue:b=""}={}){return i&&(j(i,["N",B("sub",{text:o}),` = ${p} \u2014 \u03B1 = ${b}`]),i)}function J(i,o=[],p=""){if(!i)return i;let b=(o||[]).filter(Boolean);if(!b.length)return i.textContent=p==null?"":String(p),i;let l=[];return b.forEach((y,$)=>{$>0&&l.push(B("br")),l.push(B("strong",{text:y?.fraction||""}),` \u2014 ${y?.description||""}`)}),j(i,l),i}function t(i){return`<span class="sci-math sci-math--block">${i}</span>`}function e(i){return`<span class="sci-math">${i}</span>`}function r(i){return`<table class="sci-vars"><tbody>${i.map(([o,p])=>`<tr><td>${e(o)}</td><td>${p}</td></tr>`).join("")}</tbody></table>`}function s(i){return`<p class="sci-cite">${i}</p>`}function a(i,o){return`<div class="sci-formula"><h3 class="sci-formula__name">${i}</h3>${o}</div>`}function c(i,o){return`<table class="sci-data"><thead><tr>${i.map(p=>`<th>${p}</th>`).join("")}</tr></thead><tbody>${o.map(p=>`<tr>${p.map(b=>`<td>${b}</td>`).join("")}</tr>`).join("")}</tbody></table>`}function I(i){return String(i??"").trim().toLowerCase()}function fe(i,o){if(!o)return 0;let p=I(i.title);if(p===o)return 120;if(p.startsWith(o))return 90;if(p.includes(o))return 70;let b=I(i.sectionTitle);return b===o?52:b.includes(o)?40:I(i.text).includes(o)?18:0}function be(i){return Array.from(i.querySelectorAll(".sci-section")).flatMap(o=>{let p=String(o.id||"").trim(),b=o.querySelector(".sci-section__title")?.textContent?.trim()||"Science Section";return Array.from(o.querySelectorAll(".sci-formula")).map((l,y)=>{let $=`${p}-formula-${y+1}`;return l.id=$,l.setAttribute("tabindex","-1"),{id:$,sectionId:p,sectionTitle:b,title:l.querySelector(".sci-formula__name")?.textContent?.trim()||"Formula",text:l.textContent||""}})})}function ue(i,o,p=8){let b=I(o);return b?i.map(l=>({entry:l,score:fe(l,b)})).filter(l=>l.score>0).sort((l,y)=>y.score-l.score||l.entry.sectionTitle.localeCompare(y.entry.sectionTitle)||l.entry.title.localeCompare(y.entry.title)).slice(0,p).map(l=>l.entry):[]}function ge(){return[a("Mass-Luminosity Relation",`<div class="sci-formula__eq">${t("L = c \\cdot M^{\\alpha}")}</div>
      ...
      ${r([["R","Stellar radius (R&#9737;)"],["M","Stellar mass (M&#9737;)"],["L","Luminosity from MLR (L&#9737;)"],["T_{\\text{eff}}","Temperature from MTR (K)"],["\\text{norm}","1 / 0.992 (solar normalisation)"],["\\text{norm}_{\\text{SB}}","Continuity factor at 1.5 M&#9737;"]])}
      ...
      ${r([["T_{\\text{eff}}","Effective temperature (K)"],["M","Stellar mass (M&#9737;)"]])}
      ...
      ${r([["L","Luminosity (L&#9737;)"],["R","Radius (R&#9737;)"]])}`),a("Maximum Stellar Age",`<div class="sci-formula__eq">${t("\\tau_{\\max} = \\frac{M}{L} \\times 10 \\text{ Gyr}")}</div>
      ...
      ${s("Tanner Helland (2012) &mdash; tannerhelland.com")}`),a("Spectral Classification",`<p>Temperature mapped to spectral class via boundary lookup, then linear interpolation within the class gives subtype 0&ndash;9.9:</p>
      ${c(["Class","T range (K)","Span"],[["O","33,000 &ndash; 95,000","62,000"],["B","10,000 &ndash; 33,000","23,000"],["A","7,500 &ndash; 10,000","2,500"],["F","6,000 &ndash; 7,500","1,500"],["G","5,200 &ndash; 6,000","800"],["K","3,700 &ndash; 5,200","1,500"],["M","2,000 &ndash; 3,700","1,700"]])}`)].join("")}function ye(){return[a("Metallicity Conversion",`<div class="sci-formula__eq">${t("Z = Z_\\odot \\cdot 10^{[\\text{Fe/H}]}")}</div>
      <p>Converts spectroscopic iron abundance to total metal mass fraction, where ${e("Z_\\odot = 0.02")}.</p>
      ${r([["Z","Total metal mass fraction"],["[\\text{Fe/H}]","Iron abundance relative to Solar (dex), clamped to [&minus;3, +1]"]])}`),a("ZAMS Luminosity",`<div class="sci-formula__eq">${t("L_{\\text{ZAMS}} = \\frac{a_0 M^{5.5} + a_1 M^{11}}{a_2 + M^3 + a_3 M^5 + a_4 M^7 + a_5 M^8 + a_6 M^{9.5}}")}</div>
      ...
      ${r([["\\tau","Fractional age = age / t_MS (0 at ZAMS, 1 at TMS)"],["\\alpha_L,\\, \\alpha_R","Evolution rates (Hurley eqs. 19&ndash;20)"],["\\beta_L","Curvature term; mass-dependent (eq. 20)"],["\\eta","10 (M &le; 1), 20 (M &ge; 1.1), interpolated between"],["\\gamma","Chosen so L(1) = L_TMS exactly"]])}
      ...
      ${s("Hurley, Pols &amp; Tout (2000), MNRAS 315, 543")}`)].join("")}function _e(){return[a("Mass&ndash;Radius Relation (Radius-First)",`<div class="sci-formula__eq">${t("R_\\oplus = (1.07 - 0.21 \\cdot \\text{CMF}) \\cdot M_\\oplus^{\\,\\alpha}")}</div>
      ...
      ${r([["M_\\oplus","Planet mass (Earth masses)"],["\\text{CMF}","Core-mass fraction (0&ndash;1)"],["\\alpha","Mass-dependent exponent (approaches 1/3 at low mass)"],["R_\\oplus","Planet radius (Earth radii)"],["\\rho","Bulk density (g/cm&sup3;)"]])}
      <div class="sci-try">
        <div class="sci-try__title">Try it</div>
        <div class="sci-try__row">
          <label>Mass <span class="unit">M&#8853;</span></label>
      ...
      <p>Pressure-dependent boiling point of water. ${e("L_v/R_g = 40700/8.314 = 4894.4")} K. Calibrated: 1 atm &rarr; 373 K, 218 atm &rarr; 647 K (critical point).</p>`),a("Atmospheric Density (Ideal Gas Law)",`<div class="sci-formula__eq">${t("\\rho = \\frac{p \\cdot M_w}{R \\cdot T}")}</div>
      ${r([["p","Surface pressure (Pa) &mdash; 1 atm = 101,325 Pa"],["M_w","Mean molecular weight (kg/mol)"],["R","Gas constant (8.3145 J mol&supmin;&sup1; K&supmin;&sup1;)"],["T","Surface temperature (K)"]])}`),a("Mean Molecular Weight",`<div class="sci-formula__eq">${t("M_w = \\frac{\\sum_i f_i \\cdot m_i}{100} \\quad \\text{(kg/mol)}")}</div>
      ...
      ${c(["Rotation period","Cells"],[["&ge; 48 h","1 (single Hadley)"],["6 &ndash; 48 h","3 (Earth-like)"],["3 &ndash; 6 h","7 (rapid rotator)"],["&lt; 3 h","5 (very rapid)"]])}`)].join("")}function ve(){return[a("Mass&ndash;Radius Relation",`<p>Two-regime power law calibrated to Solar System giants:</p>
      ...
      ${s("Thorngren &amp; Fortney (2019), ApJL 874, L31")}`),a("Internal Heat Ratio",`<p>Ratio of total emitted flux to absorbed flux, interpolated by mass:</p>
      ${c(["Mass range","Ratio","Analogue"],[["&lt; 0.05 M<sub>J</sub>","1.06","Uranus"],["0.1&ndash;0.2 M<sub>J</sub>","2.6","Neptune"],["0.2&ndash;0.5 M<sub>J</sub>","2.6 &rarr; 1.67","Saturn &rarr; Jupiter"],["0.5&ndash;1.5 M<sub>J</sub>","1.67","Jupiter"]])}
      ...
      ${r([["\\rho","Bulk density (g/cm\\(^3\\)), proxy for dynamo-region density"],["q_{\\text{eff}}","max(internal flux + moon tidal flux, 0.4 W/m\\(^2\\)) &mdash; compositional convection floor"],["r_o/R","Dynamo shell outer boundary fraction (see below)"]])}
      ...
      ${r([["f","Geometric flattening (equatorial bulge)"],["q","Rotation parameter"],["\\xi","Normalised moment of inertia (Saturn 0.239, Jupiter 0.269)"],["J_2","(2f &minus; q) / 3"]])}
      ...
      ${r([["\\varepsilon","Heating efficiency = 0.15"],["F_\\odot","Solar XUV at 1 AU = 4.64 erg cm&sup2; s&sup1;"],["t_\\odot","Solar age = 4.6 Gyr"]])}
      ...
      ${s("Thorngren, Fortney, Murray-Clay &amp; Lopez (2016), ApJ 831, 64")}`),a("Radius Inflation",`<div class="sci-formula__eq">${t("R_{\\text{inflated}} = R \\cdot \\left(1 + 0.1\\,(5/t)^{0.35}\\right)")}</div>
      ...
      ${s("Fortney, Marley &amp; Barnes (2007), ApJ 659")}`),a("Ring Properties",`<p>Ring mass model with a Gaussian enhancement peaked at Saturn&rsquo;s mass:</p>
      ...
      ${r([["k_{\\min}","0.09 (heavily core-dominated sub-ice-giant)"],["k_{\\max}","0.385 (fluid-envelope-dominated gas giant)"],["M_{\\text{mid}}","0.072 M_J (core-to-envelope transition mass)"],["s","15 (transition steepness)"]])}
      ...
      ${s("Peale, Cassen &amp; Reynolds (1979); Wahl et al. (2016); Lainey et al. (2009, 2017); Fuller, Luan &amp; Quataert (2016)")}`)].join("")}function xe(){return[a("Kepler&rsquo;s Third Law (Moon Orbit)",`<div class="sci-formula__eq">${t("P = \\frac{2\\pi\\sqrt{a^3 / G(M_p + M_m)}}{86400} \\text{ days}")}</div>
      ${r([["a","Semi-major axis (metres)"],["G","6.674 &times; 10&supmin;&sup1;&sup1; N m&sup2; kg&supmin;&sup2;"],["M_p, M_m","Planet and moon masses (kg)"]])}`),a("Periapsis and Apoapsis",`<div class="sci-formula__eq">${t("r_p = a(1-e), \\quad r_a = a(1+e)")}</div>
      ...
      ${r([["\\mu","Rigidity (composition-dependent: 3.5&ndash;100 GPa)"],["\\rho","Mean density (kg/m&sup3;)"],["g","Surface gravity (m/s&sup2;)"],["R","Body radius (metres)"]])}
      ...
      ${s("Duchene &amp; Kraus (2013); tidal dissipation model")}`),a("Spin-Orbit Resonance",`<div class="sci-formula__eq">${t("H(p,\\,e)")}</div>
      ${r([["H(\\tfrac{3}{2},\\,e) = \\tfrac{7}{2}\\,e","3:2 resonance (e.g. Mercury)"],["H(2,\\,e) = \\tfrac{17}{2}\\,e^2","2:1 resonance"],["H(\\tfrac{5}{2},\\,e) = \\tfrac{845}{48}\\,e^3","5:2 resonance"]])}
      ...
      ${r([["P_s","Surface pressure (atm)"],["S = L/a^2","Insolation relative to Earth"],["g","Surface gravity (m/s&sup2;)"],["T_{\\text{eq}}","Equilibrium temperature (K), before greenhouse"],["C = 12","Calibration constant (Venus &rarr; b &gt; 1)"]])}
      ...
      ${r([["k_2","Love number of the moon (composition-dependent)"],["Q","Tidal quality factor of the moon"],["M_p","Parent body mass (kg)"],["R_m","Moon radius (metres)"],["n","Mean orbital motion = 2&pi;/P<sub>sid</sub> (rad/s)"],["e","Orbital eccentricity"],["a","Semi-major axis (metres)"],["f(e)","Wisdom (2008) eccentricity function (see below)"]])}
      ...
      ${s("Wisdom (2004); Peale, Cassen &amp; Reynolds (1979)")}`),a("Moon Tidal Heating on Planet",`<div class="sci-formula__eq">${t("\\dot{E}_{\\text{planet}} = \\frac{21}{2}\\,\\frac{k_{2p}}{Q_p} \\, \\frac{G \\, M_m^{\\,2} \\, R_p^{\\,5} \\, n}{a^6} \\, f(e)")}</div>
      ${r([["k_{2p}, Q_p","Love number and quality factor of the planet"],["M_m","Moon mass (kg) &mdash; the perturber"],["R_p","Planet radius (metres) &mdash; the heated body"],["n","Moon mean orbital motion = 2&pi;/P<sub>sid</sub> (rad/s)"],["a","Moon semi-major axis (metres)"],["f(e)","Wisdom (2008) eccentricity function (same as moon heating)"]])}
      ...
      ${r([["\\Omega_p","Planet spin angular velocity (rad/s)"],["n","Moon mean orbital motion (rad/s)"],["k_{2p}, Q_p","Love number and quality factor of the planet"],["k_{2m}, Q_m","Love number and quality factor of the moon"],["m_p, m_m","Planet and moon masses (kg)"],["R_p, R_m","Planet and moon radii (metres)"]])}
      ...
      ${r([["F_0","Initial tidal flux from cold material properties (W/m\xB2)"],["F_{\\text{crit}}","Critical flux for partial melting (0.02 W/m\xB2)"],["\\mu_{\\text{melt}}","10 GPa (partially molten rigidity)"],["Q_{\\text{melt}}","10 (partially molten quality factor)"]])}
      ...
      ${r([["L_\\star","Star luminosity (W)"],["a","Moon Bond albedo"],["d","Star\u2013planet distance (metres)"],["\\sigma","Stefan-Boltzmann constant (5.67\xD710\u207B\u2078 W m\u207B\xB2 K\u207B\u2074)"],["F_{\\text{tidal}}","Tidal heating surface flux (W/m\xB2)"],["A","Radioisotope abundance (\xD7 Earth)"],["M_m, R_m","Moon mass (kg) and radius (metres)"]])}
      ...
      ${r([["B_{\\text{surf}}","Planet surface field (Gauss)"],["R_p","Planet radius"],["r","Moon semi-major axis"]])}
      ...
      ${s("Paranicas et al. (2009); Divine & Garrett (1983) \u2014 Jupiter radiation environment")}`),a("Volatile Inventory &amp; Atmospheric Retention",`<p>Identifies surface ices and thin atmospheres on airless moons via three checks per species:</p>
      ...
      ${r([["m_s","Molecular mass of species (kg)"],["v_{\\text{esc}}","Moon surface escape velocity (m/s)"],["k_B","Boltzmann constant"],["T","Surface temperature (K)"]])}
      ...
      ${r([["P","Surface vapor pressure (Pa)"],["g","Moon surface gravity (m/s&sup2;)"]])}
      ...
      ${s("Fray & Schmitt (2009, PSS 57, 2053); Jeans (1916) \u2014 atmospheric escape theory; Chamberlain (1963) \u2014 hydrodynamic escape timescale")}`),a("Moon Worlds in WorldSmith",`<p>WorldSmith&rsquo;s moon-world outputs are built as a layered model rather than a single published formula. The Moon page combines atmosphere, hydrosphere, climate, geology, biosphere, and habitability outputs into one reference state for each moon.</p>
      ...
      <p><b>Reference inputs:</b> this moon-world layer sits on top of the tidal heating, moon temperature, magnetospheric radiation, volatile-retention, magnetosphere, and atmosphere-stability blocks documented above, then adds WorldSmith&rsquo;s own hydrosphere, geology, biosphere, and integrated habitability policy layers.</p>`)].join("")}function $e(){return[a("Hill Sphere",`<div class="sci-formula__eq">${t("r_H = a \\left(\\frac{m}{3M_\\star}\\right)^{1/3}")}</div>
      ...
      are shown as dimmed amber diamonds.</p>`)].join("")}function Me(){return[a("Stellar Absolute Magnitude",`<div class="sci-formula__eq">${t("M_V = 4.81 - 2.5\\log_{10}(L)")}</div>
      ...
      ${r([["\\alpha","Phase angle (radians)"],["G","Slope parameter (0.28 rocky, 0.15 tiny)"]])}
      ...
      ${r([["r","Body&rsquo;s orbital distance (AU)"],["\\Delta","Body-to-observer distance (AU)"],["d_h","Observer&rsquo;s orbital distance (AU)"]])}`),a("Elongation",`<div class="sci-formula__eq">${t("\\cos(\\text{elong}) = \\frac{d_h^2 + \\Delta^2 - r^2}{2\\,d_h\\,\\Delta}")}</div>
      ...
      <p>If ${e("\\theta_{\\text{moon}} \\ge \\theta_\\star")}: total eclipses. Otherwise: annular only.</p>`)].join("")}function Se(){return[a("Scale Height Correction",`<div class="sci-formula__eq">${t("\\frac{H_{\\text{planet}}}{H_\\oplus} = \\frac{T / T_\\oplus}{g / g_\\oplus}")}</div>
      ...
      ${r([["P","Total surface pressure (atm)"],["p_X","Partial pressure of gas X = P &times; X% / 100 (atm)"],["p_{\\text{ref}}","Reference partial pressure = 0.001 atm (linear&ndash;logarithmic transition scale)"],["\\text{pb}","Pressure-broadening factor (Robinson &amp; Catling 2012)"],["\\omega","CO&#8322;&ndash;H&#8322;O band overlap factor (see next formula)"]])}
      ...
      ${r([["\\omega","Overlap suppression factor (0 to 1)"],["\\tau_{\\text{CO}_2}","CO&#8322; optical depth (computed above)"],["k","Half-saturation constant = 6"]])}
      ...
      ${r([["f_{\\text{H}_2},\\; f_{\\text{N}_2}","Volume fractions (0&ndash;1) of H&#8322; and N&#8322;"],["P","Total surface pressure (atm)"],["\\text{pb}","Pressure-broadening factor = P<sup>0.684</sup>"],["\\tau_{\\text{core}}","&tau;<sub>CO&#8322;</sub> + &tau;<sub>H&#8322;O</sub> + &tau;<sub>CH&#8324;</sub> (computed from core gases)"]])}
      ...
      ${r([["v_{\\text{esc}}","Surface escape velocity (m/s)"],["M","Molar mass of gas species (kg/mol)"],["R","Universal gas constant = 8.3145 J/(mol&middot;K)"],["T_{\\text{exo}}","Exobase temperature (K)"]])}
      ...
      ${r([["T_{\\text{eq}}","Equilibrium temperature without greenhouse (K)"],["\\eta_{\\text{abs}}","XUV absorption efficiency (Beer-Lambert saturation)"],["P_{1/2}","Half-absorption pressure = 0.06 atm"],["F_{\\text{XUV}}","XUV flux at the planet's orbit (erg cm&#8315;&#178; s&#8315;&#185;)"],["F_0","Present-day solar XUV at 1 AU = 4.64 erg cm&#8315;&#178; s&#8315;&#185;"],["P","Surface pressure (atm)"],["f_{\\text{CO}_2}","CO&#8322; volume fraction (0&ndash;1)"]])}
      ...
      ${r([["F_0","Present-day solar XUV at 1 AU = 4.64 erg cm&#8315;&#178; s&#8315;&#185;"],["L_\\star","Stellar luminosity (L&#9737;)"],["t","Stellar age (Gyr)"],["d","Orbital distance (AU)"]])}
      ...
      ${s("Gunell et al. (2018), A&amp;A 614, L3 &mdash; Why an intrinsic magnetic field does not protect a planet against atmospheric escape. Gronoff et al. (2020), JGR Space Physics 125 &mdash; Atmospheric Escape Processes and Planetary Atmospheric Evolution.")}`)].join("")}function we(){return[a("Temperature at Latitude",`<div class="sci-formula__eq">${t("T(\\phi) = T_{\\text{eq}} - G\\,\\sin^2\\phi")}</div>
      <div class="sci-formula__eq">${t("T_{\\text{eq}} = T_{\\text{global}} + G/3")}</div>
      ${r([["\\phi","Latitude (radians)"],["T_{\\text{eq}}","Equatorial mean temperature"],["G","Equator-to-pole temperature gradient (K)"]])}`),a("Equator&ndash;Pole Gradient",`<div class="sci-formula__eq">${t("G = \\frac{60}{1 + 0.8\\,P/\\sqrt{g}}")}</div>
      ...
      elevated terrain. Used for all climate zone calculations.</p>`)].join("")}function qe(){return[a("Power-Law Flare Rate",`<div class="sci-formula__eq">${t("N(> E) = N_{32} \\left(\\frac{E}{10^{32} \\text{ erg}}\\right)^{-\\alpha}")}</div>
      ...
      ${s("Yashiro et al. (2006) flare-CME association rates; probabilities WorldSmith-calibrated")}`),a("CME Rate from Activity Cycle",`<div class="sci-formula__eq">${t("\\text{rate} = 0.5 + 5.5t \\text{ CME/day}")}</div>
      ...
      ${s("G\xFCnther et al. (2020) TESS superflare rates; binning WorldSmith")}`),a("Flare Cycle Multiplier",`<p>Flare rate is modulated by an 11-year-analogue activity cycle. At cycle phase ${e("\\phi \\in [0,1]")}:</p>
      ...
      observations that M-dwarf activity varies less over magnetic cycles.</p>`)].join("")}function Te(){return[a("Local Day Scale",`<div class="sci-formula__eq">${t("\\text{scale} = \\frac{P_{\\text{rot}}}{24}")}</div>
      ...
      <p>Day-of-week index for year ${e("y")}, where ${e("L_i")} is the length of year ${e("i")} and ${e("D_w")} is days per week. Computed in O(rules) time by counting leap rule firings.</p>`)].join("")}function Ce(){return[a("Neighbourhood Volume",`<div class="sci-formula__eq">${t("V = \\frac{4}{3}\\pi R^3")}</div>
      ...
      ${r([["r","Distance from galactic centre (ly)"],["R","Galaxy radius (ly)"]])}
      ...
      ${r([["g_R","Radial gradient: &minus;0.06 dex/kpc (Luck &amp; Lambert 2011)"],["g_z","Vertical gradient: &minus;0.30 dex/kpc (Schlesinger et al. 2014)"],["\\delta_{\\text{class}}","Spectral-class shift (e.g. white dwarfs &minus;0.15 dex)"],["\\sigma","Scatter &sigma; = 0.20 dex"]])}
      ...
      <p>Galactic disk flattening for large neighbourhood radii. At 50 ly the neighbourhood is spherical; at 500 ly z &asymp; 0.55; floors at 0.15.</p>`)].join("")}function ke(){return[a("Ocean Fraction by Water Regime",`${c(["Water regime","Ocean fraction"],[["Dry","0%"],["Shallow oceans","50%"],["Extensive oceans","71%"],["Global ocean","90%"],["Deep ocean","95%"],["Ice world","0%"]])}
      ...
      ${r([["A","Habitable land area (km&sup2;)"],["d","Tech-era population density (people/km&sup2;)"],["C_e","Crop efficiency = 4&times; (crops feed more than livestock)"],["f_c","Crop fraction; 0.77 = Earth reference"]])}
      ...
      ${r([["K","Carrying capacity"],["P_0","Initial population"],["r","Growth rate (per year)"],["t","Time (years)"]])}
      ...
      ${s("Zipf (1949), Human Behavior and the Principle of Least Effort")}`)].join("")}function Re(){return[a("Water-Aware Radius (Zeng & Sasselov 2016)",`<div class="sci-formula__eq">${t("R = R_{\\text{dry}} \\cdot \\left(1 + \\left(\\frac{R_{50}}{R_{\\text{ref}}} - 1\\right) \\cdot \\min\\!\\left(\\frac{\\text{WMF}}{0.5},\\;1\\right)\\right)")}</div>
      ...
      ${r([["R_{\\text{dry}}","Radius from the CMF-based density formula (unchanged)"],["R_{50} = 1.38\\,M^{0.263}","Zeng 50%-water mass-radius curve"],["R_{\\text{ref}} = 1.00\\,M^{0.270}","Zeng Earth-like dry mass-radius curve"],["\\text{WMF}","Water mass fraction (0&ndash;0.5)"]])}
      ...
      ${r([["\\text{CRF}","Core radius / total planetary radius"],["\\text{CMF}","Core mass fraction (0&ndash;1)"]])}
      ${s("Zeng, L. &amp; Jacobsen, S. (2017)")}`),a("Magnetic Dipole Scaling (Olson & Christensen 2006)",`<div class="sci-formula__eq">${t("B_{\\text{surf}} = \\frac{\\sqrt{\\rho_c} \\cdot \\text{CRF}^3 \\cdot M^{1/3} \\cdot C_b}{B_{\\oplus}}")}</div>
      ...
      ${r([["\\rho_c = \\text{CMF} \\cdot \\rho / \\text{CRF}^3","Core density from mass conservation (two-layer model)"],["\\text{CRF} = \\sqrt{\\text{CMF}}","Core radius fraction (Zeng &amp; Jacobsen 2017)"],["M^{1/3}","Heat-flux proxy from planet mass"],["C_b","Three-phase convective boost (see below)"],["B_{\\oplus}","Earth reference: same formula with CMF=0.33, &rho;=5.514, M=1"]])}
      ...
      ${s("Olson, P. &amp; Christensen, U. (2006), EPSL 250, 561; Christensen, U. &amp; Aubert, J. (2006), GJI 166")}`),a("Core Solidification Timescale",`<div class="sci-formula__eq">${t("\\tau = (2 + 12 \\cdot \\text{CMF} \\cdot \\sqrt{M_\\oplus}) \\;\\times\\; A \\quad \\text{(Gyr)}")}</div>
      ...
      ${r([["\\tau","Core solidification timescale (Gyr)"],["\\text{CMF}","Core mass fraction (0&ndash;1)"],["M_\\oplus","Planet mass (Earth masses)"],["A","Radioisotope abundance (1.0 = Earth; see below)"]])}`),a("Radioisotope Abundance",`<div class="sci-formula__eq">${t("A = \\sum_i a_i \\cdot w_i")}</div>
      ...
      ${r([["[\\text{Fe/H}]","Stellar iron-to-hydrogen ratio (dex)"],["55.85","Molar mass of iron (g/mol)"],["172","Effective molar mass of silicate mantle"]])}
      ...
      ${s("Ortenzi et al. (2020), Sci. Rep. 10, 10907")}`)].join("")}function Pe(){return[a("Maximum Mountain Height",`<div class="sci-formula__eq">${t("H_{\\max} = \\frac{\\sigma_y}{\\rho \\cdot g} \\approx \\frac{9{,}267}{g} \\text{ m}")}</div>
      ...
      ${r([["\\sigma_y","Compressive yield strength of silicate rock (~100 MPa)"],["\\rho","Crustal rock density (~2,800 kg/m&sup3;)"],["g","Surface gravity (relative to Earth, where g = 9.81 m/s&sup2;)"]])}
      ...
      ${r([["d_r","Mid-ocean ridge depth (default 2,600 m below sea level, GDH1)"],["t","Crustal age (Myr)"],["350","Subsidence rate coefficient (m/Myr&frac12;)"],["6{,}400","Asymptotic ocean depth (m)"],["3{,}073","Intersection amplitude (WS-modified from PSM&rsquo;s 3,200 m)"],["62.8","Thermal time constant (Myr)"]])}
      ...
      ${r([["h","Mountain elevation above datum (m)"],["\\rho_c","Crustal density (2,800 kg/m&sup3;)"],["\\rho_m","Mantle density (3,300 kg/m&sup3;)"]])}
      ...
      ${r([["h","Elevation above datum (m)"],["D","Compensation depth (default 100 km)"],["\\rho_0","Base crustal density (2,800 kg/m&sup3;)"]])}
      ...
      ${r([["h_{\\text{slab}}","Depth to slab top beneath volcanic front (110 km global mean)"],["\\theta","Subduction angle (10&ndash;90&deg;)"]])}
      ...
      ${r([["H_0","Initial mountain height (m)"],["\\varepsilon","Erosion rate (default 5 m/Myr)"],["t","Elapsed time (Myr)"]])}
      ...
      ${r([["10{,}000","Earth reference shield height (m), Mauna Kea base-to-peak"],["g","Surface gravity (Earth = 1)"],["f_{\\text{lid}}","Stagnant-lid factor: 1.5 if stagnant lid, 1.0 otherwise"]])}
      ...
      ${r([["f_M(i)","Mass factor &mdash; log-Gaussian centred on optimal mass for regime i"],["f_t(i)","Age factor &mdash; Gaussian in t centred on optimal age for regime i"],["f_W(i)","Water factor &mdash; WMF-dependent multiplier (Korenaga 2010)"],["f_C(i)","CMF factor &mdash; penalises mobile lid for high core fraction"],["f_T(i)","Tidal heating factor &mdash; reduces stagnant lid probability"]])}
    ...
    </div>`].join("")}function Ae(){return[a("Mean-Motion Resonance Positions",`<div class="sci-formula__eq">${t("a_{\\text{res}} = a_p\\,(p/q)^{2/3}")}</div>
      ...
      ${r([["r_{\\text{H,OC}}","Characteristic Hill / tidal size of the exo-Oort reservoir"],["M_\\star","Total mass of the host stellar system"],["R_{\\text{gal}}","Galactocentric distance of the system"],["R_{\\text{gal},\\odot}","Solar-circle reference radius used by WorldSmith's Local Cluster defaults"]])}
      ...
      ${r([["a_{\\text{in}}","Hill-scaled inner edge before architecture adjustment"],["a_{\\text{out}}","Hill-scaled outer edge"],["a_{\\text{giant,max}}","Semi-major axis of the outermost giant planet in the system"]])}
      ...
      ${r([["F_{\\text{giants}}","Total giant-mass factor relative to the Solar giant planets"],["F_{\\text{outer}}","Outermost-giant extent factor relative to Neptune's orbit"],["F_{\\text{eject}}","Penalty for massive inner ejectors that deplete the cloud"],["F_{\\text{age}}","Age saturation term for cloud emplacement"],["F_{\\text{ret}}","Retention factor from galactocentric location and stellar density"],["\\Gamma_{\\text{LPC}}","New long-period comet injection rate"]])}
      ...
      ${s("Automatic baseline science: see the Oort-cloud entries above. Guided and manual controls are WorldSmith authoring overlays resolved in engine/oortCloud.js.")}`),a("IR Excess at 24 &mu;m",`<div class="sci-formula__eq">${t("\\text{excess} = f \\cdot \\frac{B_\\nu(T_{\\text{disk}},\\,24\\,\\mu\\text{m})}{B_\\nu(T_\\star,\\,24\\,\\mu\\text{m})}")}</div>
      <p>Ratio of disk to stellar flux at 24 &mu;m via Planck functions.
      Detectable if &gt; 0.1 (easily), marginal 0.01&ndash;0.1, undetectable &lt; 0.01.</p>`)].join("")}function Ee(){let i=(o,p)=>`<div class="sci-formula"><h3 class="sci-formula__name">${o}</h3>${p}</div>`;return[`<p class="sci-diverge-intro">WorldSmith aims to reproduce published astrophysical models wherever possible.
    ...
    to WorldSmith; items marked <b>Simplified</b> are reductions of published work.</p>`,i("Oort Cloud Mass And LPC Flux Proxy (WS-derived / Simplified)",`<p>WorldSmith&rsquo;s Oort-cloud mass and long-period-comet flux are not taken from a single published closed-form model. The edge scaling follows exo-Oort Hill-radius work, but the mass and LPC-rate outputs are WorldSmith proxies built from five factors: total giant mass, outer-giant extent, inner-ejector depletion, age saturation, and environmental retention.</p>
      ...
      ${s("Coefficients: WorldSmith calibration. Functional forms: Myhre (1998), IPCC TAR (2001), Robinson &amp; Catling (2012).")}`),i("CO&#8322;&ndash;H&#8322;O Band Overlap Suppression (WS-derived)",`<p>The half-saturation model ${e("\\omega = 1/(1 + \\tau_{\\text{CO}_2}/6)")} and the constant
      ...
      ${s("H&#8322;&ndash;N&#8322; CIA: Wordsworth &amp; Pierrehumbert (2013), Science 339. SO&#8322;/NH&#8323; overlap: WorldSmith calibration.")}`),i("Surface Temperature Divisor (WS-derived)",`<p>The factor ${e("\\text{surfDiv} = 1 - 0.1 \\cdot \\min(\\tau, 1)")} that ramps from
      ...
      ${s("CMF scaling: Zeng &amp; Sasselov (2016). Exponent: WorldSmith fit to Solar System data.")}`),i("Core Solidification Timescale (Simplified)",`<p>The formula ${e("\\tau = 2 + 12 \\cdot \\text{CMF} \\cdot \\sqrt{M_\\oplus}")} is a WorldSmith
      ...
      ${s("Zeng, L. &amp; Jacobsen, S. (2017). Approximation: WorldSmith simplification.")}`),i("Water-Radius Inflation: Linear Interpolation (Simplified)",`<p>WorldSmith linearly interpolates between the Zeng dry curve (${e("R = 1.00\\,M^{0.270}")})
      ...
      ${s("Qualitative basis: Valencia et al. (2007), O'Neill &amp; Lenardic (2007), Noack &amp; Breuer (2014), Korenaga (2010). Quantitative model: WorldSmith.")}`),i("Habitable Zone: Chromant Desmos Correction (Modified)",`<p>The Seff polynomials and the temperature proxy ${e("T_{\\text{eff}} = 5778 \\cdot M^{0.55}")}
      ...
      ${s("Schulze, J. et al. (2021), PSJ 2, 113. Simplification: WorldSmith.")}`),i("Vegetation Colour Extrapolation (Simplified)",`<p>PanoptesV provides pre-computed vegetation colours at 1, 3, and 10 atm for spectral
      ...
      ${s("LUT data: PanoptesV (panoptesv.com/SciFi). Extrapolation: WorldSmith.")}`),i("Spin-Orbit Resonance Capture Thresholds (Simplified)",`<p>The thresholds for resonance capture (H &gt; 0.25 for 3:2, H &gt; 0.5 for 2:1 and 5:2)
      ...
      ${s("Goldreich, P. &amp; Peale, S. (1966), AJ 71, 425. Thresholds: WorldSmith simplification.")}`),i("Flare Rate Binning (Simplified)",`<p>The N&#8323;&#8322; values (flares per day above 10&sup3;&sup2; erg) are discretised into three spectral
      ...
      ${s("Parsons &amp; Sclater (1977), JGR 82, 803. Adjustment: WorldSmith calibration.")}`),i("Shield Volcano 1/g Scaling (Simplified)",`<p>The shield volcano height formula ${e("H = 10{,}000/g \\times f_{\\text{lid}}")} uses a
      ...
      patterns that depend on composition, pressure, and stellar spectrum.</p>`)].join("")}function Fe(){return[a("Titius-Bode Orbit Spacing",`<div class="sci-formula__eq">${t("a_n = \\begin{cases} a_1 & n = 1 \\\\ a_1 + s \\cdot 2^{n-2} & n \\ge 2 \\end{cases}")}</div>
      ...
      ${r([["a_{\\text{c,S}}","outer stability edge for circumstellar S-type planets"],["a_{\\text{c,P}}","inner stability edge for circumbinary P-type planets"],["a_{\\text{bin}}","binary semi-major axis"],["e","binary eccentricity"],["\\mu","companion mass fraction = M_2 / (M_1 + M_2)"]])}
      ...
      ${s("WorldSmith hierarchical host-frame flux model in engine/homeSystem/flux.js")}`),a("Hierarchical Guardrail for Triples and Quads",`<p>For nested triples and quads, WorldSmith uses a Mardling-Aarseth style separation floor so outer branches do not crowd inner pairs:</p>
      ...
      ${r([["a_{\\text{in}}","inner pair semi-major axis"],["a_{\\text{out,min}}","recommended minimum outer semi-major axis"],["q_{\\text{out}}","outer companion-to-inner-system mass ratio"],["e_{\\text{out}}","outer eccentricity"],["f_i","inclination factor used by WorldSmith (0.7&ndash;1.0)"]])}
      ...
      ${s("Mardling &amp; Aarseth (2001), MNRAS 321, 398; WorldSmith hierarchy guardrails in engine/homeSystem/stability.js")}`)].join("")}function He(i,o){let p=o/100,b=Math.log(Math.max(i,1e-6)),l=Math.min(1/3,.257-.0161*b),y=(1.07-.21*p)*Math.pow(i,l),$=i*5.51/Math.pow(y,3);return{R:y,rho:$}}function Le(i){let o=i.querySelector("#sci-mlr-mass"),p=i.querySelector("#sci-mlr-slider"),b=i.querySelector("#sci-mlr-result");if(o&&p&&b){let f=()=>{let d=V(Number(o.value));b.textContent=`${w(d,4)} L\u2609`};P({numberEl:o,sliderEl:p,min:.075,max:100,step:.01,mode:"log"}),o.addEventListener("input",f),p.addEventListener("input",()=>{o.dispatchEvent(new Event("input"))}),f()}let l=i.querySelector("#sci-hz-mass"),y=i.querySelector("#sci-hz-slider"),$=i.querySelector("#sci-hz-result");if(l&&y&&$){let f=()=>{let d=Number(l.value),v=V(d),S=te(d),L=ae({luminosityLsol:v,teffK:S});K($,`${w(L.innerAu,2)} \u2013 ${w(L.outerAu,2)} AU`)};P({numberEl:l,sliderEl:y,min:.075,max:100,step:.01,mode:"log"}),l.addEventListener("input",f),y.addEventListener("input",()=>{l.dispatchEvent(new Event("input"))}),f()}let q=i.querySelector("#sci-dens-mass"),R=i.querySelector("#sci-dens-mass-slider"),M=i.querySelector("#sci-dens-cmf"),A=i.querySelector("#sci-dens-cmf-slider"),E=i.querySelector("#sci-dens-result");if(q&&R&&M&&A&&E){let f=()=>{let{R:d,rho:v}=He(Number(q.value),Number(M.value));K(E,`${w(d,3)} R\u2295 \u2014 ${w(v,2)} g/cm\xB3`)};P({numberEl:q,sliderEl:R,min:.01,max:100,step:.01,mode:"log"}),P({numberEl:M,sliderEl:A,min:0,max:100,step:1,mode:"linear"}),[q,R,M,A].forEach(d=>d.addEventListener("input",f)),f()}let C=i.querySelector("#sci-hmag-rad"),F=i.querySelector("#sci-hmag-alb"),H=i.querySelector("#sci-hmag-alb-slider"),D=i.querySelector("#sci-hmag-result");if(C&&F&&D){let f=()=>{let d=Number(C.value),v=Number(F.value);if(d>0&&v>0){let S=se({radiusKm:d,geometricAlbedo:v});D.textContent=`H = ${w(S,2)}`}};H&&P({numberEl:F,sliderEl:H,min:.01,max:1,step:.01,mode:"linear"}),[C,F].forEach(d=>d.addEventListener("input",f)),H&&H.addEventListener("input",f),f()}let W=i.querySelector("#sci-flare-temp"),O=i.querySelector("#sci-flare-temp-slider"),G=i.querySelector("#sci-flare-age"),n=i.querySelector("#sci-flare-age-slider"),u=i.querySelector("#sci-flare-result");if(W&&G&&u){let f=()=>{let d=ie({teffK:Number(W.value),ageGyr:Number(G.value)});pe(u,{countLabel:"32",countValue:d.N32,alphaValue:d.alpha})};O&&P({numberEl:W,sliderEl:O,min:2e3,max:1e4,step:100,mode:"linear"}),n&&P({numberEl:G,sliderEl:n,min:.01,max:13,step:.1,mode:"linear"}),[W,O,G,n].filter(Boolean).forEach(d=>d.addEventListener("input",f)),f()}let g=i.querySelector("#sci-leap-len"),_=i.querySelector("#sci-leap-result");if(g&&_){let f=v=>{if(v===0)return{p:0,q:1};let S=0,L=1;for(let k=1;k<=1e4;k++){let x=Math.round(v*k);if(Math.abs(x/k-v)<1e-9){S=x,L=k;break}}return{p:S,q:L}},d=()=>{let v=Number(g.value),S=v-Math.floor(v);if(S<1e-9){J(_,[],"No leap cycle needed (integer year)");return}let k=re(S,6).slice(1).map(f).filter(x=>x.q>0);if(k.length===0){J(_,[],"No usable cycle found");return}J(_,k.map(x=>{let Y=Math.abs(x.p/x.q-S),ee=Y>1e-12?Math.round(1/Y):1/0,he=ee===1/0?"exact":`~1 day drift per ${w(ee,0)} yr`;return{fraction:`${x.p}/${x.q}`,description:`${x.p} leap day${x.p!==1?"s":""} every ${x.q} years (${he})`}}))};g.addEventListener("input",d),d()}let T=i.querySelector("#sci-ghz-r"),m=i.querySelector("#sci-ghz-loc"),h=i.querySelector("#sci-ghz-result");if(T&&m&&h){let f=()=>{let d=Number(T.value),v=Number(m.value),S=.53*d,L=.1*d,k=Math.exp(-.5*Math.pow((v-S)/L,2)),x=v>=.47*d&&v<=.6*d;K(h,`${w(k*100,1)}%${x?" (in hard band)":""}`)};[T,m].forEach(d=>d.addEventListener("input",f)),f()}let N=i.querySelector("#sci-tec-grav"),z=i.querySelector("#sci-tec-grav-slider"),Z=i.querySelector("#sci-tec-mtn"),Q=i.querySelector("#sci-tec-shield"),X=i.querySelector("#sci-tec-root");if(N&&z&&Z&&Q&&X){let f=()=>{let d=Number(N.value);d>0&&(Z.textContent=`${w(oe(d),0)} m`,Q.textContent=`${w(le(d),0)} m`,X.textContent=`${w(ne(5e3),0)} m`)};P({numberEl:N,sliderEl:z,min:.05,max:5,step:.01,mode:"log"}),N.addEventListener("input",f),z.addEventListener("input",()=>{N.dispatchEvent(new Event("input"))}),f()}}var me=[{id:"stellar",title:"Stellar Physics",count:8,builder:ge},{id:"evolution",title:"Stellar Evolution",count:7,builder:ye},{id:"planetary",title:"Planetary Physics",count:11,builder:_e},{id:"gasgiant",title:"Gas Giant Physics",count:13,builder:ve},{id:"interior",title:"Interior &amp; Composition",count:7,builder:Re},{id:"tectonics",title:"Tectonics &amp; Geodynamics",count:14,builder:Pe},{id:"orbital",title:"Orbital Mechanics",count:13,builder:xe},{id:"lagrange",title:"Lagrange Points",count:4,builder:$e},{id:"photometry",title:"Photometry &amp; Magnitudes",count:8,builder:Me},{id:"atmosphere",title:"Atmosphere &amp; Colour",count:9,builder:Se},{id:"climate",title:"Climate Classification",count:7,builder:we},{id:"activity",title:"Stellar Activity",count:7,builder:qe},{id:"calendar",title:"Calendar Systems",count:5,builder:Te},{id:"cluster",title:"Local Cluster",count:7,builder:Ce},{id:"population",title:"Population Dynamics",count:5,builder:ke},{id:"system",title:"System Architecture",count:7,builder:Fe},{id:"debris",title:"Debris Disks",count:12,builder:Ae},{id:"divergences",title:"Divergences from Published Science",count:36,builder:Ee}];function Ue(i){let o=document.createElement("div");o.className="page";let p=me.map(n=>`<a class="sci-toc__link" data-target="sci-${n.id}" href="#sci-${n.id}">${n.title}</a>`).join(""),b=me.map((n,u)=>`<details class="sci-section" id="sci-${n.id}"${u===0?" open":""}>
        <summary class="sci-section__summary">
          <span class="sci-section__title">${n.title}</span>
          <span class="sci-section__count">${n.count} equations</span>
        </summary>
        <div class="sci-section__body">${n.builder()}</div>
    ...
    <div class="panel">
      ...
      <div class="panel__body">
        ...
        <div class="sci-toc">${p}</div>
      </div>
    </div>
    <div class="sci-sections">${b}</div>
  `,i.innerHTML="",i.appendChild(o);let l=Array.from(o.querySelectorAll(".sci-section")),y=Array.from(o.querySelectorAll(".sci-formula")),$=Array.from(o.querySelectorAll(".sci-toc__link")),q=o.querySelector("#sciencePageSearch"),R=o.querySelector("#sciencePageSearchStatus"),M=o.querySelector("#sciencePageSearchResults"),A=be(o),E=null,C=!1,F=l.find(n=>n.open)?.id||l[0]?.id||null,H=(n,{exclusive:u=!C}={})=>{n&&(u?l.forEach(g=>{g.open=g===n}):n.open=!0,!C&&n.id&&(F=n.id))},D=()=>{E&&(clearTimeout(E),E=null),o.querySelectorAll(".sci-formula.is-search-hit").forEach(n=>{n.classList.remove("is-search-hit")})},W=n=>{if(!n)return;let u=o.querySelector(`#${n}`);if(!u)return;let g=u.closest(".sci-section");H(g),D(),u.classList.add("is-search-hit"),U(u,{block:"center"}),u.focus?.({preventScroll:!0}),E=window.setTimeout(()=>{u.classList.remove("is-search-hit"),E=null},2200)},O=n=>{let u=I(n);if(D(),!u){C=!1,o.classList.remove("is-search-filtering"),y.forEach(h=>{h.hidden=!1,h.classList.remove("is-search-match","is-search-filtered-out")}),l.forEach(h=>{h.hidden=!1,h.classList.remove("is-search-match","is-search-filtered-out")}),$.forEach(h=>{h.hidden=!1,h.classList.remove("is-search-filtered-out")});let m=l.find(h=>h.id===F)||l[0]||null;return H(m,{exclusive:!0}),[]}let g=ue(A,u,A.length),_=new Set(g.map(m=>m.id)),T=new Set(g.map(m=>m.sectionId));return C=!0,o.classList.add("is-search-filtering"),y.forEach(m=>{let h=_.has(m.id);m.hidden=!h,m.classList.toggle("is-search-match",h),m.classList.toggle("is-search-filtered-out",!h)}),l.forEach(m=>{let h=T.has(m.id);m.hidden=!h,m.open=h,m.classList.toggle("is-search-match",h),m.classList.toggle("is-search-filtered-out",!h)}),$.forEach(m=>{let h=T.has(m.dataset.target||"");m.hidden=!h,m.classList.toggle("is-search-filtered-out",!h)}),g},G=(n,u=null)=>{if(!M||!R)return[];let g=I(n);if(!g)return M.hidden=!0,M.innerHTML="",R.hidden=!0,R.textContent="",[];let _=u||ue(A,g,A.length),T=_.slice(0,8);return R.hidden=!1,R.textContent=_.length>0?`${_.length} match${_.length===1?"":"es"}${_.length>T.length?`, showing top ${T.length}`:""}`:"No matches found",_.length===0?(M.hidden=!1,M.innerHTML='<div class="sci-search__empty">No formulas or topics matched that search.</div>',[]):(M.hidden=!1,M.innerHTML=T.map(m=>`
          <button class="sci-search__result" type="button" data-target-id="${m.id}">
            <span class="sci-search__result-title">${m.title}</span>
            <span class="sci-search__result-meta">${m.sectionTitle}</span>
          </button>`).join(""),T)};l.forEach(n=>{n.addEventListener("toggle",()=>{if(n.open){if(!C&&n.id&&(F=n.id),C)return;l.forEach(u=>{u!==n&&(u.open=!1)})}})}),$.forEach(n=>{n.addEventListener("click",u=>{u.preventDefault();let g=n.dataset.target,_=o.querySelector(`#${g}`);_&&(H(_),U(_,{block:"start"}))})}),q?.addEventListener("input",()=>{let n=O(q.value);G(q.value,n)}),q?.addEventListener("keydown",n=>{if(n.key!=="Enter")return;let u=O(q.value),[g]=G(q.value,u);g&&(n.preventDefault(),W(g.id))}),M?.addEventListener("click",n=>{let u=n.target.closest(".sci-search__result");u&&W(u.getAttribute("data-target-id"))}),Le(o),ce().then(n=>de(o,n))}export{Ue as initSciencePage};
