import{a as he,b as fe,c as ee}from"./chunk-XCN227BY.js";import{b as pe,d as ue,e as me}from"./chunk-KD5CBWLO.js";import{a as de}from"./chunk-D6EOQ2LP.js";import{d as ce}from"./chunk-I2FVXFMW.js";import{d as le}from"./chunk-3MZTWI7A.js";import{a as O}from"./chunk-WYZYYRUA.js";import{b as J,c as Y}from"./chunk-XMLMEZIZ.js";import"./chunk-24KHYB4L.js";import{a as L,b as z}from"./chunk-WNGVR2CK.js";import{K as X,N as oe,O as ne}from"./chunk-PEDZU4MZ.js";import{j as m}from"./chunk-T2JC7ILW.js";import"./chunk-VC46IEJQ.js";function j(i,r){return i&&(i.textContent=r==null?"":String(r),i)}function be(i,{countLabel:r="32",countValue:l="",alphaValue:p=""}={}){return i&&(Y(i,["N",J("sub",{text:r}),` = ${l} \u2014 \u03B1 = ${p}`]),i)}function U(i,r=[],l=""){if(!i)return i;let p=(r||[]).filter(Boolean);if(!p.length)return i.textContent=l==null?"":String(l),i;let n=[];return p.forEach((u,f)=>{f>0&&n.push(J("br")),n.push(J("strong",{text:u?.fraction||""}),` \u2014 ${u?.description||""}`)}),Y(i,n),i}function t(i){return`<span class="sci-math sci-math--block">${i}</span>`}function e(i){return`<span class="sci-math">${i}</span>`}function o(i){return`<table class="sci-vars"><tbody>${i.map(([r,l])=>`<tr><td>${e(r)}</td><td>${l}</td></tr>`).join("")}</tbody></table>`}function s(i){return`<p class="sci-cite">${i}</p>`}function a(i,r){return`<div class="sci-formula"><h3 class="sci-formula__name">${i}</h3>${r}</div>`}function d(i,r){return`<table class="sci-data"><thead><tr>${i.map(l=>`<th>${l}</th>`).join("")}</tr></thead><tbody>${r.map(l=>`<tr>${l.map(p=>`<td>${p}</td>`).join("")}</tr>`).join("")}</tbody></table>`}var Z=Object.freeze({tempMinK:240,tempMaxK:715,pressureMinGPa:.05,pressureMaxGPa:10,left:104,right:664,top:44,bottom:300});function ve(i,r,l){return Math.min(l,Math.max(r,i))}function K(i){let{tempMinK:r,tempMaxK:l,left:p,right:n}=Z,u=(Number(i)-r)/(l-r);return p+ve(u,0,1)*(n-p)}function V(i){let{pressureMinGPa:r,pressureMaxGPa:l,top:p,bottom:n}=Z,u=Math.log10(r),f=Math.log10(l),v=(Math.log10(Math.max(Number(i),r))-u)/(f-u);return n-ve(v,0,1)*(n-p)}function xe(i,r){return`${m(i,1)},${m(r,1)}`}function $e(i,r,l=3){let p=[];for(let u=i;u<=r;u+=l){let f=z(u);!f.validRange||!Number.isFinite(f.pressurePa)||p.push({tempK:u,pressureGPa:f.pressurePa/1e9,x:K(u),y:V(f.pressurePa/1e9)})}let n=z(r);return n.validRange&&Number.isFinite(n.pressurePa)&&p.push({tempK:r,pressureGPa:n.pressurePa/1e9,x:K(r),y:V(n.pressurePa/1e9)}),p}function Se(i){return i.length?`M ${i.map(r=>xe(r.x,r.y)).join(" L ")}`:""}function we(){let i=[250,300,350,400,500,600,700],r=[.1,.3,.6,1,2.2,5,10],{left:l,right:p,top:n,bottom:u}=Z;return[...i.map(f=>{let v=K(f);return`<g class="sci-phase-gridline"><line x1="${m(v,1)}" y1="${n}" x2="${m(v,1)}" y2="${u}" /><text x="${m(v,1)}" y="${u+24}">${f} K</text></g>`}),...r.map(f=>{let v=V(f);return`<g class="sci-phase-gridline"><line x1="${l}" y1="${m(v,1)}" x2="${p}" y2="${m(v,1)}" /><text class="sci-phase-y-tick" x="${l-14}" y="${m(v+4,1)}">${f>=1?m(f,1):m(f,1)} GPa</text></g>`})].join("")}function Me(){return[{tempK:288,pressureGPa:.52,label:"0.52 GPa / 288 K",note:"below liquidus",className:"liquid",dx:10,dy:-12},{tempK:288,pressureGPa:1.2,label:"1.2 GPa / 288 K",note:"ice VI stable",className:"ice",dx:10,dy:-12},{tempK:500,pressureGPa:2.3,label:"2.3 GPa / 500 K",note:"hot deep liquid",className:"warm",dx:10,dy:18}].map(r=>{let l=K(r.tempK),p=V(r.pressureGPa);return`<g class="sci-phase-marker sci-phase-marker--${r.className}" transform="translate(${m(l,1)} ${m(p,1)})">
        <circle r="5" />
        <text x="${r.dx}" y="${r.dy}">${r.label}</text>
        <text x="${r.dx}" y="${r.dy+14}" class="sci-phase-marker__note">${r.note}</text>
      </g>`}).join("")}function qe(){let{left:i,right:r,top:l,bottom:p}=Z,n=r-i,u=p-l,f=i+n/2,v=l+u/2,M=$e(L.iceIhIceIiiLiquid.tempK,715),w=Se(M),P=M[0],R=M[M.length-1],T=P&&R?`${w} L ${r},${l} L ${i},${l} Z`:"",A=[{label:"III",tempK:L.iceIhIceIiiLiquid.tempK,pressureGPa:L.iceIhIceIiiLiquid.pressurePa/1e9},{label:"V",tempK:L.iceIiiIceVLiquid.tempK,pressureGPa:L.iceIiiIceVLiquid.pressurePa/1e9},{label:"VI",tempK:L.iceVIceViLiquid.tempK,pressureGPa:L.iceVIceViLiquid.pressurePa/1e9},{label:"VII",tempK:L.iceViIceViiLiquid.tempK,pressureGPa:L.iceViIceViiLiquid.pressurePa/1e9}].map(W=>{let E=K(W.tempK),G=V(W.pressureGPa);return`<g class="sci-phase-triple">
        <circle cx="${m(E,1)}" cy="${m(G,1)}" r="4" />
        <text x="${m(E+7,1)}" y="${m(G-7,1)}">ice ${W.label}</text>
    ...
    <svg class="sci-phase-diagram" viewBox="0 0 700 360" role="img" aria-label="WorldSmith ocean-floor water phase diagram">
      <rect x="${i}" y="${l}" width="${n}" height="${u}" class="sci-phase-liquid" />
      <path d="${T}" class="sci-phase-ice" />
      <rect x="${i}" y="${l}" width="${m(K(251.165)-i,1)}" height="${u}" class="sci-phase-unsupported" />
      ${we()}
      <path d="${w}" class="sci-phase-liquidus" />
      ${A}
      ${Me()}
    ...
    <div class="sci-phase-legend" aria-label="Diagram legend">
  ...
  </div>`}function N(i){return String(i??"").trim().toLowerCase()}function Te(i,r){if(!r)return 0;let l=N(i.title);if(l===r)return 120;if(l.startsWith(r))return 90;if(l.includes(r))return 70;let p=N(i.sectionTitle);return p===r?52:p.includes(r)?40:N(i.text).includes(r)?18:0}function Ce(i){return Array.from(i.querySelectorAll(".sci-section")).flatMap(r=>{let l=String(r.id||"").trim(),p=r.querySelector(".sci-section__title")?.textContent?.trim()||"Science Section";return Array.from(r.querySelectorAll(".sci-formula")).map((n,u)=>{let f=`${l}-formula-${u+1}`;return n.id=f,n.setAttribute("tabindex","-1"),{id:f,sectionId:l,sectionTitle:p,title:n.querySelector(".sci-formula__name")?.textContent?.trim()||"Formula",text:n.textContent||""}})})}function ge(i,r,l=8){let p=N(r);return p?i.map(n=>({entry:n,score:Te(n,p)})).filter(n=>n.score>0).sort((n,u)=>u.score-n.score||n.entry.sectionTitle.localeCompare(u.entry.sectionTitle)||n.entry.title.localeCompare(u.entry.title)).slice(0,l).map(n=>n.entry):[]}function ke(){return[a("Mass-Luminosity Relation",`<div class="sci-formula__eq">${t("L = c \\cdot M^{\\alpha}")}</div>
      <p>Six-piece empirical power law relating stellar mass to luminosity, fitted to 509 eclipsing binary components.</p>
      ${d(["Mass range (M&#9737;)","\\(\\alpha\\)","c"],[["M &lt; 0.45","2.028","0.0892"],["0.45 &le; M &lt; 0.72","4.572","0.68"],["0.72 &le; M &lt; 1.05","5.743","1.0"],["1.05 &le; M &lt; 2.4","4.329","1.072"],["2.4 &le; M &lt; 7.0","3.967","1.471"],["M &ge; 7.0","2.865","12.55"]])}
      ...
      ${d(["Class","T range (K)","Span"],[["O","33,000 &ndash; 95,000","62,000"],["B","10,000 &ndash; 33,000","23,000"],["A","7,500 &ndash; 10,000","2,500"],["F","6,000 &ndash; 7,500","1,500"],["G","5,200 &ndash; 6,000","800"],["K","3,700 &ndash; 5,200","1,500"],["M","2,000 &ndash; 3,700","1,700"]])}`),a("Stellar Environment Diagnostics",`<div class="sci-formula__eq">${t("Ro = P_{\\text{rot}} / \\tau_c")}</div>
      ...
      ${s("Barnes (2007) ApJ 669, 1167; Mamajek &amp; Hillenbrand (2008) ApJ 687, 1264; Wood et al. (2005) ApJ 628, L143; Ranjan &amp; Sasselov (2017) Astrobiology 17, 169; Arney et al. (2016) Astrobiology 16, 873")}`)].join("")}function Pe(){return[a("Metallicity Conversion",`<div class="sci-formula__eq">${t("Z = Z_\\odot \\cdot 10^{[\\text{Fe/H}]}")}</div>
      ...
      ${s("Hurley, Pols &amp; Tout (2000), MNRAS 315, 543")}`)].join("")}function Re(){return[a("Mass&ndash;Radius Relation (Radius-First)",`<div class="sci-formula__eq">${t("R_\\oplus = (1.07 - 0.21 \\cdot \\text{CMF}) \\cdot M_\\oplus^{\\,\\alpha}")}</div>
      ...
      ${d(["Boundary used","Temperature range","Anchor in model"],[["ice III / liquid","251.165&ndash;256.164 K","0.209&ndash;0.350 GPa"],["ice V / liquid","256.164&ndash;273.31 K","0.350&ndash;0.632 GPa"],["ice VI / liquid","273.31&ndash;355 K","0.632&ndash;2.216 GPa"],["ice VII / liquid","355&ndash;715 K",`${m(z(550).pressurePa/1e9,2)} GPa at 550 K`]])}
      ...
      ${d(["Gas","Formula","m<sub>i</sub> (kg/mol)"],[["N&#8322;","remainder (100 &minus; &sum; others)","0.028"],["O&#8322;","user input","0.032"],["CO&#8322;","user input","0.044"],["Ar","user input","0.040"],["H&#8322;O","user input","0.018"],["CH&#8324;","user input","0.016"],["H&#8322;","user input (Full mode)","0.002"],["He","user input (Full mode)","0.004"],["SO&#8322;","user input (Full mode)","0.064"],["NH&#8323;","user input (Full mode)","0.017"]])}
      ...
      ${d(["Rotation period","Cells"],[["&ge; 48 h","1 (single Hadley)"],["6 &ndash; 48 h","3 (Earth-like)"],["3 &ndash; 6 h","7 (rapid rotator)"],["&lt; 3 h","5 (very rapid)"]])}`)].join("")}function Ae(){return[a("Mass&ndash;Radius Relation",`<p>Two-regime power law calibrated to Solar System giants:</p>
      ...
      ${d(["Class","T<sub>eq</sub> (K)","Cloud deck","Albedo"],[["I","&le; 150","NH&#8323; ice","0.34"],["II","150&ndash;250","H&#8322;O","0.81"],["III","250&ndash;800","Cloudless","0.12"],["IV","800&ndash;1400","Alkali metals","0.10"],["V","&gt; 1400","Silicate/iron","0.55"]])}
      ...
      ${d(["Species","T<sub>cond</sub> (K)","Pressure level"],[["Iron (Fe)","1800","0.01 bar"],["Silicate (MgSiO&#8323;)","1400","0.1 bar"],["H&#8322;O","300","5 bar"],["NH&#8324;SH","200","2 bar"],["NH&#8323;","150","0.7 bar"],["CH&#8324; (ice giants)","80","1.5 bar"]])}
      ...
      <p>Gas and ice giants emit absorbed starlight plus retained internal heat from Kelvin-Helmholtz cooling and interior transport. WorldSmith reports equilibrium temperature (${e("T_{\\text{eq}}")}) and effective temperature (${e("T_{\\text{eff}}")}) separately so cold, self-luminous giants are not treated like rocky surfaces.</p>
      ...
      ${d(["Planet","Model B (G)","Observed B (G)","Ratio"],[["Jupiter","4.28","4.28","1.00&times;"],["Saturn","~0.21","0.21","~0.99&times;"],["Uranus","~0.24","0.23","~1.02&times;"],["Neptune","~0.14","0.14","~1.00&times;"]])}
      ...
      ${d(["Planet","Model R<sub>mp</sub>","Observed R<sub>mp</sub>","Error"],[["Jupiter","75 Rp","75 Rp","~0%"],["Saturn","22 Rp","22 Rp","~0%"],["Uranus","19 Rp","18 Rp","~3%"],["Neptune","18 Rp","23 Rp","~21%"]])}
      ...
      ${d(["Body","Mass (M<sub>J</sub>)","Model k&#8322;","Observed k&#8322;","Source"],[["Jupiter","1.0","0.385","0.379","Wahl+ 2016 (Juno)"],["Saturn","0.30","0.385","0.390","Lainey+ 2017 (Cassini)"],["Uranus","0.046","0.104","0.104","Gavrilov &amp; Zharkov 1977"],["Neptune","0.054","0.128","0.127","Gavrilov &amp; Zharkov 1977"]])}
      ...
      ${d(["Regime","Mass range","Q","Analogue"],[["Ice giant","&lt; 0.15 M<sub>J</sub>","15,000","Uranus, Neptune"],["Saturn-like","0.2&ndash;0.5 M<sub>J</sub>","2,500","Saturn (resonance locking)"],["Jupiter-like","&ge; 0.8 M<sub>J</sub>","35,000","Jupiter"]])}
      ...
      ${s("Peale, Cassen &amp; Reynolds (1979); Wahl et al. (2016); Lainey et al. (2009, 2017); Fuller, Luan &amp; Quataert (2016)")}`)].join("")}function Ee(){return[a("Kepler&rsquo;s Third Law (Moon Orbit)",`<div class="sci-formula__eq">${t("P = \\frac{2\\pi\\sqrt{a^3 / G(M_p + M_m)}}{86400} \\text{ days}")}</div>
      ...
      ${d(["Label","Separation","Meaning"],[["Stable","&ge; 10 R<sub>H,m</sub>","Comfortably separated for this simplified diagnostic"],["Packed","6 to &lt; 10 R<sub>H,m</sub>","Dynamically close but often plausible"],["Crowded","2&radic;3 to &lt; 6 R<sub>H,m</sub>","Long-term stability is questionable"],["Unstable","&lt; 2&radic;3 R<sub>H,m</sub>","Below the circular coplanar Hill-stability boundary"]])}
      ...
      ${s("Paranicas et al. (2009); Divine & Garrett (1983) \u2014 Jupiter radiation environment")}`),a("Icy Moon Sputtered Oxygen Exospheres",`<p>Europa-like icy moons can carry an abiotic O<sub>2</sub>/H<sub>2</sub> exosphere without having breathable air. WorldSmith treats this as a surface-boundary context: parent magnetospheric particles dissociate exposed water ice, sputtering and radiolysis release neutrals, and pickup ions plus surface trapping remove them.</p>
      ...
      ${d(["Species","T_{sub} (K)","\\rho_{max}","Source"],[["N\\u2082","35","2.5 g/cm\\u00B3","Fray & Schmitt (2009)"],["CO","35","2.5 g/cm\\u00B3",""],["CH\\u2084","50","2.5 g/cm\\u00B3",""],["CO\\u2082","115","3.2 g/cm\\u00B3",""],["NH\\u2083","130","2.5 g/cm\\u00B3",""],["SO\\u2082","140","volcanic",""],["H\\u2082O","210","3.2 g/cm\\u00B3",""]])}
      ...
      <p><b>Reference inputs:</b> this moon-world layer sits on top of the tidal heating, moon temperature, magnetospheric radiation, volatile-retention, magnetosphere, and atmosphere-stability blocks documented above, then adds WorldSmith&rsquo;s own hydrosphere, geology, biosphere, and integrated habitability policy layers.</p>`)].join("")}function He(){return[a("Hill Sphere",`<div class="sci-formula__eq">${t("r_H = a \\left(\\frac{m}{3M_\\star}\\right)^{1/3}")}</div>
      ...
      evidence; stable points do not guarantee populated Trojans.</p>`)].join("")}function Le(){return[a("Stellar Absolute Magnitude",`<div class="sci-formula__eq">${t("M_V = 4.81 - 2.5\\log_{10}(L)")}</div>
      ...
      <p>If ${e("\\theta_{\\text{moon}} \\ge \\theta_\\star")}: total eclipses. Otherwise: annular only.</p>`)].join("")}function Fe(){return[a("Scale Height Correction",`<div class="sci-formula__eq">${t("\\frac{H_{\\text{planet}}}{H_\\oplus} = \\frac{T / T_\\oplus}{g / g_\\oplus}")}</div>
      ...
      ${d(["Body","P (atm)","CO&#8322; %","H&#8322;O %","CH&#8324; %","&tau;","T<sub>surf</sub> (K)"],[["Earth","1.0","0.04","0.40","0","0.70","288"],["Venus","92","96.5","0","0","126","737"],["Mars","0.006","95.3","0","0","0.029","211"]])}
      ...
      ${d(["\\u03BB range","Status","Meaning"],[["\\u2265 6","Retained","Firmly held over geological time (> 4.5 Gyr)"],["3 &ndash; 6","Marginal","Slow escape; may be lost on Gyr timescales"],["< 3","Lost","Rapid thermal escape (lost within ~100 Myr)"]])}
      ...
      <p><b>Calibration:</b></p>
      ${d(["Body","T<sub>eq</sub> (K)","F/F<sub>0</sub>","P (atm)","&eta;<sub>abs</sub>","T<sub>exo</sub> (K)","Observed"],[["Earth","254","1.0","1.0","0.94","~944","700&ndash;1400"],["Venus","229","1.9","92","1.00","~229","250&ndash;300"],["Mars","210","0.43","0.006","0.09","~233","200&ndash;350"],["Pluto","32","0.0006","10&#8315;&#8309;","&lt;0.001","~32","~65&ndash;70"]])}
      ...
      ${d(["Gas","Factor","Lost","Marginal","Retained"],[["H&#8322;","&times;3.0","&lambda; < 9","9 &le; &lambda; < 18","&lambda; &ge; 18"],["He","&times;5.0","&lambda; < 15","15 &le; &lambda; < 30","&lambda; &ge; 30"],["Others","&times;1.0","&lambda; < 3","3 &le; &lambda; < 6","&lambda; &ge; 6"]])}
      ...
      ${d(["Zone","Base moisture"],[["Hadley equatorial (lat &lt; 0.7&times;cell)","0.9 &minus; 0.75&times;fraction"],["Hadley subsidence edge","0.15"],["Ferrel warm-coast","0.70"],["Ferrel cold-coast","0.45"],["Ferrel general","0.55"],["Polar","0.20"]])}
      ...
      elevated terrain. Used for all climate zone calculations.</p>`)].join("")}function Ge(){return[a("Power-Law Flare Rate",`<div class="sci-formula__eq">${t("N(> E) = N_{32} \\left(\\frac{E}{10^{32} \\text{ erg}}\\right)^{-\\alpha}")}</div>
      <p>Cumulative flare frequency distribution: expected flares per day with energy above ${e("E")}.</p>
      ${d(["Spectral bin","T<sub>eff</sub> (K)","Old","Mid","Young","&alpha;"],[["FGK","&ge; 3900","0.05","0.25","1.0","1.8"],["Early M","3200&ndash;3900","0.5","2.0","8.0","2.0"],["Late M","&lt; 3200","2.0","8.0","30.0","2.2"]])}
      ...
      ${d(["Flare energy (erg)","P(CME)"],[["&lt; 10&sup3;&sup2;","0.005"],["10&sup3;&sup2; &ndash; 10&sup3;&sup3;","0.12"],["10&sup3;&sup3; &ndash; 10&sup3;&sup4;","0.4"],["&gt; 10&sup3;&sup4;","0.75"]])}
      ...
      ${d(["Spectral bin","Old","Mid","Young","&alpha;"],[["FGK (T &ge; 3900 K)","0.05","0.25","1.0","1.8"],["Early M (3200&ndash;3900 K)","0.5","2.0","8.0","2.0"],["Late M (&lt; 3200 K)","2.0","8.0","30.0","2.2"]])}
      ...
      ${d(["Spectral bin","Min (&phi;=0)","Mid (&phi;=0.5)","Max (&phi;=1)"],[["FGK","0.35","1.0","1.65"],["Early M","0.6","1.0","1.4"],["Late M","0.75","1.0","1.25"]])}
      ...
      ${d(["Category","Fraction"],[["Main-sequence stars","72%"],["White dwarfs","6%"],["Brown dwarfs","19%"],["Other (giants, subdwarfs)","3%"]])}
      ...
      ${d(["Class","Binary","Triple","Quadruple"],[["O","0.70","0.12","0.05"],["B","0.50","0.09","0.04"],["A","0.45","0.08","0.03"],["F","0.46","0.08","0.03"],["G","0.46","0.08","0.03"],["K","0.35","0.05","0.02"],["M","0.27","0.03","0.01"],["WD","0.25","0.02","0.005"],["L/T/Y","0.15","0.01","0.003"]])}
      ...
      ${d(["Era","Density (km&sup2;)","Growth (%/yr)"],[["Hunter-Gatherer","0.05","0.5"],["Neolithic","2","0.8"],["Bronze Age","8","1.0"],["Iron Age","15","1.0"],["Medieval","30","1.0"],["Early Industrial","80","1.5"],["Industrial","200","2.0"],["Post-Industrial","400","0.5"],["Sci-Fi High","1000","0.3"]])}`),a("Logistic Growth (Verhulst)",`<div class="sci-formula__eq">${t("P(t) = \\frac{K}{1 + \\frac{K - P_0}{P_0}\\,e^{-rt}}")}</div>
      ...
      ${s("Zipf (1949), Human Behavior and the Principle of Least Effort")}`)].join("")}function Ne(){return[a("Water-Aware Radius (Zeng & Sasselov 2016)",`<div class="sci-formula__eq">${t("R = R_{\\text{dry}} \\cdot \\left(1 + \\left(\\frac{R_{50}}{R_{\\text{ref}}} - 1\\right) \\cdot \\min\\!\\left(\\frac{\\text{WMF}}{0.5},\\;1\\right)\\right)")}</div>
      ...
      ${d(["Isotope","Half-life (Gyr)","Heat fraction <i>w<sub>i</sub></i>"],[["U-238","4.47","0.39"],["U-235","0.70","0.04"],["Th-232","14.05","0.40"],["K-40","1.25","0.17"]])}
      ...
      ${d(["Class","Condition"],[["Dwarf planet","M &lt; 0.01 M&#8853;"],["Planet","M &ge; 0.01 M&#8853;"]])}
      ...
      ${d(["State","&Delta;IW","Primary gases"],[["Highly reduced","&minus;4","H&#8322; + CO"],["Moderately reduced","&minus;2","H&#8322; + CO&#8322; (mixed)"],["Earth-like","+1","CO&#8322; + H&#8322;O"],["Oxidised","+3","CO&#8322; + H&#8322;O + SO&#8322;"]])}
      <p>The iron-w&uuml;stite (IW) buffer sets the reference. Earth is approximately IW+1.</p>
      ${s("Ortenzi et al. (2020), Sci. Rep. 10, 10907")}`)].join("")}function Ke(){return[a("Maximum Mountain Height",`<div class="sci-formula__eq">${t("H_{\\max} = \\frac{\\sigma_y}{\\rho \\cdot g} \\approx \\frac{9{,}267}{g} \\text{ m}")}</div>
      ...
      ${d(["Regime","Rate (mm/yr)","Label"],[["Mobile lid","20&ndash;200","Active spreading"],["Episodic overturn","5&ndash;50","Episodic spreading"],["Plutonic-squishy","2&ndash;20","Sluggish spreading"],["Stagnant lid","0","No spreading"]])}
      ...
      ${d(["Zone","Typical width","Depth range","Slope"],[["Continental shelf","80 km","0&ndash;130 m","~0.1&deg;"],["Continental slope","varies","130&ndash;3,000 m","3&ndash;4&deg;"],["Continental rise","200 km","3,000&ndash;4,500 m","~0.5&deg;"],["Abyssal plain","indefinite","4,500+ m","~0&deg;"]])}
      ...
      ${d(["Resonance","(p/q)<sup>2/3</sup>"],[["3:2 exterior","1.310"],["2:1 exterior","1.587"],["5:2 exterior","1.842"],["1:2 interior","0.630"],["1:4 interior","0.397"],["1:8 interior","0.250"]])}`),a("Condensation Sequence",`<p>Species condense from a cooling solar-composition nebula at characteristic temperatures:</p>
      ${d(["Species","T<sub>cond</sub> (K)","Mass %"],[["Corundum (Al&#8322;O&#8323;)","1700","0.4"],["Iron-nickel","1450","7"],["Enstatite (MgSiO&#8323;)","1350","12"],["Forsterite (Mg&#8322;SiO&#8324;)","1300","14"],["Feldspar","1200","6"],["Troilite (FeS)","700","4"],["Organics","300","6"],["Water ice","170","33"],["NH&#8323; hydrate","130","2"],["CO&#8322; ice","70","5"],["CH&#8324; ice","31","4"],["CO ice","25","3"],["N&#8322; ice","22","2"]])}
      ...
      Detectable if &gt; 0.1 (easily), marginal 0.01&ndash;0.1, undetectable &lt; 0.01.</p>`)].join("")}function Ve(){let i=(r,l)=>`<div class="sci-formula"><h3 class="sci-formula__name">${r}</h3>${l}</div>`;return[`<p class="sci-diverge-intro">WorldSmith aims to reproduce published astrophysical models wherever possible.
    ...
    to WorldSmith; items marked <b>Simplified</b> are reductions of published work.</p>`,i("Oort Cloud Mass And LPC Flux Proxy (WS-derived / Simplified)",`<p>WorldSmith&rsquo;s Oort-cloud mass and long-period-comet flux are not taken from a single published closed-form model. The edge scaling follows exo-Oort Hill-radius work, but the mass and LPC-rate outputs are WorldSmith proxies built from five factors: total giant mass, outer-giant extent, inner-ejector depletion, age saturation, and environmental retention.</p>
      ...
      ${s("Mardling &amp; Aarseth (2001), MNRAS 321, 398; WorldSmith hierarchy guardrails in engine/homeSystem/stability.js")}`)].join("")}function Je(i,r){let l=r/100,p=Math.log(Math.max(i,1e-6)),n=Math.min(1/3,.257-.0161*p),u=(1.07-.21*l)*Math.pow(i,n),f=i*5.51/Math.pow(u,3);return{R:u,rho:f}}function je(i){let r=i.querySelector("#sci-mlr-mass"),l=i.querySelector("#sci-mlr-slider"),p=i.querySelector("#sci-mlr-result");if(r&&l&&p){let _=()=>{let h=X(Number(r.value));p.textContent=`${m(h,4)} L\u2609`};O({numberEl:r,sliderEl:l,min:.075,max:100,step:.01,mode:"log"}),r.addEventListener("input",_),l.addEventListener("input",()=>{r.dispatchEvent(new Event("input"))}),_()}let n=i.querySelector("#sci-hz-mass"),u=i.querySelector("#sci-hz-slider"),f=i.querySelector("#sci-hz-result");if(n&&u&&f){let _=()=>{let h=Number(n.value),S=X(h),C=oe(h),I=ne({luminosityLsol:S,teffK:C});j(f,`${m(I.innerAu,2)} \u2013 ${m(I.outerAu,2)} AU`)};O({numberEl:n,sliderEl:u,min:.075,max:100,step:.01,mode:"log"}),n.addEventListener("input",_),u.addEventListener("input",()=>{n.dispatchEvent(new Event("input"))}),_()}let v=i.querySelector("#sci-dens-mass"),M=i.querySelector("#sci-dens-mass-slider"),w=i.querySelector("#sci-dens-cmf"),P=i.querySelector("#sci-dens-cmf-slider"),R=i.querySelector("#sci-dens-result");if(v&&M&&w&&P&&R){let _=()=>{let{R:h,rho:S}=Je(Number(v.value),Number(w.value));j(R,`${m(h,3)} R\u2295 \u2014 ${m(S,2)} g/cm\xB3`)};O({numberEl:v,sliderEl:M,min:.01,max:100,step:.01,mode:"log"}),O({numberEl:w,sliderEl:P,min:0,max:100,step:1,mode:"linear"}),[v,M,w,P].forEach(h=>h.addEventListener("input",_)),_()}let T=i.querySelector("#sci-hmag-rad"),F=i.querySelector("#sci-hmag-alb"),A=i.querySelector("#sci-hmag-alb-slider"),W=i.querySelector("#sci-hmag-result");if(T&&F&&W){let _=()=>{let h=Number(T.value),S=Number(F.value);if(h>0&&S>0){let C=ce({radiusKm:h,geometricAlbedo:S});W.textContent=`H = ${m(C,2)}`}};A&&O({numberEl:F,sliderEl:A,min:.01,max:1,step:.01,mode:"linear"}),[T,F].forEach(h=>h.addEventListener("input",_)),A&&A.addEventListener("input",_),_()}let E=i.querySelector("#sci-flare-temp"),G=i.querySelector("#sci-flare-temp-slider"),D=i.querySelector("#sci-flare-age"),c=i.querySelector("#sci-flare-age-slider"),b=i.querySelector("#sci-flare-result");if(E&&D&&b){let _=()=>{let h=le({teffK:Number(E.value),ageGyr:Number(D.value)});be(b,{countLabel:"32",countValue:h.N32,alphaValue:h.alpha})};G&&O({numberEl:E,sliderEl:G,min:2e3,max:1e4,step:100,mode:"linear"}),c&&O({numberEl:D,sliderEl:c,min:.01,max:13,step:.1,mode:"linear"}),[E,G,D,c].filter(Boolean).forEach(h=>h.addEventListener("input",_)),_()}let x=i.querySelector("#sci-leap-len"),$=i.querySelector("#sci-leap-result");if(x&&$){let _=S=>{if(S===0)return{p:0,q:1};let C=0,I=1;for(let H=1;H<=1e4;H++){let q=Math.round(S*H);if(Math.abs(q/H-S)<1e-9){C=q,I=H;break}}return{p:C,q:I}},h=()=>{let S=Number(x.value),C=S-Math.floor(S);if(C<1e-9){U($,[],"No leap cycle needed (integer year)");return}let H=de(C,6).slice(1).map(_).filter(q=>q.q>0);if(H.length===0){U($,[],"No usable cycle found");return}U($,H.map(q=>{let se=Math.abs(q.p/q.q-C),re=se>1e-12?Math.round(1/se):1/0,_e=re===1/0?"exact":`~1 day drift per ${m(re,0)} yr`;return{fraction:`${q.p}/${q.q}`,description:`${q.p} leap day${q.p!==1?"s":""} every ${q.q} years (${_e})`}}))};x.addEventListener("input",h),h()}let k=i.querySelector("#sci-ghz-r"),g=i.querySelector("#sci-ghz-loc"),y=i.querySelector("#sci-ghz-result");if(k&&g&&y){let _=()=>{let h=Number(k.value),S=Number(g.value),C=.53*h,I=.1*h,H=Math.exp(-.5*Math.pow((S-C)/I,2)),q=S>=.47*h&&S<=.6*h;j(y,`${m(H*100,1)}%${q?" (in hard band)":""}`)};[k,g].forEach(h=>h.addEventListener("input",_)),_()}let B=i.querySelector("#sci-tec-grav"),Q=i.querySelector("#sci-tec-grav-slider"),te=i.querySelector("#sci-tec-mtn"),ae=i.querySelector("#sci-tec-shield"),ie=i.querySelector("#sci-tec-root");if(B&&Q&&te&&ae&&ie){let _=()=>{let h=Number(B.value);h>0&&(te.textContent=`${m(pe(h),0)} m`,ae.textContent=`${m(me(h),0)} m`,ie.textContent=`${m(ue(5e3),0)} m`)};O({numberEl:B,sliderEl:Q,min:.05,max:5,step:.01,mode:"log"}),B.addEventListener("input",_),Q.addEventListener("input",()=>{B.dispatchEvent(new Event("input"))}),_()}}var ye=[{id:"stellar",title:"Stellar Physics",count:9,builder:ke},{id:"evolution",title:"Stellar Evolution",count:7,builder:Pe},{id:"planetary",title:"Planetary Physics",count:14,builder:Re},{id:"gasgiant",title:"Gas Giant Physics",count:13,builder:Ae},{id:"interior",title:"Interior &amp; Composition",count:7,builder:Ne},{id:"tectonics",title:"Tectonics &amp; Geodynamics",count:14,builder:Ke},{id:"orbital",title:"Orbital Mechanics",count:22,builder:Ee},{id:"lagrange",title:"Lagrange Points",count:4,builder:He},{id:"photometry",title:"Photometry &amp; Magnitudes",count:8,builder:Le},{id:"atmosphere",title:"Atmosphere &amp; Colour",count:10,builder:Fe},{id:"climate",title:"Climate Classification",count:8,builder:We},{id:"activity",title:"Stellar Activity",count:7,builder:Ge},{id:"calendar",title:"Calendar Systems",count:5,builder:Oe},{id:"cluster",title:"Local Cluster",count:7,builder:Ie},{id:"population",title:"Population Dynamics",count:5,builder:De},{id:"system",title:"System Architecture",count:7,builder:ze},{id:"debris",title:"Debris Disks",count:12,builder:Be},{id:"divergences",title:"Divergences from Published Science",count:36,builder:Ve}];function lt(i){let r=document.createElement("div");r.className="page";let l=ye.map(c=>`<a class="sci-toc__link" data-target="sci-${c.id}" href="#sci-${c.id}">${c.title}</a>`).join(""),p=ye.map((c,b)=>`<details class="sci-section" id="sci-${c.id}"${b===0?" open":""}>
  ...
  `,i.innerHTML="",i.appendChild(r);let n=Array.from(r.querySelectorAll(".sci-section")),u=Array.from(r.querySelectorAll(".sci-formula")),f=Array.from(r.querySelectorAll(".sci-toc__link")),v=r.querySelector("#sciencePageSearch"),M=r.querySelector("#sciencePageSearchStatus"),w=r.querySelector("#sciencePageSearchResults"),P=Ce(r),R=null,T=!1,F=n.find(c=>c.open)?.id||n[0]?.id||null,A=(c,{exclusive:b=!T}={})=>{c&&(b?n.forEach(x=>{x.open=x===c}):c.open=!0,!T&&c.id&&(F=c.id))},W=()=>{R&&(clearTimeout(R),R=null),r.querySelectorAll(".sci-formula.is-search-hit").forEach(c=>{c.classList.remove("is-search-hit")})},E=c=>{if(!c)return;let b=r.querySelector(`#${c}`);if(!b)return;let x=b.closest(".sci-section");A(x),W(),b.classList.add("is-search-hit"),ee(b,{block:"center"}),b.focus?.({preventScroll:!0}),R=window.setTimeout(()=>{b.classList.remove("is-search-hit"),R=null},2200)},G=c=>{let b=N(c);if(W(),!b){T=!1,r.classList.remove("is-search-filtering"),u.forEach(y=>{y.hidden=!1,y.classList.remove("is-search-match","is-search-filtered-out")}),n.forEach(y=>{y.hidden=!1,y.classList.remove("is-search-match","is-search-filtered-out")}),f.forEach(y=>{y.hidden=!1,y.classList.remove("is-search-filtered-out")});let g=n.find(y=>y.id===F)||n[0]||null;return A(g,{exclusive:!0}),[]}let x=ge(P,b,P.length),$=new Set(x.map(g=>g.id)),k=new Set(x.map(g=>g.sectionId));return T=!0,r.classList.add("is-search-filtering"),u.forEach(g=>{let y=$.has(g.id);g.hidden=!y,g.classList.toggle("is-search-match",y),g.classList.toggle("is-search-filtered-out",!y)}),n.forEach(g=>{let y=k.has(g.id);g.hidden=!y,g.open=y,g.classList.toggle("is-search-match",y),g.classList.toggle("is-search-filtered-out",!y)}),f.forEach(g=>{let y=k.has(g.dataset.target||"");g.hidden=!y,g.classList.toggle("is-search-filtered-out",!y)}),x},D=(c,b=null)=>{if(!w||!M)return[];let x=N(c);if(!x)return w.hidden=!0,w.innerHTML="",M.hidden=!0,M.textContent="",[];let $=b||ge(P,x,P.length),k=$.slice(0,8);return M.hidden=!1,M.textContent=$.length>0?`${$.length} match${$.length===1?"":"es"}${$.length>k.length?`, showing top ${k.length}`:""}`:"No matches found",$.length===0?(w.hidden=!1,w.innerHTML='<div class="sci-search__empty">No formulas or topics matched that search.</div>',[]):(w.hidden=!1,w.innerHTML=k.map(g=>`
          ...
          </button>`).join(""),k)};n.forEach(c=>{c.addEventListener("toggle",()=>{if(c.open){if(!T&&c.id&&(F=c.id),T)return;n.forEach(b=>{b!==c&&(b.open=!1)})}})}),f.forEach(c=>{c.addEventListener("click",b=>{b.preventDefault();let x=c.dataset.target,$=r.querySelector(`#${x}`);$&&(A($),ee($,{block:"start"}))})}),v?.addEventListener("input",()=>{let c=G(v.value);D(v.value,c)}),v?.addEventListener("keydown",c=>{if(c.key!=="Enter")return;let b=G(v.value),[x]=D(v.value,b);x&&(c.preventDefault(),E(x.id))}),w?.addEventListener("click",c=>{let b=c.target.closest(".sci-search__result");b&&E(b.getAttribute("data-target-id"))}),je(r),he().then(c=>fe(r,c))}export{lt as initSciencePage};
