import{a as he,b as fe,c as ee}from"./chunk-XCN227BY.js";import{b as pe,d as ue,e as me}from"./chunk-KD5CBWLO.js";import{a as de}from"./chunk-D6EOQ2LP.js";import{d as ce}from"./chunk-I2FVXFMW.js";import{a as O}from"./chunk-WYZYYRUA.js";import{d as le}from"./chunk-3MZTWI7A.js";import{b as J,c as Y}from"./chunk-XMLMEZIZ.js";import{a as E,b as z}from"./chunk-WNGVR2CK.js";import{K as X,N as oe,O as ne}from"./chunk-PEDZU4MZ.js";import{j as m}from"./chunk-T2JC7ILW.js";import"./chunk-24KHYB4L.js";import"./chunk-VC46IEJQ.js";function j(i,r){return i&&(i.textContent=r==null?"":String(r),i)}function ge(i,{countLabel:r="32",countValue:l="",alphaValue:p=""}={}){return i&&(Y(i,["N",J("sub",{text:r}),` = ${l} \u2014 \u03B1 = ${p}`]),i)}function U(i,r=[],l=""){if(!i)return i;let p=(r||[]).filter(Boolean);if(!p.length)return i.textContent=l==null?"":String(l),i;let n=[];return p.forEach((u,f)=>{f>0&&n.push(J("br")),n.push(J("strong",{text:u?.fraction||""}),` \u2014 ${u?.description||""}`)}),Y(i,n),i}function t(i){return`<span class="sci-math sci-math--block">${i}</span>`}function e(i){return`<span class="sci-math">${i}</span>`}function o(i){return`<table class="sci-vars"><tbody>${i.map(([r,l])=>`<tr><td>${e(r)}</td><td>${l}</td></tr>`).join("")}</tbody></table>`}function s(i){return`<p class="sci-cite">${i}</p>`}function a(i,r){return`<div class="sci-formula"><h3 class="sci-formula__name">${i}</h3>${r}</div>`}function d(i,r){return`<table class="sci-data"><thead><tr>${i.map(l=>`<th>${l}</th>`).join("")}</tr></thead><tbody>${r.map(l=>`<tr>${l.map(p=>`<td>${p}</td>`).join("")}</tr>`).join("")}</tbody></table>`}var Z=Object.freeze({tempMinK:240,tempMaxK:715,pressureMinGPa:.05,pressureMaxGPa:10,left:104,right:664,top:44,bottom:300});function ve(i,r,l){return Math.min(l,Math.max(r,i))}function K(i){let{tempMinK:r,tempMaxK:l,left:p,right:n}=Z,u=(Number(i)-r)/(l-r);return p+ve(u,0,1)*(n-p)}function V(i){let{pressureMinGPa:r,pressureMaxGPa:l,top:p,bottom:n}=Z,u=Math.log10(r),f=Math.log10(l),v=(Math.log10(Math.max(Number(i),r))-u)/(f-u);return n-ve(v,0,1)*(n-p)}function xe(i,r){return`${m(i,1)},${m(r,1)}`}function $e(i,r,l=3){let p=[];for(let u=i;u<=r;u+=l){let f=z(u);!f.validRange||!Number.isFinite(f.pressurePa)||p.push({tempK:u,pressureGPa:f.pressurePa/1e9,x:K(u),y:V(f.pressurePa/1e9)})}let n=z(r);return n.validRange&&Number.isFinite(n.pressurePa)&&p.push({tempK:r,pressureGPa:n.pressurePa/1e9,x:K(r),y:V(n.pressurePa/1e9)}),p}function Se(i){return i.length?`M ${i.map(r=>xe(r.x,r.y)).join(" L ")}`:""}function Me(){let i=[250,300,350,400,500,600,700],r=[.1,.3,.6,1,2.2,5,10],{left:l,right:p,top:n,bottom:u}=Z;return[...i.map(f=>{let v=K(f);return`<g class="sci-phase-gridline"><line x1="${m(v,1)}" y1="${n}" x2="${m(v,1)}" y2="${u}" /><text x="${m(v,1)}" y="${u+24}">${f} K</text></g>`}),...r.map(f=>{let v=V(f);return`<g class="sci-phase-gridline"><line x1="${l}" y1="${m(v,1)}" x2="${p}" y2="${m(v,1)}" /><text class="sci-phase-y-tick" x="${l-14}" y="${m(v+4,1)}">${f>=1?m(f,1):m(f,1)} GPa</text></g>`})].join("")}function we(){return[{tempK:288,pressureGPa:.52,label:"0.52 GPa / 288 K",note:"below liquidus",className:"liquid",dx:10,dy:-12},{tempK:288,pressureGPa:1.2,label:"1.2 GPa / 288 K",note:"ice VI stable",className:"ice",dx:10,dy:-12},{tempK:500,pressureGPa:2.3,label:"2.3 GPa / 500 K",note:"hot deep liquid",className:"warm",dx:10,dy:18}].map(r=>{let l=K(r.tempK),p=V(r.pressureGPa);return`<g class="sci-phase-marker sci-phase-marker--${r.className}" transform="translate(${m(l,1)} ${m(p,1)})">
    ...
    <div class="sci-phase-legend" aria-label="Diagram legend">
      ...
      ${s("IAPWS R14-08(2011), Revised Release on the Pressure along the Melting and Sublimation Curves of Ordinary Water Substance; Wagner, Riethmann, Feistel &amp; Harvey (2011), JPCRD 40, 043103.")}`),a("Atmospheric Density (Ideal Gas Law)",`<div class="sci-formula__eq">${t("\\rho = \\frac{p \\cdot M_w}{R \\cdot T}")}</div>
      ...
      ${s("Peale, Cassen &amp; Reynolds (1979); Wahl et al. (2016); Lainey et al. (2009, 2017); Fuller, Luan &amp; Quataert (2016)")}`)].join("")}function He(){return[a("Kepler&rsquo;s Third Law (Moon Orbit)",`<div class="sci-formula__eq">${t("P = \\frac{2\\pi\\sqrt{a^3 / G(M_p + M_m)}}{86400} \\text{ days}")}</div>
      ...
      <p><b>Reference inputs:</b> this moon-world layer sits on top of the tidal heating, moon temperature, magnetospheric radiation, volatile-retention, magnetosphere, and atmosphere-stability blocks documented above, then adds WorldSmith&rsquo;s own hydrosphere, geology, biosphere, and integrated habitability policy layers.</p>`)].join("")}function Le(){return[a("Hill Sphere",`<div class="sci-formula__eq">${t("r_H = a \\left(\\frac{m}{3M_\\star}\\right)^{1/3}")}</div>
      ...
      evidence; stable points do not guarantee populated Trojans.</p>`)].join("")}function Ee(){return[a("Stellar Absolute Magnitude",`<div class="sci-formula__eq">${t("M_V = 4.81 - 2.5\\log_{10}(L)")}</div>
    ...
    to WorldSmith; items marked <b>Simplified</b> are reductions of published work.</p>`,i("Oort Cloud Mass And LPC Flux Proxy (WS-derived / Simplified)",`<p>WorldSmith&rsquo;s Oort-cloud mass and long-period-comet flux are not taken from a single published closed-form model. The edge scaling follows exo-Oort Hill-radius work, but the mass and LPC-rate outputs are WorldSmith proxies built from five factors: total giant mass, outer-giant extent, inner-ejector depletion, age saturation, and environmental retention.</p>
      ...
      ${s("Mardling &amp; Aarseth (2001), MNRAS 321, 398; WorldSmith hierarchy guardrails in engine/homeSystem/stability.js")}`)].join("")}function Je(i,r){let l=r/100,p=Math.log(Math.max(i,1e-6)),n=Math.min(1/3,.257-.0161*p),u=(1.07-.21*l)*Math.pow(i,n),f=i*5.51/Math.pow(u,3);return{R:u,rho:f}}function je(i){let r=i.querySelector("#sci-mlr-mass"),l=i.querySelector("#sci-mlr-slider"),p=i.querySelector("#sci-mlr-result");if(r&&l&&p){let _=()=>{let h=X(Number(r.value));p.textContent=`${m(h,4)} L\u2609`};O({numberEl:r,sliderEl:l,min:.075,max:100,step:.01,mode:"log"}),r.addEventListener("input",_),l.addEventListener("input",()=>{r.dispatchEvent(new Event("input"))}),_()}let n=i.querySelector("#sci-hz-mass"),u=i.querySelector("#sci-hz-slider"),f=i.querySelector("#sci-hz-result");if(n&&u&&f){let _=()=>{let h=Number(n.value),S=X(h),C=oe(h),I=ne({luminosityLsol:S,teffK:C});j(f,`${m(I.innerAu,2)} \u2013 ${m(I.outerAu,2)} AU`)};O({numberEl:n,sliderEl:u,min:.075,max:100,step:.01,mode:"log"}),n.addEventListener("input",_),u.addEventListener("input",()=>{n.dispatchEvent(new Event("input"))}),_()}let v=i.querySelector("#sci-dens-mass"),w=i.querySelector("#sci-dens-mass-slider"),M=i.querySelector("#sci-dens-cmf"),P=i.querySelector("#sci-dens-cmf-slider"),R=i.querySelector("#sci-dens-result");if(v&&w&&M&&P&&R){let _=()=>{let{R:h,rho:S}=Je(Number(v.value),Number(M.value));j(R,`${m(h,3)} R\u2295 \u2014 ${m(S,2)} g/cm\xB3`)};O({numberEl:v,sliderEl:w,min:.01,max:100,step:.01,mode:"log"}),O({numberEl:M,sliderEl:P,min:0,max:100,step:1,mode:"linear"}),[v,w,M,P].forEach(h=>h.addEventListener("input",_)),_()}let T=i.querySelector("#sci-hmag-rad"),F=i.querySelector("#sci-hmag-alb"),A=i.querySelector("#sci-hmag-alb-slider"),W=i.querySelector("#sci-hmag-result");if(T&&F&&W){let _=()=>{let h=Number(T.value),S=Number(F.value);if(h>0&&S>0){let C=ce({radiusKm:h,geometricAlbedo:S});W.textContent=`H = ${m(C,2)}`}};A&&O({numberEl:F,sliderEl:A,min:.01,max:1,step:.01,mode:"linear"}),[T,F].forEach(h=>h.addEventListener("input",_)),A&&A.addEventListener("input",_),_()}let H=i.querySelector("#sci-flare-temp"),G=i.querySelector("#sci-flare-temp-slider"),D=i.querySelector("#sci-flare-age"),c=i.querySelector("#sci-flare-age-slider"),g=i.querySelector("#sci-flare-result");if(H&&D&&g){let _=()=>{let h=le({teffK:Number(H.value),ageGyr:Number(D.value)});ge(g,{countLabel:"32",countValue:h.N32,alphaValue:h.alpha})};G&&O({numberEl:H,sliderEl:G,min:2e3,max:1e4,step:100,mode:"linear"}),c&&O({numberEl:D,sliderEl:c,min:.01,max:13,step:.1,mode:"linear"}),[H,G,D,c].filter(Boolean).forEach(h=>h.addEventListener("input",_)),_()}let x=i.querySelector("#sci-leap-len"),$=i.querySelector("#sci-leap-result");if(x&&$){let _=S=>{if(S===0)return{p:0,q:1};let C=0,I=1;for(let L=1;L<=1e4;L++){let q=Math.round(S*L);if(Math.abs(q/L-S)<1e-9){C=q,I=L;break}}return{p:C,q:I}},h=()=>{let S=Number(x.value),C=S-Math.floor(S);if(C<1e-9){U($,[],"No leap cycle needed (integer year)");return}let L=de(C,6).slice(1).map(_).filter(q=>q.q>0);if(L.length===0){U($,[],"No usable cycle found");return}U($,L.map(q=>{let se=Math.abs(q.p/q.q-C),re=se>1e-12?Math.round(1/se):1/0,_e=re===1/0?"exact":`~1 day drift per ${m(re,0)} yr`;return{fraction:`${q.p}/${q.q}`,description:`${q.p} leap day${q.p!==1?"s":""} every ${q.q} years (${_e})`}}))};x.addEventListener("input",h),h()}let k=i.querySelector("#sci-ghz-r"),b=i.querySelector("#sci-ghz-loc"),y=i.querySelector("#sci-ghz-result");if(k&&b&&y){let _=()=>{let h=Number(k.value),S=Number(b.value),C=.53*h,I=.1*h,L=Math.exp(-.5*Math.pow((S-C)/I,2)),q=S>=.47*h&&S<=.6*h;j(y,`${m(L*100,1)}%${q?" (in hard band)":""}`)};[k,b].forEach(h=>h.addEventListener("input",_)),_()}let B=i.querySelector("#sci-tec-grav"),Q=i.querySelector("#sci-tec-grav-slider"),te=i.querySelector("#sci-tec-mtn"),ae=i.querySelector("#sci-tec-shield"),ie=i.querySelector("#sci-tec-root");if(B&&Q&&te&&ae&&ie){let _=()=>{let h=Number(B.value);h>0&&(te.textContent=`${m(pe(h),0)} m`,ae.textContent=`${m(me(h),0)} m`,ie.textContent=`${m(ue(5e3),0)} m`)};O({numberEl:B,sliderEl:Q,min:.05,max:5,step:.01,mode:"log"}),B.addEventListener("input",_),Q.addEventListener("input",()=>{B.dispatchEvent(new Event("input"))}),_()}}var ye=[{id:"stellar",title:"Stellar Physics",count:9,builder:ke},{id:"evolution",title:"Stellar Evolution",count:7,builder:Pe},{id:"planetary",title:"Planetary Physics",count:13,builder:Re},{id:"gasgiant",title:"Gas Giant Physics",count:13,builder:Ae},{id:"interior",title:"Interior &amp; Composition",count:7,builder:Ne},{id:"tectonics",title:"Tectonics &amp; Geodynamics",count:14,builder:Ke},{id:"orbital",title:"Orbital Mechanics",count:22,builder:He},{id:"lagrange",title:"Lagrange Points",count:4,builder:Le},{id:"photometry",title:"Photometry &amp; Magnitudes",count:8,builder:Ee},{id:"atmosphere",title:"Atmosphere &amp; Colour",count:10,builder:Fe},{id:"climate",title:"Climate Classification",count:8,builder:We},{id:"activity",title:"Stellar Activity",count:7,builder:Ge},{id:"calendar",title:"Calendar Systems",count:5,builder:Oe},{id:"cluster",title:"Local Cluster",count:7,builder:Ie},{id:"population",title:"Population Dynamics",count:5,builder:De},{id:"system",title:"System Architecture",count:7,builder:ze},{id:"debris",title:"Debris Disks",count:12,builder:Be},{id:"divergences",title:"Divergences from Published Science",count:36,builder:Ve}];function lt(i){let r=document.createElement("div");r.className="page";let l=ye.map(c=>`<a class="sci-toc__link" data-target="sci-${c.id}" href="#sci-${c.id}">${c.title}</a>`).join(""),p=ye.map((c,g)=>`<details class="sci-section" id="sci-${c.id}"${g===0?" open":""}>
  ...
  `,i.innerHTML="",i.appendChild(r);let n=Array.from(r.querySelectorAll(".sci-section")),u=Array.from(r.querySelectorAll(".sci-formula")),f=Array.from(r.querySelectorAll(".sci-toc__link")),v=r.querySelector("#sciencePageSearch"),w=r.querySelector("#sciencePageSearchStatus"),M=r.querySelector("#sciencePageSearchResults"),P=Ce(r),R=null,T=!1,F=n.find(c=>c.open)?.id||n[0]?.id||null,A=(c,{exclusive:g=!T}={})=>{c&&(g?n.forEach(x=>{x.open=x===c}):c.open=!0,!T&&c.id&&(F=c.id))},W=()=>{R&&(clearTimeout(R),R=null),r.querySelectorAll(".sci-formula.is-search-hit").forEach(c=>{c.classList.remove("is-search-hit")})},H=c=>{if(!c)return;let g=r.querySelector(`#${c}`);if(!g)return;let x=g.closest(".sci-section");A(x),W(),g.classList.add("is-search-hit"),ee(g,{block:"center"}),g.focus?.({preventScroll:!0}),R=window.setTimeout(()=>{g.classList.remove("is-search-hit"),R=null},2200)},G=c=>{let g=N(c);if(W(),!g){T=!1,r.classList.remove("is-search-filtering"),u.forEach(y=>{y.hidden=!1,y.classList.remove("is-search-match","is-search-filtered-out")}),n.forEach(y=>{y.hidden=!1,y.classList.remove("is-search-match","is-search-filtered-out")}),f.forEach(y=>{y.hidden=!1,y.classList.remove("is-search-filtered-out")});let b=n.find(y=>y.id===F)||n[0]||null;return A(b,{exclusive:!0}),[]}let x=be(P,g,P.length),$=new Set(x.map(b=>b.id)),k=new Set(x.map(b=>b.sectionId));return T=!0,r.classList.add("is-search-filtering"),u.forEach(b=>{let y=$.has(b.id);b.hidden=!y,b.classList.toggle("is-search-match",y),b.classList.toggle("is-search-filtered-out",!y)}),n.forEach(b=>{let y=k.has(b.id);b.hidden=!y,b.open=y,b.classList.toggle("is-search-match",y),b.classList.toggle("is-search-filtered-out",!y)}),f.forEach(b=>{let y=k.has(b.dataset.target||"");b.hidden=!y,b.classList.toggle("is-search-filtered-out",!y)}),x},D=(c,g=null)=>{if(!M||!w)return[];let x=N(c);if(!x)return M.hidden=!0,M.innerHTML="",w.hidden=!0,w.textContent="",[];let $=g||be(P,x,P.length),k=$.slice(0,8);return w.hidden=!1,w.textContent=$.length>0?`${$.length} match${$.length===1?"":"es"}${$.length>k.length?`, showing top ${k.length}`:""}`:"No matches found",$.length===0?(M.hidden=!1,M.innerHTML='<div class="sci-search__empty">No formulas or topics matched that search.</div>',[]):(M.hidden=!1,M.innerHTML=k.map(b=>`
          ...
          </button>`).join(""),k)};n.forEach(c=>{c.addEventListener("toggle",()=>{if(c.open){if(!T&&c.id&&(F=c.id),T)return;n.forEach(g=>{g!==c&&(g.open=!1)})}})}),f.forEach(c=>{c.addEventListener("click",g=>{g.preventDefault();let x=c.dataset.target,$=r.querySelector(`#${x}`);$&&(A($),ee($,{block:"start"}))})}),v?.addEventListener("input",()=>{let c=G(v.value);D(v.value,c)}),v?.addEventListener("keydown",c=>{if(c.key!=="Enter")return;let g=G(v.value),[x]=D(v.value,g);x&&(c.preventDefault(),H(x.id))}),M?.addEventListener("click",c=>{let g=c.target.closest(".sci-search__result");g&&H(g.getAttribute("data-target-id"))}),je(r),he().then(c=>fe(r,c))}export{lt as initSciencePage};
