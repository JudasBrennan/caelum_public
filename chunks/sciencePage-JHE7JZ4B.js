import{a as he,b as fe,c as ee}from"./chunk-RFBC3ZM4.js";import{b as pe,d as ue,e as me}from"./chunk-RQTCOANH.js";import{a as de}from"./chunk-BRHMJNX4.js";import"./chunk-VAQJMZ3A.js";import{d as ce}from"./chunk-TPKSL75V.js";import{d as le}from"./chunk-K4NDJ4Q4.js";import{a as O}from"./chunk-WYZYYRUA.js";import{b as J,c as Y}from"./chunk-XMLMEZIZ.js";import{a as L,b as V}from"./chunk-I7AFU66Y.js";import{O as X,R as oe,S as ne}from"./chunk-QLDK4AVT.js";import"./chunk-56JJ2DZ6.js";import"./chunk-EZDQFTR7.js";import"./chunk-FGTR2OSL.js";import{A as m}from"./chunk-JOWNJXZP.js";import"./chunk-RUSIDZ3J.js";import"./chunk-74DXXQZP.js";import"./chunk-VC46IEJQ.js";function j(i,r){return i&&(i.textContent=r==null?"":String(r),i)}function ge(i,{countLabel:r="32",countValue:n="",alphaValue:p=""}={}){return i&&(Y(i,["N",J("sub",{text:r}),` = ${n} \u2014 \u03B1 = ${p}`]),i)}function U(i,r=[],n=""){if(!i)return i;let p=(r||[]).filter(Boolean);if(!p.length)return i.textContent=n==null?"":String(n),i;let l=[];return p.forEach((u,f)=>{f>0&&l.push(J("br")),l.push(J("strong",{text:u?.fraction||""}),` \u2014 ${u?.description||""}`)}),Y(i,l),i}function t(i){return`<span class="sci-math sci-math--block">${i}</span>`}function e(i){return`<span class="sci-math">${i}</span>`}function o(i){return`<table class="sci-vars"><tbody>${i.map(([r,n])=>`<tr><td>${e(r)}</td><td>${n}</td></tr>`).join("")}</tbody></table>`}function s(i){return`<p class="sci-cite">${i}</p>`}function a(i,r){return`<div class="sci-formula"><h3 class="sci-formula__name">${i}</h3>${r}</div>`}var xe=Object.freeze({stellar:[{label:"Star",href:"#/star"}],evolution:[{label:"Star",href:"#/star"}],activity:[{label:"Star",href:"#/star"}],planetary:[{label:"Planet",href:"#/planet"},{label:"Climate",href:"#/climate"}],interior:[{label:"Planet",href:"#/planet"}],tectonics:[{label:"Planet",href:"#/planet"},{label:"Tectonics",href:"#/tectonics"}],atmosphere:[{label:"Planet",href:"#/planet"},{label:"Climate",href:"#/climate"}],climate:[{label:"Climate",href:"#/climate"},{label:"Planet",href:"#/planet"}],gasgiant:[{label:"Planet",href:"#/planet"},{label:"Moon",href:"#/moon"}],orbital:[{label:"Moon",href:"#/moon"},{label:"System",href:"#/system"}],calendar:[{label:"Calendar",href:"#/calendar"}],system:[{label:"System",href:"#/system"},{label:"Star",href:"#/star"}],divergences:[{label:"Validation",href:"#/validation"}]});function $e(i){let r=xe[i]||[];return r.length?`<nav class="sci-related-pages" aria-label="Related app pages">
    <span>Related app pages</span>
    ${r.map(n=>`<a class="sci-related-pages__link" href="${n.href}">${n.label}</a>`).join("")}
  </nav>`:""}function d(i,r){return`<table class="sci-data"><thead><tr>${i.map(n=>`<th>${n}</th>`).join("")}</tr></thead><tbody>${r.map(n=>`<tr>${n.map(p=>`<td>${p}</td>`).join("")}</tr>`).join("")}</tbody></table>`}var Z=Object.freeze({tempMinK:240,tempMaxK:715,pressureMinGPa:.05,pressureMaxGPa:10,left:104,right:664,top:44,bottom:300});function ve(i,r,n){return Math.min(n,Math.max(r,i))}function K(i){let{tempMinK:r,tempMaxK:n,left:p,right:l}=Z,u=(Number(i)-r)/(n-r);return p+ve(u,0,1)*(l-p)}function z(i){let{pressureMinGPa:r,pressureMaxGPa:n,top:p,bottom:l}=Z,u=Math.log10(r),f=Math.log10(n),v=(Math.log10(Math.max(Number(i),r))-u)/(f-u);return l-ve(v,0,1)*(l-p)}function we(i,r){return`${m(i,1)},${m(r,1)}`}function Me(i,r,n=3){let p=[];for(let u=i;u<=r;u+=n){let f=V(u);!f.validRange||!Number.isFinite(f.pressurePa)||p.push({tempK:u,pressureGPa:f.pressurePa/1e9,x:K(u),y:z(f.pressurePa/1e9)})}let l=V(r);return l.validRange&&Number.isFinite(l.pressurePa)&&p.push({tempK:r,pressureGPa:l.pressurePa/1e9,x:K(r),y:z(l.pressurePa/1e9)}),p}function Se(i){return i.length?`M ${i.map(r=>we(r.x,r.y)).join(" L ")}`:""}function qe(){let i=[250,300,350,400,500,600,700],r=[.1,.3,.6,1,2.2,5,10],{left:n,right:p,top:l,bottom:u}=Z;return[...i.map(f=>{let v=K(f);return`<g class="sci-phase-gridline"><line x1="${m(v,1)}" y1="${l}" x2="${m(v,1)}" y2="${u}" /><text x="${m(v,1)}" y="${u+24}">${f} K</text></g>`}),...r.map(f=>{let v=z(f);return`<g class="sci-phase-gridline"><line x1="${n}" y1="${m(v,1)}" x2="${p}" y2="${m(v,1)}" /><text class="sci-phase-y-tick" x="${n-14}" y="${m(v+4,1)}">${f>=1?m(f,1):m(f,1)} GPa</text></g>`})].join("")}function Ce(){return[{tempK:288,pressureGPa:.52,label:"0.52 GPa / 288 K",note:"below liquidus",className:"liquid",dx:10,dy:-12},{tempK:288,pressureGPa:1.2,label:"1.2 GPa / 288 K",note:"ice VI stable",className:"ice",dx:10,dy:-12},{tempK:500,pressureGPa:2.3,label:"2.3 GPa / 500 K",note:"hot deep liquid",className:"warm",dx:10,dy:18}].map(r=>{let n=K(r.tempK),p=z(r.pressureGPa);return`<g class="sci-phase-marker sci-phase-marker--${r.className}" transform="translate(${m(n,1)} ${m(p,1)})">
        <circle r="5" />
        <text x="${r.dx}" y="${r.dy}">${r.label}</text>
        <text x="${r.dx}" y="${r.dy+14}" class="sci-phase-marker__note">${r.note}</text>
      </g>`}).join("")}function Te(){let{left:i,right:r,top:n,bottom:p}=Z,l=r-i,u=p-n,f=i+l/2,v=n+u/2,S=Me(L.iceIhIceIiiLiquid.tempK,715),M=Se(S),P=S[0],R=S[S.length-1],C=P&&R?`${M} L ${r},${n} L ${i},${n} Z`:"",A=[{label:"III",tempK:L.iceIhIceIiiLiquid.tempK,pressureGPa:L.iceIhIceIiiLiquid.pressurePa/1e9},{label:"V",tempK:L.iceIiiIceVLiquid.tempK,pressureGPa:L.iceIiiIceVLiquid.pressurePa/1e9},{label:"VI",tempK:L.iceVIceViLiquid.tempK,pressureGPa:L.iceVIceViLiquid.pressurePa/1e9},{label:"VII",tempK:L.iceViIceViiLiquid.tempK,pressureGPa:L.iceViIceViiLiquid.pressurePa/1e9}].map(G=>{let H=K(G.tempK),I=z(G.pressureGPa);return`<g class="sci-phase-triple">
    ...
    <svg class="sci-phase-diagram" viewBox="0 0 700 360" role="img" aria-label="Caelum ocean-floor water phase diagram">
      ...
      <rect x="${i}" y="${n}" width="${m(K(251.165)-i,1)}" height="${u}" class="sci-phase-unsupported" />
      ${qe()}
    ...
    <div class="sci-phase-legend" aria-label="Diagram legend">
  ...
  </div>`}function W(i){return String(i??"").trim().toLowerCase()}function ke(i,r){if(!r)return 0;let n=W(i.title);if(n===r)return 120;if(n.startsWith(r))return 90;if(n.includes(r))return 70;let p=W(i.sectionTitle);return p===r?52:p.includes(r)?40:W(i.text).includes(r)?18:0}function Pe(i){return Array.from(i.querySelectorAll(".sci-section")).flatMap(r=>{let n=String(r.id||"").trim(),p=r.querySelector(".sci-section__title")?.textContent?.trim()||"Science Section";return Array.from(r.querySelectorAll(".sci-formula")).map((l,u)=>{let f=`${n}-formula-${u+1}`;return l.id=f,l.setAttribute("tabindex","-1"),{id:f,sectionId:n,sectionTitle:p,title:l.querySelector(".sci-formula__name")?.textContent?.trim()||"Formula",text:l.textContent||""}})})}function be(i,r,n=8){let p=W(r);return p?i.map(l=>({entry:l,score:ke(l,p)})).filter(l=>l.score>0).sort((l,u)=>u.score-l.score||l.entry.sectionTitle.localeCompare(u.entry.sectionTitle)||l.entry.title.localeCompare(u.entry.title)).slice(0,n).map(l=>l.entry):[]}function Re(){return[a("Mass-Luminosity Relation",`<div class="sci-formula__eq">${t("L = c \\cdot M^{\\alpha}")}</div>
      ...
      ${s("Barnes (2007) ApJ 669, 1167; Mamajek &amp; Hillenbrand (2008) ApJ 687, 1264; Wood et al. (2005) ApJ 628, L143; Ranjan &amp; Sasselov (2017) Astrobiology 17, 169; Arney et al. (2016) Astrobiology 16, 873")}`)].join("")}function Ae(){return[a("Metallicity Conversion",`<div class="sci-formula__eq">${t("Z = Z_\\odot \\cdot 10^{[\\text{Fe/H}]}")}</div>
      ...
      ${s("Hurley, Pols &amp; Tout (2000), MNRAS 315, 543")}`),a("Stellar Track Provider",`<p>Automatic stellar luminosity, radius, and effective temperature pass through a provider layer. The default provider remains the deterministic analytic SSE/Hurley-Tout track for stable existing behaviour. Explicit grid/auto modes prefer bundled MIST v2.5 main-sequence grids for covered mass, fractional age, composition, and requested rotation, then use the compact generic grid before analytic fallback.</p>
      ...
      ${d(["Mode","Behavior","Fallback"],[["analytic","Uses internal SSE/Hurley-Tout equations","None"],["grid","Uses bundled MIST grids when in coverage","Generic grid, then analytic"],["auto","Prefers bundled MIST/generic grids only when in coverage","Analytic"]])}
      ...
      <p>Cool dwarfs also expose a generic photosphere correction. The raw track temperature is preserved separately, while the visible photospheric temperature can be lowered by a smooth function of mass, metallicity, fractional main-sequence age, and Rossby/activity context.</p>
      ...
      <p><b>No catalog anchoring:</b> known stars are validation fixtures only. Caelum does not use named-star spectral classes, observed radii, or catalog temperatures as runtime truth for generated stars.</p>`),a("Analytic Stellar Lifecycle Track",`<div class="sci-formula__eq">${t("S(t) = \\{\\text{MS},\\,\\text{TMS},\\,\\text{subgiant/blue dwarf},\\,\\text{giant},\\,\\text{He burning},\\,\\text{AGB/supergiant},\\,\\text{remnant}\\}")}</div>
      ...
      ${s("Hurley, Pols &amp; Tout (2000), MNRAS 315, 543; Kalirai et al. (2008), ApJ 676, 594; Spera et al. (2015), MNRAS 451, 4086; Kopparapu et al. (2013/2014)")}`)].join("")}function He(){return[a("Mass&ndash;Radius Relation (Radius-First)",`<div class="sci-formula__eq">${t("R_\\oplus = (1.07 - 0.21 \\cdot \\text{CMF}) \\cdot M_\\oplus^{\\,\\alpha}")}</div>
      ...
      <p>Pressure-dependent boiling point of water. ${e("L_v/R_g = 40700/8.314 = 4894.4")} K. Calibrated: 1 atm &rarr; 373 K, 218 atm &rarr; 647 K (critical point).</p>`),a("Ocean-Floor Water Phase Boundary",`${Te()}
      ...
      ${d(["Boundary used","Temperature range","Anchor in model"],[["ice III / liquid","251.165&ndash;256.164 K","0.209&ndash;0.350 GPa"],["ice V / liquid","256.164&ndash;273.31 K","0.350&ndash;0.632 GPa"],["ice VI / liquid","273.31&ndash;355 K","0.632&ndash;2.216 GPa"],["ice VII / liquid","355&ndash;715 K",`${m(V(550).pressurePa/1e9,2)} GPa at 550 K`]])}
      ...
      ${d(["Rotation period","Cells"],[["&ge; 48 h","1 (single Hadley)"],["6 &ndash; 48 h","3 (Earth-like)"],["3 &ndash; 6 h","7 (rapid rotator)"],["&lt; 3 h","5 (very rapid)"]])}`)].join("")}function Ee(){return[a("Mass&ndash;Radius Relation",`<p>Two-regime power law calibrated to Solar System giants:</p>
      ...
      ${s("Peale, Cassen &amp; Reynolds (1979); Wahl et al. (2016); Lainey et al. (2009, 2017); Fuller, Luan &amp; Quataert (2016)")}`)].join("")}function Le(){return[a("Kepler&rsquo;s Third Law (Moon Orbit)",`<div class="sci-formula__eq">${t("P = \\frac{2\\pi\\sqrt{a^3 / G(M_p + M_m)}}{86400} \\text{ days}")}</div>
      ...
      evidence; stable points do not guarantee populated Trojans.</p>`)].join("")}function Ge(){return[a("Stellar Absolute Magnitude",`<div class="sci-formula__eq">${t("M_V = 4.81 - 2.5\\log_{10}(L)")}</div>
      ...
      ${s("Winn (2010), Exoplanet Transits and Occultations; Perryman (2018), The Exoplanet Handbook; Seager &amp; Sasselov (2000), ApJ 537, 916")}`)].join("")}function Ie(){return[a("Scale Height Correction",`<div class="sci-formula__eq">${t("\\frac{H_{\\text{planet}}}{H_\\oplus} = \\frac{T / T_\\oplus}{g / g_\\oplus}")}</div>
      ...
      elevated terrain. Used for all climate zone calculations.</p>`)].join("")}function Ne(){return[a("Power-Law Flare Rate",`<div class="sci-formula__eq">${t("N(> E) = N_{32} \\left(\\frac{E}{10^{32} \\text{ erg}}\\right)^{-\\alpha}")}</div>
      ...
      observations that M-dwarf activity varies less over magnetic cycles.</p>`)].join("")}function De(){return[a("Local Day Scale",`<div class="sci-formula__eq">${t("\\text{scale} = \\frac{P_{\\text{rot}}}{24}")}</div>
      ...
      <p>Day-of-week index for year ${e("y")}, where ${e("L_i")} is the length of year ${e("i")} and ${e("D_w")} is days per week. Computed in O(rules) time by counting leap rule firings.</p>`)].join("")}function We(){return[a("Neighbourhood Volume",`<div class="sci-formula__eq">${t("V = \\frac{4}{3}\\pi R^3")}</div>
      ...
      <p>Galactic disk flattening for large neighbourhood radii. At 50 ly the neighbourhood is spherical; at 500 ly z &asymp; 0.55; floors at 0.15.</p>`)].join("")}function Ke(){return[a("Ocean Fraction by Water Regime",`${d(["Water regime","Ocean fraction"],[["Dry","0%"],["Shallow oceans","50%"],["Extensive oceans","71%"],["Global ocean","90%"],["Deep ocean","95%"],["Ice world","0%"]])}
      ...
      ${s("Zipf (1949), Human Behavior and the Principle of Least Effort")}`)].join("")}function Be(){return[a("Water-Aware Radius (Zeng & Sasselov 2016)",`<div class="sci-formula__eq">${t("R = R_{\\text{dry}} \\cdot \\left(1 + \\left(\\frac{R_{50}}{R_{\\text{ref}}} - 1\\right) \\cdot \\min\\!\\left(\\frac{\\text{WMF}}{0.5},\\;1\\right)\\right)")}</div>
      ...
      ${s("Ortenzi et al. (2020), Sci. Rep. 10, 10907")}`)].join("")}function ze(){return[a("Maximum Mountain Height",`<div class="sci-formula__eq">${t("H_{\\max} = \\frac{\\sigma_y}{\\rho \\cdot g} \\approx \\frac{9{,}267}{g} \\text{ m}")}</div>
    ...
    </div>`].join("")}function Ve(){return[a("Mean-Motion Resonance Positions",`<div class="sci-formula__eq">${t("a_{\\text{res}} = a_p\\,(p/q)^{2/3}")}</div>
      ...
      Detectable if &gt; 0.1 (easily), marginal 0.01&ndash;0.1, undetectable &lt; 0.01.</p>`)].join("")}function Je(){let i=(r,n)=>`<div class="sci-formula"><h3 class="sci-formula__name">${r}</h3>${n}</div>`;return[`<p class="sci-diverge-intro">Caelum aims to reproduce published astrophysical models wherever possible.
    ...
    to Caelum; items marked <b>Simplified</b> are reductions of published work.</p>`,i("Oort Cloud Mass And LPC Flux Proxy (WS-derived / Simplified)",`<p>Caelum&rsquo;s Oort-cloud mass and long-period-comet flux are not taken from a single published closed-form model. The edge scaling follows exo-Oort Hill-radius work, but the mass and LPC-rate outputs are Caelum proxies built from five factors: total giant mass, outer-giant extent, inner-ejector depletion, age saturation, and environmental retention.</p>
      ...
      ${s("CMF scaling: Zeng &amp; Sasselov (2016). Exponent: Caelum fit to Solar System data.")}`),i("Core Solidification Timescale (Simplified)",`<p>The formula ${e("\\tau = 2 + 12 \\cdot \\text{CMF} \\cdot \\sqrt{M_\\oplus}")} is a Caelum
      ...
      patterns that depend on composition, pressure, and stellar spectrum.</p>`)].join("")}function je(){return[a("Titius-Bode Orbit Spacing",`<div class="sci-formula__eq">${t("a_n = \\begin{cases} a_1 & n = 1 \\\\ a_1 + s \\cdot 2^{n-2} & n \\ge 2 \\end{cases}")}</div>
      ...
      ${s("Caelum stellar-neighbourhood hazard model; Lineweaver et al. (2004); Gehrels et al. (2003); NASA Oort Cloud and comet references; Oort-cloud literature such as Dones et al. (2004) and Kaib &amp; Quinn (2009)")}`)].join("")}function Ue(i,r){let n=r/100,p=Math.log(Math.max(i,1e-6)),l=Math.min(1/3,.257-.0161*p),u=(1.07-.21*n)*Math.pow(i,l),f=i*5.51/Math.pow(u,3);return{R:u,rho:f}}function Ze(i){let r=i.querySelector("#sci-mlr-mass"),n=i.querySelector("#sci-mlr-slider"),p=i.querySelector("#sci-mlr-result");if(r&&n&&p){let _=()=>{let h=X(Number(r.value));p.textContent=`${m(h,4)} L\u2609`};O({numberEl:r,sliderEl:n,min:.075,max:100,step:.01,mode:"log"}),r.addEventListener("input",_),n.addEventListener("input",()=>{r.dispatchEvent(new Event("input"))}),_()}let l=i.querySelector("#sci-hz-mass"),u=i.querySelector("#sci-hz-slider"),f=i.querySelector("#sci-hz-result");if(l&&u&&f){let _=()=>{let h=Number(l.value),w=X(h),T=oe(h),N=ne({luminosityLsol:w,teffK:T});j(f,`${m(N.innerAu,2)} \u2013 ${m(N.outerAu,2)} AU`)};O({numberEl:l,sliderEl:u,min:.075,max:100,step:.01,mode:"log"}),l.addEventListener("input",_),u.addEventListener("input",()=>{l.dispatchEvent(new Event("input"))}),_()}let v=i.querySelector("#sci-dens-mass"),S=i.querySelector("#sci-dens-mass-slider"),M=i.querySelector("#sci-dens-cmf"),P=i.querySelector("#sci-dens-cmf-slider"),R=i.querySelector("#sci-dens-result");if(v&&S&&M&&P&&R){let _=()=>{let{R:h,rho:w}=Ue(Number(v.value),Number(M.value));j(R,`${m(h,3)} R\u2295 \u2014 ${m(w,2)} g/cm\xB3`)};O({numberEl:v,sliderEl:S,min:.01,max:100,step:.01,mode:"log"}),O({numberEl:M,sliderEl:P,min:0,max:100,step:1,mode:"linear"}),[v,S,M,P].forEach(h=>h.addEventListener("input",_)),_()}let C=i.querySelector("#sci-hmag-rad"),F=i.querySelector("#sci-hmag-alb"),A=i.querySelector("#sci-hmag-alb-slider"),G=i.querySelector("#sci-hmag-result");if(C&&F&&G){let _=()=>{let h=Number(C.value),w=Number(F.value);if(h>0&&w>0){let T=ce({radiusKm:h,geometricAlbedo:w});G.textContent=`H = ${m(T,2)}`}};A&&O({numberEl:F,sliderEl:A,min:.01,max:1,step:.01,mode:"linear"}),[C,F].forEach(h=>h.addEventListener("input",_)),A&&A.addEventListener("input",_),_()}let H=i.querySelector("#sci-flare-temp"),I=i.querySelector("#sci-flare-temp-slider"),D=i.querySelector("#sci-flare-age"),c=i.querySelector("#sci-flare-age-slider"),g=i.querySelector("#sci-flare-result");if(H&&D&&g){let _=()=>{let h=le({teffK:Number(H.value),ageGyr:Number(D.value)});ge(g,{countLabel:"32",countValue:h.N32,alphaValue:h.alpha})};I&&O({numberEl:H,sliderEl:I,min:2e3,max:1e4,step:100,mode:"linear"}),c&&O({numberEl:D,sliderEl:c,min:.01,max:13,step:.1,mode:"linear"}),[H,I,D,c].filter(Boolean).forEach(h=>h.addEventListener("input",_)),_()}let x=i.querySelector("#sci-leap-len"),$=i.querySelector("#sci-leap-result");if(x&&$){let _=w=>{if(w===0)return{p:0,q:1};let T=0,N=1;for(let E=1;E<=1e4;E++){let q=Math.round(w*E);if(Math.abs(q/E-w)<1e-9){T=q,N=E;break}}return{p:T,q:N}},h=()=>{let w=Number(x.value),T=w-Math.floor(w);if(T<1e-9){U($,[],"No leap cycle needed (integer year)");return}let E=de(T,6).slice(1).map(_).filter(q=>q.q>0);if(E.length===0){U($,[],"No usable cycle found");return}U($,E.map(q=>{let se=Math.abs(q.p/q.q-T),re=se>1e-12?Math.round(1/se):1/0,_e=re===1/0?"exact":`~1 day drift per ${m(re,0)} yr`;return{fraction:`${q.p}/${q.q}`,description:`${q.p} leap day${q.p!==1?"s":""} every ${q.q} years (${_e})`}}))};x.addEventListener("input",h),h()}let k=i.querySelector("#sci-ghz-r"),b=i.querySelector("#sci-ghz-loc"),y=i.querySelector("#sci-ghz-result");if(k&&b&&y){let _=()=>{let h=Number(k.value),w=Number(b.value),T=.53*h,N=.1*h,E=Math.exp(-.5*Math.pow((w-T)/N,2)),q=w>=.47*h&&w<=.6*h;j(y,`${m(E*100,1)}%${q?" (in hard band)":""}`)};[k,b].forEach(h=>h.addEventListener("input",_)),_()}let B=i.querySelector("#sci-tec-grav"),Q=i.querySelector("#sci-tec-grav-slider"),te=i.querySelector("#sci-tec-mtn"),ae=i.querySelector("#sci-tec-shield"),ie=i.querySelector("#sci-tec-root");if(B&&Q&&te&&ae&&ie){let _=()=>{let h=Number(B.value);h>0&&(te.textContent=`${m(pe(h),0)} m`,ae.textContent=`${m(me(h),0)} m`,ie.textContent=`${m(ue(5e3),0)} m`)};O({numberEl:B,sliderEl:Q,min:.05,max:5,step:.01,mode:"log"}),B.addEventListener("input",_),Q.addEventListener("input",()=>{B.dispatchEvent(new Event("input"))}),_()}}var ye=[{id:"stellar",title:"Stellar Physics",count:9,builder:Re},{id:"evolution",title:"Stellar Evolution",count:9,builder:Ae},{id:"planetary",title:"Planetary Physics",count:14,builder:He},{id:"gasgiant",title:"Gas Giant Physics",count:13,builder:Ee},{id:"interior",title:"Interior &amp; Composition",count:10,builder:Be},{id:"tectonics",title:"Tectonics &amp; Geodynamics",count:14,builder:ze},{id:"orbital",title:"Orbital Mechanics",count:23,builder:Le},{id:"lagrange",title:"Lagrange Points",count:4,builder:Fe},{id:"photometry",title:"Photometry &amp; Magnitudes",count:9,builder:Ge},{id:"atmosphere",title:"Atmosphere &amp; Colour",count:11,builder:Ie},{id:"climate",title:"Climate Classification",count:8,builder:Oe},{id:"activity",title:"Stellar Activity",count:7,builder:Ne},{id:"calendar",title:"Calendar Systems",count:5,builder:De},{id:"cluster",title:"Local Cluster",count:7,builder:We},{id:"population",title:"Population Dynamics",count:6,builder:Ke},{id:"system",title:"System Architecture",count:8,builder:je},{id:"debris",title:"Debris Disks",count:12,builder:Ve},{id:"divergences",title:"Divergences from Published Science",count:36,builder:Je}];function dt(i){let r=document.createElement("div");r.className="page";let n=ye.map(c=>`<a class="sci-toc__link" data-target="sci-${c.id}" href="#sci-${c.id}">${c.title}</a>`).join(""),p=ye.map((c,g)=>`<details class="sci-section" id="sci-${c.id}"${g===0?" open":""}>
        ...
        <div class="sci-section__body">${$e(c.id)}${c.builder()}</div>
  ...
  `,i.innerHTML="",i.appendChild(r);let l=Array.from(r.querySelectorAll(".sci-section")),u=Array.from(r.querySelectorAll(".sci-formula")),f=Array.from(r.querySelectorAll(".sci-toc__link")),v=r.querySelector("#sciencePageSearch"),S=r.querySelector("#sciencePageSearchStatus"),M=r.querySelector("#sciencePageSearchResults"),P=Pe(r),R=null,C=!1,F=l.find(c=>c.open)?.id||l[0]?.id||null,A=(c,{exclusive:g=!C}={})=>{c&&(g?l.forEach(x=>{x.open=x===c}):c.open=!0,!C&&c.id&&(F=c.id))},G=()=>{R&&(clearTimeout(R),R=null),r.querySelectorAll(".sci-formula.is-search-hit").forEach(c=>{c.classList.remove("is-search-hit")})},H=c=>{if(!c)return;let g=r.querySelector(`#${c}`);if(!g)return;let x=g.closest(".sci-section");A(x),G(),g.classList.add("is-search-hit"),ee(g,{block:"center"}),g.focus?.({preventScroll:!0}),R=window.setTimeout(()=>{g.classList.remove("is-search-hit"),R=null},2200)},I=c=>{let g=W(c);if(G(),!g){C=!1,r.classList.remove("is-search-filtering"),u.forEach(y=>{y.hidden=!1,y.classList.remove("is-search-match","is-search-filtered-out")}),l.forEach(y=>{y.hidden=!1,y.classList.remove("is-search-match","is-search-filtered-out")}),f.forEach(y=>{y.hidden=!1,y.classList.remove("is-search-filtered-out")});let b=l.find(y=>y.id===F)||l[0]||null;return A(b,{exclusive:!0}),[]}let x=be(P,g,P.length),$=new Set(x.map(b=>b.id)),k=new Set(x.map(b=>b.sectionId));return C=!0,r.classList.add("is-search-filtering"),u.forEach(b=>{let y=$.has(b.id);b.hidden=!y,b.classList.toggle("is-search-match",y),b.classList.toggle("is-search-filtered-out",!y)}),l.forEach(b=>{let y=k.has(b.id);b.hidden=!y,b.open=y,b.classList.toggle("is-search-match",y),b.classList.toggle("is-search-filtered-out",!y)}),f.forEach(b=>{let y=k.has(b.dataset.target||"");b.hidden=!y,b.classList.toggle("is-search-filtered-out",!y)}),x},D=(c,g=null)=>{if(!M||!S)return[];let x=W(c);if(!x)return M.hidden=!0,M.innerHTML="",S.hidden=!0,S.textContent="",[];let $=g||be(P,x,P.length),k=$.slice(0,8);return S.hidden=!1,S.textContent=$.length>0?`${$.length} match${$.length===1?"":"es"}${$.length>k.length?`, showing top ${k.length}`:""}`:"No matches found",$.length===0?(M.hidden=!1,M.innerHTML='<div class="sci-search__empty">No formulas or topics matched that search.</div>',[]):(M.hidden=!1,M.innerHTML=k.map(b=>`
          ...
          </button>`).join(""),k)};l.forEach(c=>{c.addEventListener("toggle",()=>{if(c.open){if(!C&&c.id&&(F=c.id),C)return;l.forEach(g=>{g!==c&&(g.open=!1)})}})}),f.forEach(c=>{c.addEventListener("click",g=>{g.preventDefault();let x=c.dataset.target,$=r.querySelector(`#${x}`);$&&(A($),ee($,{block:"start"}))})}),v?.addEventListener("input",()=>{let c=I(v.value);D(v.value,c)}),v?.addEventListener("keydown",c=>{if(c.key!=="Enter")return;let g=I(v.value),[x]=D(v.value,g);x&&(c.preventDefault(),H(x.id))}),M?.addEventListener("click",c=>{let g=c.target.closest(".sci-search__result");g&&H(g.getAttribute("data-target-id"))}),Ze(r),he().then(c=>fe(r,c))}export{dt as initSciencePage};
