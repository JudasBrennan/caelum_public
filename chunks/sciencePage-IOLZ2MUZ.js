import{a as fe,b as ge,c as ee}from"./chunk-ET2G6WPW.js";import{a as pe}from"./chunk-4UJIQIOS.js";import{b as ue,d as me,e as he}from"./chunk-ME2DCGO3.js";import{d as ce}from"./chunk-WXXZZXYD.js";import{d as de}from"./chunk-VGAEQJV7.js";import"./chunk-ZXIDEW7O.js";import{a as O}from"./chunk-EHRPTLYW.js";import{a as oe}from"./chunk-EZA3XW5K.js";import{b as J,c as Y}from"./chunk-TR2TDQN3.js";import{a as L,b as z}from"./chunk-LHEZNGZ5.js";import{Q as ne,R as le,T as X}from"./chunk-NYJOF3NP.js";import"./chunk-Q3EXAOWE.js";import"./chunk-ASLSWSPR.js";import"./chunk-MTXM7GCO.js";import{A as m}from"./chunk-JAT3QFT3.js";import"./chunk-6DA3R6ZF.js";import"./chunk-RC2KHOII.js";import"./chunk-5RCZOBSY.js";function j(i,r){return i&&(i.textContent=r==null?"":String(r),i)}function be(i,{countLabel:r="32",countValue:n="",alphaValue:p=""}={}){return i&&(Y(i,["N",J("sub",{text:r}),` = ${n} \u2014 \u03B1 = ${p}`]),i)}function U(i,r=[],n=""){if(!i)return i;let p=(r||[]).filter(Boolean);if(!p.length)return i.textContent=n==null?"":String(n),i;let l=[];return p.forEach((u,f)=>{f>0&&l.push(J("br")),l.push(J("strong",{text:u?.fraction||""}),` \u2014 ${u?.description||""}`)}),Y(i,l),i}function t(i){return`<span class="sci-math sci-math--block">${i}</span>`}function e(i){return`<span class="sci-math">${i}</span>`}function o(i){return`<table class="sci-vars"><tbody>${i.map(([r,n])=>`<tr><td>${e(r)}</td><td>${n}</td></tr>`).join("")}</tbody></table>`}function s(i){return`<p class="sci-cite">${i}</p>`}function a(i,r){return`<div class="sci-formula"><h3 class="sci-formula__name">${i}</h3>${r}</div>`}var $e=Object.freeze({stellar:[{label:"Star",href:"#/star"}],evolution:[{label:"Star",href:"#/star"}],activity:[{label:"Star",href:"#/star"}],planetary:[{label:"Planet",href:"#/planet"},{label:"Climate",href:"#/climate"}],interior:[{label:"Planet",href:"#/planet"}],tectonics:[{label:"Planet",href:"#/planet"},{label:"Tectonics",href:"#/tectonics"}],atmosphere:[{label:"Planet",href:"#/planet"},{label:"Climate",href:"#/climate"}],climate:[{label:"Climate",href:"#/climate"},{label:"Planet",href:"#/planet"}],gasgiant:[{label:"Planet",href:"#/planet"},{label:"Moon",href:"#/moon"}],orbital:[{label:"Moon",href:"#/moon"},{label:"System",href:"#/system"}],calendar:[{label:"Calendar",href:"#/calendar"}],system:[{label:"System",href:"#/system"},{label:"Star",href:"#/star"}],divergences:[{label:"Validation",href:"#/validation"}]});function Me(i){let r=$e[i]||[];return r.length?`<nav class="sci-related-pages" aria-label="Related app pages">
    ...
    <div class="sci-phase-legend" aria-label="Diagram legend">
      ...
      ${c(["Boundary used","Temperature range","Anchor in model"],[["ice III / liquid","251.165&ndash;256.164 K","0.209&ndash;0.350 GPa"],["ice V / liquid","256.164&ndash;273.31 K","0.350&ndash;0.632 GPa"],["ice VI / liquid","273.31&ndash;355 K","0.632&ndash;2.216 GPa"],["ice VII / liquid","355&ndash;715 K",`${m(z(550).pressurePa/1e9,2)} GPa at 550 K`]])}
      ...
      <p>Apparent lunar phase cycle as seen from the planet&rsquo;s surface. The displayed subtraction applies to prograde motion. For a retrograde moon, add the positive reciprocal periods instead. More generally, subtract signed cycle frequencies in a common reference frame, then take the magnitude.</p>
      <p>The same rule gives a planet&rsquo;s mean solar day from its sidereal rotation and orbital year: prograde rotation subtracts the year frequency; retrograde rotation adds it. Venus has a roughly 243-day sidereal rotation and a 116.75-day solar day. Slow prograde rotation still uses the absolute frequency difference.</p>
      <p>Equal signed frequencies have no recurring solar-day cycle. In that case the calendar explicitly uses sidereal rotation as a civil-day unit. These are mean-cycle estimates; highly inclined or polar geometry, libration, and the changing apparent speed of the Sun require a phase-resolved model.</p>
      ${s("University of Nebraska-Lincoln, NAAP: Sidereal vs. Synodic; NASA Space Place: All About Venus")}`),a("Tidal Heating",`<div class="sci-formula__eq">${t("\\dot{E} = \\frac{21}{2}\\,\\frac{k_2}{Q} \\, \\frac{G \\, M_p^{\\,2} \\, R_m^{\\,5} \\, n}{a^6} \\, f(e)")}</div>
      ...
      <p>The parent supplies its modeled magnetopause range. A moon inside the minimum dayside
      scale is classified inside; one beyond the maximum is classified outside; overlap or a
      missing bound remains indeterminate. The older point fallback is used only when a parent
      has no explicit environment range:</p>
      <div class="sci-formula__eq">${t("L_{\\text{mp}} = 10 \\times B_{\\oplus}^{1/3} \\times d_{\\text{AU}}^{1/3} \\;\\text{[planet radii]}")}</div>
      <p>For a resolved reference point, the simplified dayside trapped-belt term is zero beyond
      that point. This is not a claim that real exposure is zero: magnetotail shape and orbital
      phase can change the moon&rsquo;s actual environment, and an overlapping range keeps the dose
      unresolved.</p>
      ...
      <p>Low-light environments favour broader-spectrum absorption (darker pigments). Log&#8322; scaling gives smooth correction across orders of magnitude of insolation ${e("S")}.</p>`),a("Effective Pressure for Sky Colour","<p>The scale-height-adjusted effective pressure is used to index a sky colour lookup table derived from PanoptesV atmospheric data. This accounts for the fact that a low-gravity world with 1 atm surface pressure has a thicker optical column than Earth at 1 atm.</p>"),a("Rocky Planet Radiation Environment",`<p>Rocky planets report a bounded radiation-environment context that keeps high-energy
      stellar forcing, atmospheric particle attenuation, atmospheric/ozone UV shielding, and
      magnetic space-environment context separate.</p>
      <div class="sci-formula__eq">${t("H_{\\text{surface}} = 0.56\\,H_{\\text{particle,atm}} + 0.44\\,H_{\\text{UV,atm+ozone}}")}</div>
      <div class="sci-formula__eq">${t("P_{\\text{rad}} = 1 - 0.88\\,H_{\\text{surface}}")}</div>
      <p>${e("P_{\\text{rad}}")} is a comparative habitability input, not a physical dose. Its
      scalar value is deliberately <b>field-neutral</b>: intrinsic dynamo state and magnetopause
      range remain visible context but do not create a generic shielding bonus. Dense atmospheres
      can attenuate particles without a strong dynamo, ozone/photochemistry is needed for UV
      shielding, and the effect of a magnetic field depends on topology, wind interaction, and
      particle transport that this model does not solve.</p>
      ...
      <p>Galactic disk flattening for large neighbourhood radii. At 50 ly the neighbourhood is spherical; at 500 ly z &asymp; 0.55; floors at 0.15.</p>`)].join("")}function Ve(){return[a("Ocean Fraction by Water Regime",`${c(["Water regime","Ocean fraction"],[["Dry","0%"],["Shallow oceans","50%"],["Extensive oceans","71%"],["Global ocean","90%"],["Deep ocean","95%"],["Ice world","0%"]])}
      ...
      ${s("Zeng, L. &amp; Jacobsen, S. (2017)")}`),a("Rocky Dynamo Evolution and Magnetic Field (v1/v3)",`<p><b>Core phase and magnetic activity are separate domains.</b> The Earth-anchored
      core-crystallisation model below describes material state only. A liquid outer core is
      neither a promise of a dynamo nor a mass-based veto. The dynamo owner separately evaluates
      conducting source layers, convective/entropy power, magnetic Reynolds number, and field
      morphology.</p>
      <p><b>Published numerical domain.</b> The quantitative massive-rocky tier is a deterministic
      reduced extract of EVO-1D-SUPER-EARTH v1.1. Trilinear interpolation is allowed only inside
      the complete coupled axes from 1 through 10 Earth masses,
      ${e("1 \\le M/M_\\oplus \\le 10")},
      ${e("0.10 \\le \\mathrm{CMF} \\le 0.90")}, and
      ${e("0 \\le t \\le 10\\;\\mathrm{Gyr}")}; it never extrapolates or reconstructs the
      grid as separable one-dimensional trends. Valid 10&ndash;20 Earth-mass rocky planets therefore
      report <b>indeterminate</b>, not inactive. The sub-Earth path remains a separate,
      low-confidence comparative estimator, with a closed 1.0&ndash;1.2 Earth-mass overlap.</p>
      <p><b>Source scenarios.</b> The fixed set contains the coupled core/BMO history with
      Stixrude conductivity, the same coupled history with Nakajima conductivity as conditional
      evidence, and a conditional BMO-absent core-only reference. These branches share assumptions
      and are <em>not</em> independent samples: every statistical weight is null and no probability
      average is reported.</p>
      <div class="sci-formula__eq">${t("\\Phi = p_S M_S \\Omega^3 L^2, \\qquad R_m = \\mu_0 \\sigma v L")}</div>
      <p>${e("\\Phi")} is reconstructed from the published source quantities. Worldsmith v1
      explicitly maps that source dissipation power to the Zuluaga&ndash;Cuartas convective-power
      projection; this is a documented mapping, not an additional observation. ${e("R_m<40")}
      is inactive, 40&ndash;50 is marginal, and ${e("R_m>50")} still needs the independent energetic
      criterion. Rotation recomputes velocity, ${e("R_m")}, internal field, and Rossby morphology
      only inside the reviewed 5&ndash;2400 hour range; there is no period-only shutdown.</p>
      <div class="sci-formula__eq">${t("B_{\\text{surf,dip}} = \\frac{B_{\\text{rms}}}{7} \\left(\\frac{r_{\\text{source}}}{R_p}\\right)^3")}</div>
      <p>The cubic term projects each physical source radius to the surface. The source&rsquo;s
      published large-scale value is retained as a diagnostic; the authoritative equatorial
      interval allows the published dipole convention to span one-half to one times that value.
      Simultaneous sources combine as vector-magnitude bounds rather than by adding central values.
      Modified local Rossby number classifies metallic-core morphology as dipolar below 0.04,
      transitional through 0.10, and multipolar above 0.10.</p>
      <p><b>Applicability and confidence.</b> Source-compatible inferred chemistry can use the
      numerical tier at most at medium confidence. Unreviewed manual chemistry, water-rich
      structure, off-source thermal boundaries, or out-of-domain spin keeps the source result only
      as labelled conditional evidence. Conditional values do not become authoritative field,
      magnetopause, atmosphere, radiation, productivity, or habitability inputs.</p>
      <p><b>Evidence limit:</b> no magnetic field has been directly observed for a super-Earth and
      none is used as a calibration anchor. The grid reproduces a published physical model;
      Worldsmith&rsquo;s interpolation, applicability gates, field-convention interval, and downstream
      mappings are derived predictions. Solar System bodies are diagnostic fixtures passed through
      the same generic engine, never runtime identity branches.</p>
      ${s("Lherm, Nakajima &amp; Blackman (2026), doi:10.1029/2025JE009447; EVO-1D-SUPER-EARTH v1.1, doi:10.5281/zenodo.18349207; Nakajima et al. (2026), doi:10.1038/s41550-025-02729-x; Christensen &amp; Aubert (2006), doi:10.1111/j.1365-246X.2006.03009.x; Olson &amp; Christensen (2006), doi:10.1016/j.epsl.2006.08.008; Zuluaga &amp; Cuartas (2012), doi:10.1016/j.icarus.2011.10.014")}`),a("Rocky Magnetosphere Pressure Balance (v2)",`<div class="sci-formula__eq">${t("\\frac{R_{mp}}{R_p} = 10 \\left(\\frac{B_{eq}}{B_{\\oplus}}\\right)^{1/3} \\left(\\frac{P_{ram}}{P_{\\oplus}}\\right)^{-1/6}")}</div>
      <p>The dayside standoff is a range calculation. Its minimum pairs the weakest permitted
      equatorial dipole with the strongest wind pressure; its maximum pairs the strongest field
      with the weakest pressure. A range wholly at or below one planet radius is collapsed, a
      range wholly above it has a standing cavity, and a crossing range is scenario-dependent.
      Missing field or pressure bounds remain indeterminate rather than becoming zero or false.</p>
      <p>The external-pressure term is <b>stellar-wind ram pressure only</b>. Thermal pressure,
      stellar magnetic pressure, reconnection, magnetotail geometry, topology, and time-dependent
      particle transport are not solved. A standing dipole cavity is space-environment context;
      it is not converted into a numeric atmosphere-retention, surface-radiation, health,
      productivity, or habitability bonus.</p>
      ${s("NASA Earth magnetosphere overview; Zuluaga &amp; Cuartas (2012), doi:10.1016/j.icarus.2011.10.014")}`),a("Core Crystallisation and Inner-Core Growth",`<div class="sci-formula__eq">${t("\\tau_{\\text{proxy}} = 2 + 12\\,\\text{CMF}\\sqrt{\\max(M,0.01)}, \\qquad p = t/\\tau_{\\text{proxy}}")}</div>
